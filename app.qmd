...
However, after converting the contents of `pkgApp` into an app-package (like the versions in the `main` and `02_roxygen2` branches), the contents of `R/` are loaded with `devtools::load_all()`. The `app.R` file should account for running apps from an app-package (not a `shiny` project).
...
`runApp()` is designed to run standalone apps located in a separate directory containing `ui.R` and `server.R` (or `app.R`) files. This feature is beneficial for app-packages, because apps can be organized inside a dedicated sub-directory and be launched with `runApp()`.
...
We'll make `test` an optional `TRUE`/`FALSE` option in our updated  `movies_app()` function.

```{r}
#| eval: false 
#| code-fold: false
movies_app <- function(test = TRUE) {
  app <- shiny::shinyApp(
                  ui = movies_ui, 
                  server = movies_server)
   if (test)
      shiny::runApp(appDir = app, test.mode = TRUE)
   else
      shiny::runApp(appDir = app, test.mode = FALSE)
}
```
...
![`if (interactive())` in `flowLayout()` example](img/05_app_interactive.png){#fig-05_app_interactive width=100%}
...
![Running `movies_app()` in **Console**](img/05_app_movies_app_console.png){#fig-05_app_movies_app_console width=100%}
...
![Sending `app.R` to **Console**](img/05_app_run_app_dot_r.png){#fig-05_app_run_app_dot_r width=100%}
