...
2. [`02_movies-app`](https://github.com/mjfrigaard/projApp/tree/02_movies-app) the boilerplate app in `app.R` is replaced with the code for the movie review application, the `scatter_plot()` utility function is added to `utils.R`, and `movies.RData` is added to the project.
...
1. [`03_projApp`](https://github.com/mjfrigaard/projApp/tree/03_projApp) is ‘advanced’ package from the previous chapter   
...
The next three sections will continue with app-package development, but with an emphasis on the particulars that come with developing a shiny application in a package structure. We'll be using the code stored in the [`pkgApp` repository](https://github.com/mjfrigaard/pkgApp). 

::: {.column-margin}

![New Git Branch](img/new_branch_ico.png){width='70%'}

The code for the next section is in the  [[`main`](https://github.com/mjfrigaard/pkgApp/tree/main)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.
