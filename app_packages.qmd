...
The [Shiny](shiny.qmd) Chapter covers Shiny projects (@sec-shiny-projects) and some 'pre-package practices' (@sec-adv-shiny-projects) to adopt in your Shiny app that can make the transition to working with app-packages easier. 
...
[Packages](packages.qmd) illustrates what separates projects from R packages. This chapter also covers 1) how to create a new Shiny app-package (@sec-create-new-app-packages) or 2) convert an existing Shiny project into an app-package (@sec-convert-shiny-app-packages). 
...
@sec-dev introduced the `devtools` package and its core functions (`load_all()` (@sec-dev-load), `document()` (@sec-dev-document), and `install()` (@sec-dev-install)).
...
A preview of each topic covered in the upcoming chapters is provided below.  
