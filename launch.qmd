...
![**Run App** icon in `app.R` of `main` branch](images/launch_run_app_from_imports.png){width=100%}
...
display_type <- function(run = "w") {
  switch(run, # <1>
    ...
    NULL = options(shiny.launch.browser = NULL)) # <1>
  
    shinyViewerType <- getOption('shiny.launch.browser') |> # <2>
                        attributes() |> 
                        unlist() |> 
                        unname() # <2>
    cli::cli_alert_info("shinyViewerType set to {shinyViewerType}") # <3>
}
```
1. Set option   
2. Get option  
...
![The updated `movies_app()` function launches the application in the window (without the `loadSupport()` warning)](images/launch_movies_app_updates.png){#fig-launch_movies_app_fun width=100%}
...
![`test` and `run` arguments in `movies_app()`](images/launch_movies_app_fun.png){width=100%}
...
`pkgload::load_all()` is the function that's actually called when we run `devtools::load_all()` (or use [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd>]{style="font-weight: bold; font-size: 0.75em"}), and this is somewhat analogous to running `library(moviesApp)`
...
![Send app.R to **Console** and click **Publish**](images/08_launch_deploy_app_dot_r.png){#fig-08_launch_deploy_app_dot_r width=100%}
...
On [shinyapps.io](https://www.shinyapps.io/), the deployment log shows the `loadSupport()` warning persists (but the application still [deploys](https://paradigmdata.shinyapps.io/moviesApp/)). The server-side warning is a known issue.[^launch-loadsupport-warning]
...
man/mod_var_input_ui.Rd, man/movies_app.Rd,
