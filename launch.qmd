...
In the previous chapter we removed the calls to `library()` in `app.R`, which is a critical step in managing dependencies. However, we will still want a way to lunch our app during development (and a process for deploying the application into a production environment).
...
co_box(
  ...
  header = "Terminology",
  contents = "
I'll be using the term 'launch' to differentiate successfully deploying an application locally (either in the IDE window, viewer pane, or browser) from clicking on the **Run App** icon or calling `runApp()`.",
  fold = FALSE
)
```

## [`app.R`]{style="font-size: 0.95em"} ([`main`]{style="font-size: 0.95em"} branch)

We're going to revert back to the `main` branch of `shinyAppPkg` to get a better understanding of the evolution of our `app.R` file. When we initially used `app.R` to run `movies_app(),` it contained the `library()` calls for the necessary packages, the code for `movies_app()`, *and* a call to the function:
...
# packages ------------------------------------
library(shiny)                        # <1>
library(shinythemes)                  # <1>
library(stringr)                      # <1>
library(ggplot2)                      # <1>
library(rlang)                        # <1>

movies_app <- function() {                   # <2>
  shiny::shinyApp(
    ui = shiny::fluidPage(
...
```
1. Install packages 
2. Build standalone app function  
...
::: {#fig-06_app_main_runApp_console}

![Calling `runApp()` in `main`](img/06_app_main_runApp_console.png){#fig-06_app_main_runApp_console width=100%}

The application is launched (with the `loadSupport()` warning)
:::

## [`app.R`]{style="font-size: 0.95em"} ([`02_roxygen2`]{style="font-size: 0.95em"} branch)

Let's jump ahead to the `02_roxygen2` branch of `shinyAppPkg`. In this version of `shinyAppPkg`, the `movies_app()` function has been placed in the `R/` folder (in `R/movies_app.R`).
...
:::{#fig-06_app_roxygen_no_runApp_icon}

![`app.R` in `02_roxygen2` branch](img/06_app_roxygen_no_runApp_icon.png){#fig-06_app_roxygen_no_runApp_icon width=100%}

No **Run App** icon in `app.R` of `02_roxygen2` branch
:::
...
![`runApp()` with `02_roxygen2` branch](img/06_app_roxygen2_runApp_console.png){#fig-06_app_roxygen2_runApp_console width=100%}

Calling `runApp()` in the **Console** of `02_roxygen2` branch
:::

This behavior might be surprising to you--`runApp()` can run the application in `shinyAppPkg`, even without the standalone app function exported from the package namespace (but it still produces the `loadSupport()` warning).
...
The examples above demonstrate the differences between calling `shinyApp()` and `runApp()`, and the tensions between having a standalone app function in an app-package vs. having an `app.R` file in a Shiny project.

In the following section, we'll compare `shinyApp()`, `shinyAppDir()`, and `runApp()` to determine which one to use in `R/movies_app.R` and the `app.R` file.   
...
#| eval: false 
#| code-fold: false
movies_app <- function(test = FALSE) {
    if (test) {
      shinyApp(                           # <1>
        ui = movies_ui,                   # <2>
...
2. Include option for `test.mode`    
3. Build the shiny app object with `movies_ui` and `movies_server`   
...
In `movies_app()`, I'll add the `display_type()` to set the `shinyViewerType` along with the `test.mode` option:
...
[<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>D</kbd>]{style="font-weight: bold; font-size: 1.25em"}

[<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>B</kbd>]{style="font-weight: bold; font-size: 1.25em"}

:::{#fig-06_launch_movies_app_updated}

![updated `movies_app()` function](img/06_launch_movies_app_updated.png){#fig-06_launch_movies_app_updated width=100%}
...
:::{#fig-06_launch_movies_app_updated_options}

![`test` and `run` arguments in `movies_app()`](img/06_launch_movies_app_updated_options.png){#fig-06_launch_movies_app_updated_options width=100%}
...
Below are a few options to consider including in your `app.R` file. There are multiple ways to launch an application from this file, so not all of these are required.   
...
:::{#fig-06_launch_run_app_dot_r}

![Sending `app.R` to **Console**](img/06_launch_run_app_dot_r.png){#fig-06_launch_run_app_dot_r width=100%}
...
:::{#fig-06_launch_deploy_app_dot_r}

![Click on the **Publish** icon](img/06_launch_deploy_app_dot_r.png){#fig-06_launch_deploy_app_dot_r width=100%}
...
:::{#fig-06_launch_publish_app}

![Publishing apps to `shinyapps.io`](img/06_launch_publish_app.png){#fig-06_launch_publish_app width=100%}
...
You can view a deployed version of this application [here.](https://mjfrigaard.shinyapps.io/shinyap-05_run-app/)
