...
#| eval: true
co_box(
  color = "b", look = "minimal", size = '1.05', hsize = '1.15',
...
:::{#fig-launch_run_app_from_imports}

![`app.R` in `main` branch](images/launch_run_app_from_imports.png){#fig-launch_run_app_from_imports width=100%}
...
::: {#fig-launch_loadsupport_error}

![Calling `runApp()` in `main`](images/launch_loadsupport_error.png){#fig-launch_loadsupport_error width=100%}

The application is launched (with the `loadSupport()` warning)
:::

Let's jump back to the `05_roxygen2` branch of `moviesApp`. This is the first version of `moviesApp` with `movies_app()` in the `R/` folder (i.e., in `R/movies_app.R`).

```{r}
#| label: git_margin_box_07_roxygen2
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
:::{#fig-launch_no_run_app_button}

![`app.R` in `05_roxygen2` branch](images/launch_no_run_app_button.png){#fig-launch_no_run_app_button width=100%}
...
:::{#fig-launch_loadsupport_error_run_app}

![`runApp()` with `05_roxygen2` branch](images/launch_loadsupport_error_run_app.png){#fig-launch_loadsupport_error_run_app width=100%}

Calling `runApp()` in the **Console** of `05_roxygen2` branch
:::


### Calling [`runApp()`]{style="font-size: 0.95em"} {.unnumbered}
...
#### [`loadSupport()`]{style="font-size: 0.95em"}--feature or bug? {.unnumbered}
...
The warning in the **Console** is telling us `loadSupport()`[^load-support-shiny-chap] has detected that `moviesApp` is a package, but it's still sourcing the `R/` subdirectory.
...
### Shiny launch functions {.unnumbered}
...
List of 5
 ...
 $ appOptions      :List of 2
  ..$ appDir       : chr "/path/to/moviesApp" # <1>
  ..$ bookmarkStore: NULL
...
##### [In `moviesApp`]{style="font-size: 0.85em"} {.unnumbered}
...
#### [`options`]{style="font-size: 0.95em"} {.unnumbered}
...
#### [`shinyViewerType`]{style="font-size: 0.95em"} {.unnumbered}
...
### Standalone options {#sec-launch-standalone-app-function}
...
:::{#fig-launch_movies_app_fun}

![updated `movies_app()` function](images/launch_movies_app_fun.png){#fig-launch_movies_app_fun width=100%}

The updated `movies_app()` function launches the application in the window (without the `loadSupport()` warning)
:::
...
:::{#fig-launch_movies_app_updates}

![`test` and `run` arguments in `movies_app()`](images/launch_movies_app_updates.png){#fig-launch_movies_app_updates width=100%}

Launching `movies_app()` in Viewer **p**ane and `test.mode`
:::



### [`app.R`]{style="font-size: 0.95em"} options {.unnumbered}
...
#### Non-interactive behaviors {.unnumbered}
...
##### [`stderr()`]{style="font-size: 0.95em"} {.unnumbered}
...
##### [`tryCatch()`]{style="font-size: 0.95em"} {.unnumbered}
...
In this case, if `library(moviesApp)` throws an error, the function specified after `error =` is executed (i.e., `pkgload::load_all()`).[^launch-pkgload-load_all]
...
#### [`pkgload`]{style="font-size: 0.95em"} {.unnumbered}
...
[^launch-calling-load_all]: `pkgload::load_all()` is the function that's actually called when we run `devtools::load_all()` (or use [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd>]{style="font-weight: bold; font-size: 0.75em"}), and this is somewhat analogous to running `library(moviesApp)`

#### [`withr`]{style="font-size: 0.95em"} {.unnumbered}
...
### Running [`app.R`]{style="font-size: 1.10em"} {.unnumbered}
...
:::{#fig-launch_app_dot_r}

![Sending `app.R` to **Console**](images/launch_app_dot_r.png){#fig-launch_app_dot_r width=100%}
...
:::{#fig-launch_publilsh_window}

![Publishing apps to `shinyapps.io`](images/launch_publilsh_window.png){#fig-launch_publilsh_window width=100%}
...
On [shinyapps.io](https://www.shinyapps.io/), the deployment log shows the `loadSupport()` warning persists (but the application still [deploys](https://mjfrigaard.shinyapps.io/moviesApp-launch/)). The server-side warning is a known issue.[^launch-loadsupport-warning]
