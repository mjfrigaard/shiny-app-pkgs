...
In the current branch of `shinyAppPkgs`, the `app.R` file loads our package and runs the standalone app function:
...
Let's jump further back to the `05_roxygen2` branch of `shinyAppPkgs`. This is the first version of the app-package with `launch_app()` in the `R/` folder (see below).
...
This behavior might be surprising to you--`runApp()` can launch the application in `shinyAppPkgs`, even without the standalone app function exported from the package namespace (but it still produces the `loadSupport()` warning).
...
``` sh
── Preparing for deployment ────────────────────────────────────────────────────
✔ Deploying "shinyAppPkgs" using "server: shinyapps.io / username: paradigmdata"
ℹ Creating application on server...
...
── Deployment complete ─────────────────────────────────────────────────────────
✔ Successfully deployed to <https://paradigmdata.shinyapps.io/shinyAppPkgs/>
Deployment completed: https://paradigmdata.shinyapps.io/shinyAppPkgs/
```
