...
### [`loadSupport()`]{style="font-size: 0.95em"} warning
...
If you recall, we covered [`loadSupport()`](https://shiny.posit.co/r/reference/shiny/1.5.0/loadsupport) back in the shiny chapter.[^load-support-1] The warning above also tells us that despite `shinyAppPkg` being structured as an app-package, the `R/` folding is still being sourced like it's a shiny project.

[^load-support-1]: `loadSupport()` is how the `R/` and `www/` folders are automatically sourced/served when we launched our app. 

## [`app.R`]{style="font-size: 1.05em"} ([`02_roxygen2`]{style="font-size: 1.05em"})

Let's jump ahead to the `02_roxygen2` branch of `shinyAppPkg`, which was the first version of `movies_app()` that we contained in the `R/` folder (in `R/movies_app.R`).
...
-   We're seeing the `loadSupport()` warning because one of the features of `shiny` projects is that all `.R` files in the `R/` folder  are `source()`ed when app launches.[^load-support-2]

[^load-support-2]: We covered the `shiny::loadSupport()` function in [Chapter 1](https://mjfrigaard.github.io/shinyap/shiny.html#r). 

After converting the contents of `shinyAppPkg` into an app-package, the contents of `R/` are loaded with `devtools::load_all()`. In the following section, we'll compare `shinyApp()` and `runApp()` to determine which one to use in `R/movies_app.R` and the `app.R` file.   

## [`shinyApp()`]{style="font-size: 1.05em"} vs. [`runApp()`]{style="font-size: 1.05em"}

`shinyApp()` is typically used to create apps within an interactive R session (like the R console or in Posit Workbench). `runApp()` is designed to run standalone apps located in a separate directory containing `ui.R` and `server.R` (or `app.R`) files. [^shiny-app-1]

[^shiny-app-1]: It's worthwhile to read the documentation on  [`shinyApp()`](https://shiny.posit.co/r/reference/shiny/0.14/shinyapp) and [`runApp()`](https://shiny.posit.co/r/reference/shiny/1.7.4/runapp).
...
List of 5
 ...
 $ appOptions      :List of 2
  ..$ appDir       : chr "/path/to/app/project/shinyAppPkg"
  ..$ bookmarkStore: NULL
...
We want to have control over where the shiny application will be launched from (i.e., the **Viewer** pane, the IDE **Window**, or an external browser). We can access this with the `shiny.launch.browser` option:[^shiny-launch-browser-2]

[^shiny-launch-browser-2]: The `shiny.launch.browser` options are covered in [this blog post by Garrick Aden-Buie](https://www.garrickadenbuie.com/blog/shiny-tip-option-where-to-run/), and I've combined them into a `run` argument in `movies_app()`
...
In this case, if `library(shinyAppPkg)` throws an error, the function specified after `error =` is executed (i.e., `pkgload::load_all()`).
...
If the session ***is*** interactive (i.e., `!interactive() == FALSE`), we want `app.R` to load all the code in the `R/` folder with `pkgload::load_all()`.[^pkgload-load_all]

[^pkgload-load_all]: `pkgload` is part of the [conscious uncoupling](https://www.tidyverse.org/blog/2018/10/devtools-2-0-0/) of the `devtools` package we learned about back in [Chapter 3](https://mjfrigaard.github.io/shinyap/dependencies.html).
...
Recall that `pkgload::load_all()` is function actually called when we run `devtools::load_all()` (or use [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd>]{style="font-weight: bold; font-size: 0.75em"}), and this is analogous to running `library(shinyAppPkg)`

## Calling [`movies_app()`]{style="font-size: 1.10em"}

After loading, documenting, and installing `shinyAppPkg`, we'll run `movies_app()` in the **Console** to see if we're still getting the `loadSupport()` warning: 
...
# movies_app ------------------------------------
shinyAppPkg::movies_app(test = FALSE)
```

When we send the contents of `app.R` to the **Console**, the control flow safely loads `shinyAppPkg` before running the app:
...
`shinyAppPkg` is loaded before running `movies_app()`
:::
