...
`launch_app()` is a wrapper function for `shinyApp()`, where it passes `movies_ui()` and `movies_server()` to the `ui` and `server` arguments.
...
Let's jump further back to the `05_roxygen2` branch of `moviesApp`. This is the first version of the app-package with `launch_app()` in the `R/` folder (see below).
...
1 directory, 6 files
```
1. `launch_app()` standalone function in `R/` folder
...
In the `05_roxygen2` branch, the `launch_app()` function hasn't been exported, so we should expect the following error when we run the contents of `app.R`:
