...
- **`runApp()`**: versatile function that can launch apps in multiple formats/locations (blocking)\n
    - A directory with a `server.R` and `ui.R`, or a single `app.R` file\n
    - An app object created by `shinyApp()`\n

- **`startApp()`**: launches app in background process, returns console control immediately (non-blocking)\n
    - Best for interactive development and rapid iteration\n
    - Requires specifying a port\n

- **`shinyAppDir()`**: launches a Shiny app programmatically from a directory.\n
"
)
```
...
co_box(
  ...
  fold = TRUE,
  contents = "At the time of this writing, the Positron version 1.130.0 was available."
)
```
...
RStudio and Positron display **Run App** and **Run Shiny App** buttons respectively in the **Source** pane. Clicking either button calls `runApp()` and launches the application, but with a `loadSupport()` warning. 
...
Clicking **Run Shiny App** in Positron launches the application in [`devmode`](https://shiny.posit.co/r/reference/shiny/latest/devmode.html), which controls how Shiny behaves during development. See [Positron's runShinyApp.R](https://github.com/posit-dev/positron) for the devmode launcher implementation.
...
### [`startApp()`]{style="font-size: 0.95em; font-weight: bold;"} {#sec-launch-start-app}

Shiny recently introduced `startApp()`, which changes how we think about launching apps interactively. While `runApp()` blocks the R session (you cannot type in the console while the app is running), `startApp()` launches the app in the background and returns control to the console immediately.

I've found this matters most when you're exploring your app interactively. Instead of restarting R each time you want to test a change, you can modify your code and re-run it while the app is still running in another process. The app stays live and you keep your REPL.

Below are some practical examples for using `startApp()`:

:::{layout="[45,55]" layout-valign="center"}

Launch and stop a shiny `app` object:
...
After the app launches, you keep your console and can continue working: run tests, modify code, re-source functions, or launch additional apps on different ports. Call `handle$stop()` to stop the background app when you're done.

The key difference is simple: `runApp()` is blocking and single-process. `startApp()` is non-blocking and spawns a background process. When to use `startApp()` vs. `runApp()`: I recommend `startApp()` for interactive development (you need console access), and `runApp()` for traditional deployment (the blocking behavior is fine). In production or when you're ready to fully focus on the running app, `runApp()` is still the right choice. But for rapid iteration in development, `startApp()` unlocks a faster workflow.

See the [Shiny documentation](https://shiny.posit.co/r/reference/shiny/latest/startapp.html) for full details on `startApp()` arguments and capabilities.

:::: {.callout-note collapse="true" appearance="default" title="startApp()"}

::: {style='font-size: 1.05em; color: #696969;'}

:::{layout="[45,55]" layout-valign="center"}

[`startApp()`]{style="font-size: 0.95em"}: Launches your app in a background process and returns console control immediately (non-blocking).
...
4. `startApp()` launches an app in a background process and returns console control immediately, making it ideal for interactive development and rapid iteration.

[^shiny-app-launchers]: It's worthwhile to read the documentation on  [`shinyApp()`, `shinyAppDir()`](https://shiny.posit.co/r/reference/shiny/0.14/shinyapp), [`runApp()`](https://shiny.posit.co/r/reference/shiny/1.7.4/runapp), and [`startApp()`](https://shiny.posit.co/r/reference/shiny/latest/startapp.html).
...
With the introduction of `startApp()`, we can add a `mode` argument to choose between blocking (`"run"`) and non-blocking (`"start"`) execution:
...
```
1. Create the shiny app object
2. Non-blocking mode—launches in background process
3. Blocking mode (default)—uses display_type() for window placement

Now you can call `launch_app(mode = "start")` for rapid iteration or `launch_app(mode = "run")` for traditional blocking execution.
...
The `display_type()`, `options` list, and `mode` argument have been added to `launch_app()`:
...
```
1. Create the shiny app object
2. Non-blocking mode—launches app in background, returns console control
3. Blocking mode (default)—uses display_type() for window placement, blocks console
...
withr::with_options(new = list(shiny.autoload.r = FALSE), code = { # <1> 
  if (!interactive()) { # <2>
    ...
    sap::launch_app( # <4>
      options = list(test.mode = TRUE), mode = "run", run = 'p') # <4>
}) 
...
-   `runApp()` is a more generalized way to launch an app. It works with:
...
-   Clicking on **Run App** will run the application with `runApp()` if it detects a file (or files) that creates a shiny object.

-   Blocks the console during execution (you cannot type while the app runs).

**`startApp()`** (non-blocking):

-   `startApp()` launches your app in a background process and returns console control immediately.

-   Best for interactive development and rapid iteration—modify code and re-run without restarting R.

-   Requires specifying a port for the background app process.

-   Not recommended for production deployments where `runApp()` is the standard approach.
  "
)
```
