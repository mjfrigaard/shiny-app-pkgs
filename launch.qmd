...
In the previous chapter, we moved `movies` into `data/` folder to ensure it's part of the `moviesApp` namespace. However, we will still want a way to lunch our app during development (and a process for deploying the application into a production environment).
