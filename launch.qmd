...
If you recall, we covered [`loadSupport()`](https://shiny.posit.co/r/reference/shiny/1.5.0/loadsupport) back in the shiny chapter.[^load-support-1] The warning above also tells us that despite `pkgApp` being structured as an app-package, the `R/` folding is still being sourced like it's a shiny project.
...
Recall that `pkgload::load_all()` is function actually called when we run `devtools::load_all()` (or use [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd>]{style="font-weight: bold; font-size: 0.75em"}), and this is analogous to running `library(pkgApp)`
