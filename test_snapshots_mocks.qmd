...
I've included additional snapshot tests (`test-text_logo.R`) in the [`A.E-mocks-snapshots` branch of `shinyAppPkgs`](https://github.com/mjfrigaard/shinyAppPkgs/tree/A.E-mocks-snapshots): 
...
The version of `check_installed()` in `shinyAppPkgs` will check if a package is installed and return `invisible()` if it is (which, when assigned to an object, evaluates to `NULL`):
...
The `roxygen2` documentation for `check_installed()` uses the `@importFrom` tag to import `is_installed` and add it to the `shinyAppPkgs` namespace (using explicit namespacing alone won't work):
