...
This chapter will cover the links between user specifications, functional requirements, and show how to use a traceability matrix to track your tests. I’ll also touch on the three layers of testing: unit tests, integration/module tests, and end-to-end or system tests. I'll focus on *what to test* and *why to test it*, not *how to write tests*, because plenty of those resources exist. [^tests-into-1] [^tests-into-2] [^tests-into-3] The only exception being some tricks I've learned for using `testServer()` with module server functions.[^tests-into-4]

 
[^tests-into-1]: Unit tests are covered extensively in [R Packages, 2ed](https://r-pkgs.org/testing-basics.html) and the [`testthat` documentation](https://testthat.r-lib.org/index.html)

[^tests-into-2]: `shinytest2` has [excellent documentation](https://rstudio.github.io/shinytest2/) (and [videos](https://www.youtube.com/watch?v=Gucwz865aqQ)), and I highly recommend reading through those resources.

[^tests-into-3]: Mastering shiny has a [Chapter on Testing](https://mastering-shiny.org/scaling-testing.html), which covers [unit tests](https://mastering-shiny.org/scaling-testing.html#basic-structure) and [`testServer()`](https://mastering-shiny.org/scaling-testing.html#testing-reactivity), and also includes some tips for using JavaScript with [`shinytest`](https://mastering-shiny.org/scaling-testing.html#testing-javascript) (not to be confused with [`shinytest2`](https://rstudio.github.io/shinytest2/)) 

[^tests-into-4]: The [`testServer()`](https://shiny.posit.co/r/articles/improve/server-function-testing/) documentation is sparse, so I'll provide a few tips and tricks I've learned for testing module server functions.
...
When you being writing tests for your Shiny app-package, you'll be faced with the two questions: 

1. What do I test?  
2. How do I test it?  

There are typically supporting documents and resources to help you address these questions, one of them is the software requirements specification (SRS). The SRS typically breaks down an application's intended purpose (or the problem it's designed to solve) into three general areas: user specifications, feature requirements, and functional requirements. These are the guidelines that direct the development process, albeit from slightly different perspectives. 
...
Understanding the relationship between the specifications, features, and functional requirements is important for knowing if the application we're developing meets the technical standards and satisfies the user's needs.

Below we'll cover a brief example of each.

### User Requirements

**User Requirements** are what the end-user (i.e., a film analyst) wants to achieve with the application.[^tests-specs-1]

[^tests-specs-1]: User Requirements are sometimes referred to as "user stories," "use cases," or "general requirements"
...
co_box(
  ...
  size = '1.10', header = "Scatter plot user requirements",
  contents = 
" 
...
The **feature requirement** translates the end-user's expectation into a high-level description of an application capability (i.e, display a scatter plot), phrased at satisfying a specific end-user need outlined in the **user requirements**.[^tests-specs-2]

[^tests-specs-2]: "Feature requirements" and "functional requirements" are sometimes used interchangeably, but they refer to different aspects of the software. **Feature requirements** are the desired high-level characteristics the application *should* have, and often capture a collection of smaller functionalities (which are broken down into specific functional requirements).
...
**FE1**: The Shiny app should offer an interactive scatter plot feature that allows users to visualize and compare movie reviews from at least two data sources (IMDb, Rotten Tomatoes), with options for selecting variables of interest (i.e., audience scores, website ratings, runtime, etc.) and the ability to zoom into specific data points for detailed analysis.
...
The **Functional Requirements** are written for the developer and provide technical details on *how* the feature (i.e., the scatter plot) should behave and *what* it needs to do. These requirements are written for developers and represent where the end-user's expectations come into direct contact with code.[^tests-specs-3]

[^tests-specs-3]: **Functional requirements** are precise, measurable, and testable. 
...
#| eval: true
co_box(
  color = "r", fold = FALSE, look = "minimal",
...
" 
  ...
  - **FR3**: Each data point on the scatter plot will represent a movie and be color-coded based on MPAA ratings, genre, title type, critics rating, and audience rating.
  
  - **FR4**: The scatter plot will have axes labeled with rating scales and must include a legend to differentiate between data sources.

  - **FR5**: The scatter plot will have interactive capabilities to provide end-users the ability to zoom into specific areas.
...
In our app-package, we want to ensure the functionalities (i.e., the code) and tests address all the user specifications, *and* that every functionality corresponds to an explicit user need (and that it's been tested). A traceability matrix is a table that links user specifications, feature requirements, and the functional requirements they give rise to with the tests used to verify that the application has been developed correctly.

Fortunately, the R package structure gives us some tools to capture this documentation in the form of vignettes. 

#### [`vignettes/`]{style="font-size: 1.05em;"}

Package vignettes are a great place store the a draft of the traceability matrix, and they can be quickly created using `usethis::use_vignette()`:
...
- Adds the `knitr` and `rmarkdown` packages to the `Suggests` field in `DESCRIPTION`[^test-specs-4]
...
- Adds `knitr` to the `VignetteBuilder` field[^test-specs-5]

    ```bash
    VignetteBuilder: knitr
    ```

- Adds `inst/doc` to `.gitignore` and `*.html`, `*.R` to `vignettes/.gitignore`[^test-specs-6]


[^test-specs-4]: We briefly covered the `Suggests` field in [Dependencies](https://mjfrigaard.github.io/shinyap/dependencies.html), but in this case it specifically applies to "*packages that are not necessarily needed. This includes packages used only in examples, tests or vignettes...*" - [Writing R Extensions, Package Dependencies](https://cran.r-project.org/doc/manuals/R-exts.html#Package-Dependencies)

[^test-specs-5]: The [documentation](https://cran.r-project.org/doc/manuals/R-exts.html#The-DESCRIPTION-file) on `VignetteBuilder` is an excellent example of how challenging if can be to trace dependencies across packages, "*Note that if, for example, a vignette has engine `knitr::rmarkdown`, then `knitr` provides the engine but both `knitr` and `rmarkdown` are needed for using it, so both these packages need to be in the `VignetteBuilder` field and at least suggested (as `rmarkdown` is only suggested by `knitr`, and hence not available automatically along with it).*"

[^test-specs-6]: We covered the `inst/` folder in the [External Files chapter](https://mjfrigaard.github.io/shinyap/), and you might recall that `docs/` was one of the folders we shouldn't create (now you know why!).

When building the traceability matrix, I highly suggest using the [Visual Markdown mode]() (especially when building tables). 

We'll start with the user specifications. The language used for these descriptions are non-technical, but should provide a basis for deriving the more technical (but still high-level) feature requirements:
...
A single functional requirement often gives rise to multiple functional requirements:
...
Now that the technical details of the user-needs have been documented, we can identify what needs to be tested: 
...
1. Each user specification has accompanying feature requirements.

2. The feature has been broken down into precise, measurable, and testable functional requirements.

3. Tests have been written for each functional requirement 

If you'd like, an additional column can be added to track whether the test has passed:
...
trace_matrix |> 
  ...
  tibble::add_column(`Test Status` = 
      paste0("T", 1:nrow(trace_matrix), "-status: "), .after = 4) |> 
  gt::gt() |> 
  gt::sub_missing(
  columns = gt::everything(),
  rows = gt::everything(),
...
Vignettes are a great place to store this information because it's self-contained and travels with the package whenever the code is updated.[^test-specs-7]
...
The next section covers setting up tests with `testthat`, the structure of unit tests, and additional elements you should consider using in your app-package: 

## The test suite

Multiple strategies exist for writing tests. For example, if you've adopted test-driven development (TDD)[^tests-testthat-1], you'll develop tests before writing utility functions, modules, or your standalone app function. However, if you're a mere mortal like the rest of us, you'll typically develop your tests and functions in tandem. 

[^tests-testthat-1]: "*The tests should be written before the functionality that is to be tested. This has been claimed to have many benefits. It helps ensure that the application is written for testability, as the developers must consider how to test the application from the outset rather than adding it later."* - [TDD, Wikipedia](https://en.wikipedia.org/wiki/Test-driven_development) 

Regardless of the testing strategy, we'll set up the testing infrastructure in our app-package with the [`testthat` package](https://testthat.r-lib.org/):
...
- [x]   Finally, we're given some advice on the next step for creating our first test: 
...
If I'm writing a unit test for the `scatter_plot()` function in `R/scatter_plot.R`, I'll create test file with `usethis::use_test("scatter_plot")`.[^test-files-folders]
...
The IDE will automatically create and open the new test file: 
...
The new test file contains a boilerplate test. Each `testthat` test has a test context (supplied to the `desc` argument) followed by the test `code` argument (supplied in curly brackets). 
...
When a test is run, you'll see feedback on whether it passes or fails:
...
Comparisons are the backbone of testing. I've found exploring how the test performs these comparisons (i.e., the underlying package(s)) can save me from surprising results. 
...
test_cmt <- function(start = NULL, end = NULL, msg) {
  withr::local_options(digits.secs = 4)
  if (is.null(start) & is.null(end)) {
    cat("\n")
    cli::cli_alert_info("\n[{Sys.time()}| {msg}]")
  } else if (!is.null(start) & is.null(end)) {
    cat("\n")
    cli::cli_alert_info("\n[ START | {Sys.time()} | {start} = {msg}]")
  } else if (is.null(start) & !is.null(end)) {
    cat("\n")
    cli::cli_alert_info("\n[ END   | {Sys.time()} | {end} = {msg}]")
  } else {
    cat("\n")
    cli::cli_alert_info("\n[ START | {Sys.time()} | {start} = {msg}]")
    cat("\n")
...
`test_cmt()` can be used to log when your test beings and ends, and includes a message for the context.[^tests-6]

[^tests-6]: If you like verbose logging outputs, check out the [`logger` package](https://daroczig.github.io/logger/) 
...
If the expected output we're interesting in testing is cumbersome to describe programmatically, we can consider using a snapshot tests. Examples of this include UI elements (which are mostly HTML created by Shiny's UI layout and input/output functions) and data visualizations.[^tests-7] 

[^tests-7]: Mastering Shiny covers [creatng a snapshot file](https://mastering-shiny.org/scaling-testing.html#user-interface-functions) to test UI elements, but notes this is probably not the best approach.
...
`expect_doppelganger()` can be dropped in `test_that()` like any other expectation. Below is an example comparing the graph outputs from `ggplot2` and the custom `scatter_plot()` utility function in `shinyAppPkg`. 

Notice I've loaded the `ggp2_movies.rds` data[^tests-8] and included the `test_cmt()` helper with `start` and `end` messages (before and after the test code).

[^tests-8]: Accessing test files is made easier with `testtthat::test_path()`
...
The fourth `devtools` [habit]{style="font-weight: bold; font-size: 1.0em;"} to adopt is regularly writing and running tests. If you're using Posit Workbench and have `devtools` installed, you can test your app-package using the **Build** pane or the keyboard shortcut:
...
Subsequent tests in `shinyAppPkg` will pass without a warning:
...
Another option for using snapshots for testing is the `expect_snapshot_file()` function [^tests-9] but `expect_doppelganger()` is probably the better option for comparing graph outputs.

[^tests-9]: Follow the `expect_snapshot_file()` example from the [`testthat` documentation](https://testthat.r-lib.org/reference/expect_snapshot_file.html#ref-examples)
...
To check the initial `NULL` value of an `inputId`, it's better to build the shiny app object, then pass this to `testServer()`.[^tests-10]

[^tests-10]: The `shiny::is.shiny.appobj()` will test if an object is a 'shiny app object.'
...
Now that we've confirmed the returned values from `mod_var_input_server()` are in a list, we want to make sure it's read correctly by the `var_inputs` argument in `mod_scatter_display_server()`. I've included the `movies_server()` function below refresh our memory of how this *should* work:[^tests-11]
...
[^tests-11]: `selected_vars` are the reactive plot values returned from `mod_var_input_server()` we confirmed `test-mod_var_input_server.R`.
...
If we pause execution with the debugger,[^tests-12] we can see the difference between calling `selected_vars` and `selected_vars()`:

[^tests-12]: We'll cover using `browser()` and the IDE's debugger in a future chapter.
...
#| eval: false 
#| include: false
testthat::describe(description = "mod_scatter_display_server", code = {
   
  shiny::testServer(app = mod_scatter_display_server, expr = {

        testthat::it(description = "is reactive", code = {


        })
    
        testthat::it(description = "is list", code = {
