...
These guidelines direct the development process, albeit from slightly different perspectives. Understanding the interplay between user specifications, features, and functional requirements is essential for developers to know how the technical standards satisfy the user's needs.
...
[^test-specs-inst-docs]: We covered the `inst/` folder in the [External files](external.qmd) chapter, and you might recall that `doc/` was one of the folders we shouldn't create inside `inst/`.
...
[^tests-specs-feat]: 'Feature requirements' and 'functional requirements' are sometimes used interchangeably, but they refer to different aspects of the application. **Feature requirements** are high-level characteristics an application *should* have, and often contain a collection of smaller functionalities (broken down into the specific **functional requirements**).
...
**Functional requirements** are written for the developer and provide technical details on *how* the feature (i.e., the scatter plot) should behave and *what* it needs to do (they're where the end-user's expectations come into direct contact with code).[^tests-specs-func]
...
I'll create helpers for components or values that aren't large enough to justify storing as static test fixtures.

### App input helpers 

For example, the `make_var_inputs()` function below creates inputs for the `scatter_plot()` utility function:
...
`glue::glue_collapse()` is your friend when you need to repeated use code in your tests.
...
In the sections below, I'll provide several examples using `testthat`'s BDD functions. The beauty of the BDD functions is their flexibility, and I'm sure there are multiple ways to achieve the same result I'm attempting to demonstrate here.

Hopefully, the following sections convey how versatile and expressive the BDD functions can be (or they inspire you to properly implement what I set out to do in your app-package).
...
R Packages, 2ed [suggests](https://r-pkgs.org/testing-basics.html#run-tests) binding `devtools::test_active_file()` to a keyboard shortcut. Using keyboard shortcuts while developing tests will improve your ability to iterate quickly, and I **highly** recommend finding a shortcut that works with your workflow.   


:::{.column-margin}

Binding [<kbd>Ctrl/Cmd</kbd> + <kbd>T</kbd>]{style="font-size: 0.95em;"} to [`test_active_file()`]{style="font-size: 0.95em;"} makes test development faster/easier!  
...
This runs slightly counter to the recommendations in [R Packages, 2ed](https://r-pkgs.org/code.html#sec-code-organising), but still adheres to the naming coventions described elsewhere[^tests-naming-files] and gives us the ability to programmatically compare test results with the traceability matrix.

[^tests-naming-files]: See Jenny Bryan's [slides/talk](https://github.com/jennybc/how-to-name-files) on naming files. 

For example, we can add the following code to the [`test-specs` vignette](https://github.com/mjfrigaard/shinyAppPkg/blob/10e_tests-system/vignettes/test-specs.Rmd) and capture our test results in a `tibble` display:
...
Placing the functional requirement as a prefix in the file name (**FR**) with a corresponding test number in each call to `testthat::it()` will make it easier to link the tests back to the original specification that generated them.
