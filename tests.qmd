...
" 
If you downloaded or cloned the [`monthApp`](https://github.com/hadley/monthApp) example from [Mastering Shiny](https://mastering-shiny.org/), then clicked on **Test** in the **Build** pane, you saw the following:

![Testing `monthApp` app-package](img/monthApp_test.png){width='100%' fig-align='center'}
  
However, the [Testing](https://mastering-shiny.org/scaling-testing.html) chapter covers setting up and running unit tests with [`testthat`](https://mastering-shiny.org/scaling-testing.html#testing-functions) and how to use Shiny's [`testServer()`](https://mastering-shiny.org/scaling-testing.html#testing-reactivity) function to test reactivity and module server functions.
  
In this Chapter, I'll expand on testing your shiny app-package and provide some testing strategies to consider adopting before moving your application into production.

"
  )
```

This chapter will cover three layers of tests: unit tests, integration/module tests, and end-to-end or system tests. I'll focus on *what to test* and *why to test it*, not *how to write tests*, because plenty of those resources exist[^tests-1] [^tests-2] (the only exception being some tricks I've learned for using `testServer()` with module server functions[^tests-3]).

I’ll also touch on the links between user specifications, functional requirements, and how to use a traceability matrix to track your tests.

[^tests-1]: Unit tests are covered extensively in [R Packages, 2ed](https://r-pkgs.org/testing-basics.html) and the [`testthat` documentation](https://testthat.r-lib.org/index.html)

[^tests-2]: `shinytest2` has [excellent documentation](https://rstudio.github.io/shinytest2/) (and [videos](https://www.youtube.com/watch?v=Gucwz865aqQ)), and I highly recommend reading through those resources.

[^tests-3]: The [`testServer()`](https://shiny.posit.co/r/articles/improve/server-function-testing/) documentation is sparse, so I'll provide a few tips and tricks I've learned for testing module server functions.
...
So far, the shiny app-package we've been using didn't explain it's intended purpose or the problem it's designed to solve. In regular application development, the user specifications, feature requirements and functional requirements are the guidelines that direct the development process, albeit from slightly different perspectives.
...
- Adds the `knitr` and `rmarkdown` packages to the `Suggests` field in `DESCRIPTION`

- Adds `knitr` to the `VignetteBuilder` field
...
```{r}
#| label: co_box_trace_matrix_add_test
#| echo: false
#| code-fold: false
trace_matrix |> 
...
trace_matrix |> 
  ...
  tibble::add_column(Status = 
      paste0("Status T", 1:nrow(trace_matrix), ": "), .after = 4) |> 
  gt::gt() |> 
  gt::sub_missing(
  columns = gt::everything(),
  rows = gt::everything(),
...
Multiple strategies exist for testing code. For example, if you've adopted test-driven development (TDD)[^tests-4], you'll develop tests before writing utility functions, modules, or your standalone app function. However, if you're a mere mortal like the rest of us, you'll typically develop your tests and functions in tandem. 

[^tests-4]: "The tests should be written before the functionality that is to be tested. This has been claimed to have many benefits. It helps ensure that the application is written for testability, as the developers must consider how to test the application from the outset rather than adding it later." - [TDD, Wikipedia](https://en.wikipedia.org/wiki/Test-driven_development) 
...
trace_matrix |> 
  tibble::add_column(Test = 
      paste0("T", 1:nrow(trace_matrix), ": "), .after = 3) |> 
 tibble::add_column(Status = 
      paste0("Status T", 1:nrow(trace_matrix), ": "), .after = 3) |>
  gt::gt() |> 
  gt::sub_missing(
  columns = gt::everything(),
  rows = gt::everything(),
