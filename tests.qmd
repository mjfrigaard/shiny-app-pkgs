...
co_box(
  ...
  contents = " 
The [Testing chapter](https://mastering-shiny.org/scaling-testing.html) of Mastering Shiny covers setting up and running unit tests with [`testthat`](https://mastering-shiny.org/scaling-testing.html#testing-functions), and how to use Shiny's [`testServer()`](https://mastering-shiny.org/scaling-testing.html#testing-reactivity) function to test reactivity.
  
However, if you downloaded or cloned the [`monthApp`](https://github.com/hadley/monthApp) example from the [Package chapter](https://mastering-shiny.org/scaling-modules.html), then clicked on **Test** in the **Build** pane, you saw the following:

![Testing `monthApp` app-package](img/monthApp_test.png){width='100%' fig-align='center'}
  

This chapter will attempt to connect the concepts in the [Testing chapter](https://mastering-shiny.org/scaling-testing.html) to our shiny app-package, and provide some general strategies to consider adopting before moving your application into production.
...
**User Requirements** are what the end-user (i.e., a film analyst) wants to achieve with the dashboard.[^tests-specs-1]
...
Each **Feature Requirement** is a high-level description of the feature's capabilities (i.e, the scatter plot), phrased at satisfying the end-user's needs outlined in the **User Requirements**.[^tests-specs-2]

[^tests-specs-2]: "Feature requirements" and "functional requirements" are sometimes used interchangeably, but they refer to different aspects of the software. **Feature requirements** are the high-level characteristics the system *should have* and often capture a collection of smaller functionalities (which are broken down into specific functional requirements).
...
#| eval: true
co_box(
  color = "y", fold = TRUE, look = "minimal",
...
The **Functional Requirements** are written for the developer and provide technical details on *how the feature* (i.e., the scatter plot) *should behave* and *what it needs to do*.[^tests-specs-3]

[^tests-specs-3]: **Functional requirements** describe what a software system *should do* and are precise, measurable, and testable. 
...
#| eval: true
co_box(
  color = "o", fold = TRUE, look = "minimal",
...
[^test-specs-4]: We didn't cover the `Suggests` field in the [Dependencies](https://mjfrigaard.github.io/shinyap/dependencies.html) chapter, but "*lists packages that are not necessarily needed. This includes packages used only in examples, tests or vignettes ... and packages loaded in the body of functions.*" - [Writing R Extensions, Pakcage Dependencies](https://cran.r-project.org/doc/manuals/R-exts.html#Package-Dependencies)
...
[^test-specs-6]: We learned about the `inst/` folder in the [External Files chapter](https://mjfrigaard.github.io/shinyap/), so this should help explain where knitted vignettes end up when a package is installed. 
...
```{r}
#| label: co_box_trace_matrix_fun_reqs
#| echo: false
#| code-fold: false
#| include: true
...
1. Each user specification has an accompanying feature requirement.

2. The feature requirements have been broken down into precise, measurable, and testable pieces of code.

3. Tests have been written for each functional requirement (and indicate whether they are passing or not).
