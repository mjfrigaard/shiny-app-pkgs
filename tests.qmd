...
Multiple strategies exist for testing code. For example, if you've adopted test-driven development (TDD)[^test-driven-dev-1], you'll develop tests before writing utility functions, modules, or your standalone app function. However, if you're a mere mortal like the rest of us, you'll typically develop your tests and functions in tandem. 

[^test-driven-dev-1]: "The tests should be written before the functionality that is to be tested. This has been claimed to have many benefits. It helps ensure that the application is written for testability, as the developers must consider how to test the application from the outset rather than adding it later." - [TDD, Wikipedia](https://en.wikipedia.org/wiki/Test-driven_development) 
...
#### Snapshots with [`vdiffr`]{style="font-size: 1.05em;"}
...
Testing modules can be accomplished with `shiny`s `testServer()` function. Modules have both UI and server functions, so these could be considered 'integration' or 'interaction' tests (i.e., we're testing how a module's functions 'interact' with values, or communicate with other modules). 
