...
If we've captured the user's needs and expectations in our traceability matrix, we can apply behavior-driven development using `testthat`'s `describe()` and `it()` functions.
...
Behavior-driven development (or behavior-driven testing) fills the void between non-technical stakeholders and developers by encouraging natural, descriptive language (often complete sentences) to define and communicate the application requirements.[^tests-bdd-describe] 
...
Test fixtures are various resources used to ensure a consistent, well-defined test environment. Fixtures can be input data, database connections, R options, environment variables, or anything else needed to create repeatable test conditions (even when the test is run in different environments).

A good test fixture is also self-sufficient--it's used and then destroyed after the test has been run to make sure any changes made during the test don't persist or interfere with other tests.
...
It's gratifying to see 100% test coverage, but in this case it's misleading. We want to include a second data source (per the feature description). We can do this by adding a nested `describe()` function: The feature (**FE1**) scope adds more context to the functional requirement (**FR1**), which includes a second test (**T2**) for the tidy `ggplot2movies::movies` data source:[^tests-ggp2_movies-raw-data]
...
Helper functions are stored in `tests/testthat/helper.R`, which is automatically loaded with `devtools::load_all()`:
...
Test helpers are functions or code that make creating and running tests easier **when the tests fail.** The article, ['Why Good Developers Write Bad Unit Tests'](https://mtlynch.io/good-developers-bad-tests/), provides great advice on complexity vs. clarity when writing unit tests,
...
However, when we're writing tests, it's more important that tests are easy to read and understand **when they fail**. For example, consider the inputs passed to the `scatter_plot()` function in the previous test:
...
If I need objects or values that aren't large enough to justify storing as static test fixtures, I'll create a helper function that makes it easier to generate the duplicated code. For example, the `make_var_inputs()` function below creates inputs for the `scatter_plot()` utility function:
...
When I call `make_var_inputs()` in the **Console**, it returns the list of values I can paste into the test:
...
`glue::glue_collapse()` is your friend if you want to repeat code in your tests. `make_ggp2_inputs()` will create a list of inputs for testing `scatter_plot()` with the tidy `ggplot2movies::movies` data:
...
We'll use `test_logger()` enough to justify placing it in [`R/testthat.R`](https://github.com/mjfrigaard/shinyAppPkg/blob/10c_tests-helpers/R/testthat.R) (which also ensures it's documented and it's dependencies become part of `shinyAppPkg`).[^tests-r-files]
...
When we check the code coverage, we can see the test above comfirms the `var_inputs` argument transfers the reactive values in `mod_scatter_display_server()`, but doesn't execute the call to `scatter_plot()`:
...
We can test this portion of the function by using `setInputs()` with the graph values: 

```{r}
#| eval: false 
#| code-fold: false 
session$setInputs(x = "imdb_rating",               # <1>
                  y = "audience_score",
                  z = "mpaa_rating",
                  alpha = 0.5,
                  size = 2,
                  plot_title = "Enter Plot Title")    # <1>

plot <- scatter_plot(movies, # <2>
  x_var = inputs()$x,
  y_var = inputs()$y,
  col_var = inputs()$z,
...
ggplot2::theme_minimal() +
ggplot2::theme(legend.position = "bottom") # <2>

testthat::expect_true(ggplot2::is.ggplot(plot)) # <3>
```
1. Set module inputs  
2. Build graph (same code from module function)   
3. Confirm `ggplot2` object is built  


This will test the code to use build the graph in `mod_scatter_display_server()`:
