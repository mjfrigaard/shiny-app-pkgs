...
Behavior-driven development (or behavior-driven testing) fills the void between non-technical stakeholders and developers by encouraging natural, descriptive language (often full sentences) to define and communicate the application requirements. 

In the sections below, I'll provide several examples using `testthat`'s BDD functions. The beauty of the BDD functions is their flexibility, and I'm certain there are multiple ways to achieve the same result I'm attempting to demonstrate here. 

Hopefully each section provides 
...
System (or end-to-end) tests verify the whole application (or system) and are run to simulate real user interactions in a 'pre-production' environment.[^tests-prod] Approaches to system testing vary, but in general, you'll want to run system tests before a release, which means the app's functional requirements are tested and released in lockstep.

[^tests-prod]: System tests should strive to replicate the production environment *conditions*, even if it's not possible to perfectly replicate.
