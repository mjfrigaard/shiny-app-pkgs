...
Another option for using snapshots for testing is the `expect_snapshot_file()` function, [^expect-snapshot-file-2] but for graph outputs it's easier to use the `expect_doppelganger()` function from the [`vdiffr` package](https://vdiffr.r-lib.org/).

[^expect-snapshot-file-2]: Follow the `expect_snapshot_file()` example from the [`testthat` documentation](https://testthat.r-lib.org/reference/expect_snapshot_file.html#ref-examples)

#### [`vdiffr`]{style="font-size: 1.05em;"}

The `expect_doppelganger()` function from [`vdiffr`](https://vdiffr.r-lib.org/) has `title` and `fig` arguments, and is designed specifically to work with ['*graphical plots*'](https://vdiffr.r-lib.org/reference/expect_doppelganger.html)

```{r}
#| eval: false 
#| code-fold: false
test_that(desc = "scatter_plot() works", code = {
  test_cmt(start =  "scatter_plot()", msg = "ggplot2movies::movies snapshot")
  ggp2_movies <- readRDS(test_path("fixtures", "ggp2_movies.rds"))
    vdiffr::expect_doppelganger(
      ...
      fig = scatter_plot(ggp2_movies, 
                    ...
                    alpha_var = 0.4, 
                    size_var = 2.5))
    test_cmt(end =  "scatter_plot()", msg = "ggplot2movies::movies snapshot")
})
```
...
shiny::testServer(app = mod_var_input_server, expr = {
  test_cmt(start = "mod_var_input_server", msg = "initial null")
  testthat::expect_equal(input$x, NULL)
  test_cmt(end = "mod_var_input_server", msg = "initial null")
}) 

shiny::testServer(app = mod_var_input_server, expr = {
  test_cmt(start = "mod_var_input_server", msg = "value types")
  session$setInputs(x = "imdb_rating", 
                    y = "audience_score", 
                    z = "genre", 
                    alpha = 0.5, 
                    size = 2,  
  ...
  testthat::expect_true(
    object = is.character(session$returned()$y))
  testthat::expect_true(
    object = is.character(session$returned()$x))
  testthat::expect_true(
    object = is.character(session$returned()$z))
  testthat::expect_true(
    object = is.numeric(session$returned()$alpha))
  testthat::expect_true(
    object = is.numeric(session$returned()$size))
  testthat::expect_true(
    object = is.character(session$returned()$plot_title))
  test_cmt(end = "mod_var_input_server", msg = "value types")
})
...
shiny::testServer(app = mod_var_input_server, expr = {
  session$setInputs(x = "imdb_rating", 
                    ...
                    size = 2,  
                    plot_title = "example title")
  test_cmt(start = "mod_var_input_server", msg = "var_input class")
  testthat::expect_equal(
    object = session$returned(), 
    expected = list("y" = "audience_score",
                    "x" = "imdb_rating",
                    "z" = "genre",
                    "alpha" = 0.5,
                    "size" = 2,
  ...
  test_cmt(end = "mod_var_input_server", msg = "var_input class")
})
```
