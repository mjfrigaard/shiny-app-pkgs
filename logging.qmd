...
```{verbatim}
[2025-01-26 21:39:01] Loading fst data 
[2025-01-26 21:39:01] Removing missing values. 
[2025-01-26 21:39:01] Inputs constructed: x=year, y=budget, z=mpaa, alpha=0.5, size=2, plot_title= 
[2025-01-26 21:39:56] Inputs constructed: x=year, y=length, z=mpaa, alpha=0.5, size=2, plot_title= 
[2025-01-26 21:40:01] Inputs constructed: x=rating, y=length, z=mpaa, alpha=0.5, size=2, plot_title= 
[2025-01-26 21:40:03] Inputs constructed: x=rating, y=length, z=genre, alpha=0.5, size=2, plot_title= 
[2025-01-26 21:40:10] Inputs constructed: x=rating, y=length, z=genre, alpha=0.5, size=2, plot_title=NEW TITLE 
[2025-01-26 21:40:13] Using all data. 
```
...
## App logs with [`logger`]{style="font-size: 1.05em;"}
...
Use descriptive messages for easier debugging in development.
...
Use lower verbosity levels in production:
...
By default, `logger` logs are written to the console, but the `logger::appender_file()` function can direct log messages to a file (or to a database or external logging system).[^external-logging] 
...
[^external-logging]: Integrate with external logging systems (e.g., [Syslog](https://daroczig.github.io/logger/reference/appender_syslog.html) or [Telegram](https://daroczig.github.io/logger/reference/appender_telegram.html)) using custom appenders.
...
Below are examples of various log levels using `logr_msg()` to debug the error in the primary application. 

### [`TRACE`]{style="font-size: 1.05em;"} logs

1. In `mod_scatter_display_server()`, add a `TRACE` log messages for initiating the scatterplot graph 
...
1. Add a `FATAL` log message (along with `tryCatch()` and `stop()`) in `launch_app()` to monitor the application being launched  
...
```{verbatim}
<ERROR> [2025-01-29 08:04:30] Failed to render scatterplot. 
...
Write package startup messages in the `.onAttach()` function (stored in `R/zzz.R`).[^on-attach] The example below includes a welcome message, the current Git branch, dependencies (imports, depends, and suggests), and the path where this package is loaded *from*.

[^on-attach]: Read more in the ['Hooks for Namespace Events' documentation](https://stat.ethz.ch/R-manual/R-devel/library/base/html/ns-hooks.html).
...
.onAttach <- function(libname, pkgname) {
  ...
  cli::cli_h2("Welcome to the {.pkg {pkgname}} package (version {version}) {cli::symbol$smiley}!") # <7>
  cli::cli_h3("Current branch:") # <8>
  cli::cli_text("{.emph {cli::symbol$pointer} {branch}}")  # <8>
  cli::cli_h3("Dependencies:")  # <9>
  cli::cli_text("{.emph {cli::symbol$pointer} Imports: {imports}}") # <10>
  cli::cli_text("{.emph {cli::symbol$pointer} Suggests: {suggests}}") # <11>
  cli::cli_text("{.emph {cli::symbol$pointer} Depends: {depends}}") # <12>
  cli::cli_h3("Loaded from:") # <13>
  cli::cli_text("{.emph {cli::symbol$pointer} {libname}}") # <14>
  cli::cli_inform(c(), class = "packageStartupMessage") # <15>
}
```
...
15. Output with appropriate class.  
...
#| code-fold: false
ℹ Loading sap
── Welcome to the sap package (version 0.0.0.9130) ☺! ──

── Current branch: 
❯ 13_logging

── Dependencies: 
❯ Imports: bslib, cli, ggplot2, logger, pkgload, rlang, sass, shiny, shinythemes,
stringr, tools, withr
❯ Suggests: None
❯ Depends: R (>= 2.10)

── Loaded from: 
❯ /Users/mjfrigaard/projects/apps/R
```
