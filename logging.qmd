...
Logging is essential for debugging and monitoring apps, helping track errors, warnings, and user activity. `validate()` ensures valid inputs and `tryCatch()` can catch unexpected errors in reactive expressions and log them instead of crashing the app. Key benefits to logging your app include: 
...
4. The `logger` package provides structured logging, allowing us to save logs to files, console, or external services. 
...
### App architecture {#sec-logging-arch-tidy-movies}

We're going to start logging the behaviors of our app in the `inst/tidy-movies` folder. The architecture of this application has been updated to include the two new input modules (review these changes in @sec-resources-inst-tidy-movies-app):
...
flowchart LR
    ...
    %% R/
    style R fill:#f9f9f9,stroke:#999,stroke-width:2px,rx:3,ry:3
    %% standalone app function
    style launch fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px,rx:10,ry:10
    %% modules
    style mod_var_input_server font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:12,ry:12
    style mod_aes_input_ui font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:12,ry:12
    style mod_aes_input_server font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:12,ry:12
    %% utility functions
    style scatter_plot fill:#333,color:#FFF,stroke:#000,stroke-width:1px,rx:25,ry:25
    style display_type fill:#666,color:#FFF,stroke:#333,stroke-width:1px,rx:25,ry:25

    %% tidy-movies/
    style tidy_movies fill:#f9f9f9,stroke:#999,stroke-width:2px,rx:3,ry:3
    %% tidy-movies/R
    style SubR fill:#fafafa,stroke:#ccc,stroke-width:1px,rx:3,ry:3
    %% tidy-movies/R/ files
    style devUI fill:#e8f4f1,stroke:#4CBB9D,stroke-width:1px,rx:6,ry:6
    style devServer fill:#e8f4f1,stroke:#4CBB9D,stroke-width:1px,rx:6,ry:6
    style dev_mod_scatter_server font-size:14px,color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    style dev_mod_scatter_ui font-size:14px,color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    style dev_mod_vars_ui font-size:14px,color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    %% files
    style data fill:#ffffff,color:#000,stroke:#ccc,stroke-width:1px,rx:6,ry:6
    style img fill:#ffffff,color:#000,stroke:#ccc,stroke-width:1px,rx:6,ry:6
    style appdotr fill:#ffffff,color:#000,stroke:#ccc,stroke-width:1px,rx:6,ry:6
```
...
The reactive values from the `R/mod_var_input.R` are printed to the **Console** from the previous chapter (review this in  @sec-debug-ui). 
...
#### [`input$alpha`]{style="font-size: 1.05em;"} {.unnumbered}
...
#### [`input$size`]{style="font-size: 1.05em;"} {.unnumbered}
...
## Base R logging {#sec-logging-funs}
...
log_message <- function(message, log_file = "logs/app_log.txt", save = FALSE) {
  ...
  # log entry
  log_entry <- sprintf("[%s] %s", timestamp, message) # <1>
  # save
  if (save) {
    log_dir <- dirname(log_file) # <2>
    if (!dir.exists(log_dir)) { # <3>
...
1.  The `sprintf()` function is preferred over `cat()` or `paste0()` for clearer and faster string formatting.\ 
2. This function uses `dirname()` to dynamically extract the directory path from `log_file`.\
3.  `dir.exists()` and `dir.create()` ensure the folder exists (`recursive = TRUE` is for nested directories).\
...
## Logging with [`logger`]{style="font-size: 1.05em;"}  {#sec-logging-logger}
...
### App architecture {#sec-logging-arch-movies}
...
flowchart TD
    ...
    %% standalone app function
    style Launch fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px,rx:12,ry:12

    %% modules
    style VarInpuUI font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3
    style VarInpuServer font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3
    style AesInpuServer font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3
    style AesInpuUI font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3
    style DisplayServer font-size:12px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3
    style DisplayUI font-size:14px,color:#fff,fill:#4CBB9D,stroke:#2d8a7a,stroke-width:1px,rx:3,ry:3

    %% Utility Functions
    style PlotUtil fill:#333,color:#FFF,stroke:#000,stroke-width:1px,rx:25,ry:25

    %% namespaces
    style VarServerNS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6
    style VarUINS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6
    style AesServerNS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6
    style AesUINS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6
    style GraphServerNS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6
    style GraphUINS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:6,ry:6

    %% UI and Server
    style Server fill:#f5f5f5,stroke:#999,stroke-width:2px,rx:12,ry:12
    style UI fill:#f5f5f5,stroke:#999,stroke-width:2px,rx:12,ry:12
```
...
2.  In `mod_scatter_display_server()`, a `TRACE` log message for initiating the scatterplot graph:
...
2.  In `movies_ui()`, an `INFO` log message prints the value of the `bslib` argument
...
2.  In `mod_scatter_display_server()`, an `ERROR` log message (with `tryCatch()`) monitors the rendered output:
...
### [`.onAttach()`]{style="font-size: 1.05em;"} {#sec-logging-on-attach}
