...
co_box(
  color = "r",
  fold = FALSE,
  header = "GitHub [`pkgApp`]:[`04-data`]",
  contents = "
  
  This shiny app project can be found in the `04-data` branch of the [`pkgApp`](https://github.com/mjfrigaard/pkgApp/tree/o4-data) repo.
...
## [`data/`]{style="font-size: 1.05em"}

You can read more about the data folder in the ['Data in packages' section of Writing R Extenstions](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Data-in-packages) and the ['Data' chapter of R Packages, 2ed](https://r-pkgs.org/data.html).
...
::: {#fig-data_namespace}
![`roxygen2`](img/data_namespace.png){#fig-data_namespace width='100%' fig-align='center'}

`movies` is now part of `pkgApp`
:::

If you'd prefer to store data using the `.rda` format, the `usethis` package has the [`use_data()` function](https://usethis.r-lib.org/reference/use_data.html) that will automatically store an object in `data/` (*this function will also add `LazyData: true` to the `DESCRIPTION`*)
...
co_box(
  color = "g", fold = TRUE,
  header = "Mastering Shiny data example",
...
::: {#fig-data_rd}
![The `movies` help file](img/data_rd.png){#fig-data_rd width='100%' fig-align='center'}
...
::: {#fig-movies_app_data}
![`movies_app()` with `movies` data file](img/movies_app_data.png){#fig-movies_app_data width='100%' fig-align='center'}
...
### [`data-raw/`]{style="font-size: 1.05em"}
...
Read more about the `data-raw` folder in [R Packages, 2ed](https://r-pkgs.org/data.html#sec-data-data-raw)

## [`inst/extdata/`]{style="font-size: 1.05em"}
...
co_box(
  ...
  header = "GitHub [`pkgApp`]:[`04-data`]",
  contents = "
  The code for this section was pushed to the  [[`04-data`](https://github.com/mjfrigaard/pkgApp/tree/04-data)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.
