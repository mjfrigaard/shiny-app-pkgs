...
We've documented the functions in `moviesApp` and successfully managed the dependencies with the `NAMESPACE` and `DESCRIPTION` files. In this chapter, we're going to cover how make sure the `movie.RData` file becomes part of `moviesApp`, and other locations for data files in app-packages. For information on how to store and retrieve inside your application, see the chapters on [reactiveValues()](@sec-reactiveValues) and [session$userData()](@sec-userData).
...
## The [`data/`]{style="font-size: 1.05em"} folder {#sec-data-data}
...
## The [`data-raw/`]{style="font-size: 1.05em"} folder {#sec-data-data-raw}
...
## The [`inst/extdata/`]{style="font-size: 1.05em"} folder {#sec-data-inst-extdata}
