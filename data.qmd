...
We've documented the functions in `shinyAppPkgs` and successfully managed the dependencies with the `NAMESPACE` and `DESCRIPTION`. In this chapter, we're going to cover how make sure the `movie.RData` file becomes part of `shinyAppPkgs`, can be loaded into our application, and some other purposes and locations for app-package data files. For information on how to store and retrieve inside your application, see the chapter on [app Data](app_data.qmd).
...
Files in `data/` should be in the `.rda` or `.RData` format. Below are the steps for adding `movies` to `shinyAppPkgs`:
...
The diagram below captures what happens to the package files and namespace after adding `LazyData: true` to the `DESCRIPTION` file and exporting `movies` from `shinyAppPkgs`.
...
We can check to see if movies has been included in `shinyAppPkgs` using the `package::data` syntax: 


![`movies` is now part of `shinyAppPkgs`](images/data_movies_namespace.png){#fig-data_movies_namespace width='100%' fig-align='center'}
...
#| eval: false 
#| code-fold: false
✔ Setting active project to '/path/to/shinyAppPkgs'
...
If we load and document `shinyAppPkgs`, we can see a preview of the help file:
...
The figure below captures what happens to the shinyAppPkgs package files after creating the `R/data.R` file. 
...
Then load, document, and install `shinyAppPkgs`:
...
tibble::as_tibble(
  fst::read_fst(path = 
      system.file("extdata/", "movies.fst", package = "shinyAppPkgs")
    )
  )
```
