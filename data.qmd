...
We've documented the functions in `moviesApp` and successfully managed the dependencies with the `NAMESPACE` and `DESCRIPTION` files. In this chapter, we're going to cover how make sure the `movie.RData` file becomes part of `moviesApp`, and other locations for data files in app-packages.
...
- **`data/`**: `usethis::use_data()`...\n
  
- **`data-raw/`**: `usethis::use_data_raw()`...\n
    - Creates new `.R` file in `data-raw/`\n
    - Store in `data/` or inst/extdata/`\n
...
Below are the steps for adding `movies` to `moviesApp`:
