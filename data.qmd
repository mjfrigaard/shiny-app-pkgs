...
We've documented the functions in `shinyrPkgs` and successfully managed the dependencies with the `NAMESPACE` and `DESCRIPTION`. In this chapter, we're going to cover how make sure our data file (`movie.RData`) becomes part of our package and can be loaded into our application. I'll also discuss other locations and purposes for app-package data files. 
...
Data in R packages are typically stored in one of three folders: `data/`, `data-raw/`, and `inst/extdata/`.[^data-pkgs-1] The folder you use will depend on the format, accessibility, and intended purpose of the data file.[^app-data-chpater]

[^app-data-chpater]: For information on how to store and retrieve inside your application, see the chapter on [app Data](app_data.qmd).
...
Files in `data/` should be in the `.rda` or `.RData` format. Below are the steps for adding `movies` to `shinyrPkgs`:
...
The diagram below captures what happens to the package files and namespace after adding `LazyData: true` to the `DESCRIPTION` file and exporting `movies` from `shinyrPkgs`.
...
We can check to see if movies has been included in `shinyrPkgs` using the `package::data` syntax: 


![`movies` is now part of `shinyrPkgs`](images/data_movies_namespace.png){#fig-data_movies_namespace width='100%' fig-align='center'}
...
#| eval: false 
#| code-fold: false
✔ Setting active project to '/path/to/shinyrPkgs'
...
If we load and document `shinyrPkgs`, we can see a preview of the help file:
...
The figure below captures what happens to the shinyrPkgs package files after creating the `R/data.R` file. 
...
Then load, document, and install `shinyrPkgs`:
...
tibble::as_tibble(
  fst::read_fst(path = 
      system.file("extdata/", "movies.fst", package = "shinyrPkgs")
    )
  )
```
