...
You can also view these outputs using `launch_app(run = 'b', bslib = TRUE)`. The `run = 'b'` parameter launches the app in an external browser, and `bslib = TRUE` applies the bslib theme. This uses the default blocking mode (`mode = "run"`).
