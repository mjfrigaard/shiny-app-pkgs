...
```{r}
#| label: git_box_24.1.0_reactive-values
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
### Testing `reactiveValues()` {#sec-testing-reactive-values .unnumbered}
...
[^view-reactive-values-test]: You can view the full `test-mod_scatter_display.R` test file in the [`24.1.4_step_04`](https://github.com/mjfrigaard/moviesApp/tree/24.1.4_step_04) branch.
...
```{r}
#| label: git_box_22.2.0_user-data
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
`make_dev_ggp2_movies()` is designed to take a path or URL (i.e., a `con`nection) as an input and returns a dataset that can be used in the `inst/dev/` application. 
...
devServer <- function(input, output, session) {
  ...
  rVals$inputs <- moviesApp::mod_var_input_server("vars", # <3>
                                                  .dev = TRUE) # <3>
