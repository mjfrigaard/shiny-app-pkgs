...
co_box(
  ...
  header = "![](images/leprechaun.png){width='10%'} &emsp; `leprechaun` framework features", 
  fold = FALSE,
  contents = "
  
- Helper functions for adding modules, js, css, html, etc.
  
- `leprechaun` apps come 'pre-packaged' with UI, server, and standalone app functions
  
- A variety of external resources can be included and 'bundled' in the `inst/` folder 
  
- `leprechaun` helps keep your app-package dependencies low (think `golem` 
à la carte)
...
- [x] **Code files**: new code files in `leprechaun` apps can be created with `usethis::use_r()` or with `leprechaun`'s helper functions.
<br><br>
  - [x] **Modules**: Add modules with `leprechaun::add_module("name")` [^leprechaun-add-module]
<br><br>
...
<br><br>
  - [x] **UI and server**: Add the modules to `R/ui.R` and `R/server.R`.
<br><br>
    -   `R/server.R` includes the `send_message` and `make_send_message(session)` functions by default.[^leprechaun-server-fun]
<br><br>
...
<br><br>
  - [x] **Adding external files**: The `R/zzz.R` file contains the `.onLoad()` function [^leprechaun-onLoad]
<br><br>
  - [x] **`app.R` file**: include an `app.R` in the root directory using  `add_app_file()` [^leprechaun-add-app-file]
...
[^leprechaun-add-app-file]: the `app.R` file includes a call to `pkgload::load_all()`.
...
The `inst/run/app.R` *looks* like a standard `app.R` file, but it's not.[^leprechaun-inst-run-app]
