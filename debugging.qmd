...
Let's start by debugging the scatter plot in `launch_app()`. After loading, documenting and installing `shinyAppPkgs`, launch the application:
...
Which is called from inside `shinyAppPkgs()`:
...
``` sh
debug at shinyAppPkgs/R/mod_scatter_display.R#68: 
  plot <- scatter_plot(
    df = movies, 
    x_var = inputs()$x, 
    y_var = inputs()$y, 
...
debugging in: 
  scatter_plot(df = movies, 
    ...
    alpha_var = inputs()$alpha, 
    size_var = inputs()$size)
debug at /shinyAppPkgs/R/scatter_plot.R#30: # <1> 
{   
...
In [`mod_var_input`](https://github.com/mjfrigaard/shinyAppPkgs/blob/18d_debugging/R/mod_var_input.R): 
...
In the `bslib` portion of [`movies_ui()`](https://github.com/mjfrigaard/shinyAppPkgs/blob/18d_debugging/R/movies_ui.R): 
...
In [`movies_server()`](https://github.com/mjfrigaard/shinyAppPkgs/blob/18d_debugging/R/movies_server.R):
