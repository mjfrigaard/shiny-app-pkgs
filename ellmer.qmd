...
co_box(
  ...
  fold = FALSE,
  contents = "
This chapter is currently being reviewd. Thank you for your patience. 
  "
)
```
...
co_box(
  ...
  fold = FALSE,
  contents = "
The `ellmer` package can streamline your Shiny app development process, offering access to a variety of LLMs, intuitive chat interface, system prompts, and more!

This chapter walks through converting a Shiny application into an app-package with `ellmer`, covering critical steps like modules, utility functions, and `roxygen2` documentation. 

Beginners and experienced Shiny developers will find `ellmer` helpful for building polished, more maintainable applications faster.
  "
)
```
...
After installing the package, we need to set up an LLM model provider. `ellmer` provides 10+ LLM provider options, but for this example, I'll use the [OpenAI](https://openai.com/) model, which requires an API key (`OPENAI_API_KEY`) which can be set in the `.Renviron` file.[^renviron]
...
If we plan on having a [conversation](https://ellmer.tidyverse.org/articles/ellmer.html#what-is-a-conversation) with an LLM, we should pick a location to store our [prompts](https://ellmer.tidyverse.org/articles/ellmer.html#what-is-a-prompt) (and responses). The `ellmer` authors recommend storing these in `inst/prompts`, with a sensible naming convention.[^inst-prompts] 
...
We're going to use `ellmer` to help us convert an existing Shiny app into a Shiny app-package. The application comes from the [Shiny Examples GitHub repo](https://github.com/rstudio/shiny-examples/tree/main/051-movie-explorer). The `README.md` from the original app is available below: 
...
The application files are also stored in the [`inst/51-movie-explorer`](https://github.com/mjfrigaard/sap/tree/27_llm-ellmer/inst/51-movies-explorer) folder of this branch:
...
We'll create an app folder in [`inst/db_app`](https://github.com/mjfrigaard/sap/tree/27_llm-ellmer/inst/db_app), then structure the files like a 'mini' R package: 
...
**Prompt 3**: We'll get even more detailed in the third prompt (stored in [`inst/prompts/prompt-03-app-ast-design.md`](https://github.com/mjfrigaard/sap/blob/27_llm-ellmer/inst/prompts/prompt-03-app-ast-design.md)). In these instructions, we'll include an abstract syntax tree (courtesy of `lobstr::ast()`) with proposed function names and descriptions:
