...
-   `usethis::create_package()` will create a new app-packages, and can be used to convert existing shiny app projects into a shiny app-packages.
...
If you'd like a refresher on the files and folders created in chapter 1, I've provided a summary of these topics below:
...
After creating a `DESCRIPTION` file with the seven mandatory fields and placing all of the `.R` files in the `R/` directory, we'll need to test it's ‘functionality’ with the [`devtools` package](https://devtools.r-lib.org/) (which we'll install and load below).
...
The differences between developing an R package and a shiny app can be boiled down to a handful of habits. These [habit]{style="font-weight: bold; font-size: 1.0em; color: #772953"} call various `devtools` functions: 

1. [Load]{style="font-weight: bold; font-size: 1.0em; color: #772953"} all the functions and data in your app-package with `load_all()`

2. [Document]{style="font-weight: bold; font-size: 1.0em; color: #772953"} the app-package functions and data with `document()`

3. [Install]{style="font-weight: bold; font-size: 1.0em; color: #772953"} the app-package with `install()`
