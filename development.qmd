...
After creating a `DESCRIPTION` file the seven mandatory fields, moving the `.R` files into the `R/` folder, and configuring the project build tools in `.Rproj`, we're ready to test our app-package ‘functionality’ with the [`devtools` package](https://devtools.r-lib.org/).
...
co_box(
  ...
  header = "![](img/devtools.png){width='10%'} &emsp; TLDR", 
  fold = TRUE,
  contents = "
...
We're backing up to the branch we created by [manually editing the `DESCRIPTION` file](https://github.com/mjfrigaard/projApp/tree/04_description) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file.[^dev-1]
...
The differences between developing an R package and a shiny app can be boiled down to a handful of habits. These [habit]{style="font-weight: bold; font-size: 1.0em;"} call various `devtools` functions: 
...
Creating an app-package involves adopting some new `devtools` [habits]{style="font-weight: bold; font-size: 1.0em;"}, and the initial contents of `projApp` hopefully helped demonstrate the purpose of each function. 
