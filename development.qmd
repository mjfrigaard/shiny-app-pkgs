...
## Building [`shinyAppPkgs`]{style="font-size: 0.90em"}

Let's assume we're continuing with the app project we converted manually in the [previous branch](https://github.com/mjfrigaard/shinyAppPkgs/tree/03.1_description)  of `shinyAppPkgs` (the files and folders are below).
...
We're backing up to the branch we created by manually editing the [`DESCRIPTION` file](https://github.com/mjfrigaard/shinyAppPkgs/tree/03.1_description/DESCRIPTION) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file.[^dev-1]
...
The version of `shinyAppPkgs` in this branch has a `DESCRIPTION` file with the seven mandatory fields:
...
### [`shinyAppPkgs.Rproj`]{style="font-size: 1.05em"}
...
Using `load_all()` is similar to calling `library(shinyAppPkgs)` because it loads the code in `R/` along with any data files. `load_all()` is also designed for iteration (unlike using `source()`), and when it's successful, the output is a single informative message:
...
No man pages found in package  ‘shinyAppPkgs’ 
** help
...
-   The first line in the output should look familiar--we saw both of these settings in the `shinyAppPkgs.Rproj` file from the previous chapter
...
-   `No man pages found in package 'shinyAppPkgs'` tells us none of the code in `R/` has adequately been documented (which we'll cover in the `roxygen2` chapter) 
...
-   `DONE (shinyAppPkgs)` means `shinyAppPkgs` was successfully installed!
...
However, I've included an example of running `check()` on `shinyAppPkgs` in the callout box below to demonstrate how it works.
...
Creating an app-package involves adopting some new `devtools` [habits]{style="font-weight: bold; font-size: 1.0em;"}, and the initial contents of `shinyAppPkgs` hopefully helped demonstrate the purpose of each function. 
