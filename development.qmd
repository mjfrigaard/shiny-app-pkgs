...
## [`moviesApp`]{style="font-size: 1.10em"}

Let's assume we're continuing with the app project we converted manually in the [`04_description` branch](https://github.com/mjfrigaard/moviesApp/tree/04_description)  of `moviesApp` (the files and folders are below).
...
We're backing up to the branch we created by [manually editing the `DESCRIPTION` file](https://github.com/mjfrigaard/moviesApp/tree/04_description) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file.[^dev-1]
...
### [`moviesApp.Rproj`]{style="font-size: 1.05em"}
...
::: {#fig-dev_namespace .column}
![`NAMESPACE`](images/dev_namespace.png){#fig-dev_namespace width='100%' fig-align='center'}
...
co_box(
  color = "r",
  header = "GitHub [`moviesApp`]:[`06_devtools`]",
  contents = "
The code for this section can be found in the [[`06_devtools`](https://github.com/mjfrigaard/moviesApp/tree/06_devtools)] branch of the [[`moviesApp`](https://github.com/mjfrigaard/moviesApp)] repo.
...
```{r}
#| label: git_box_moviesApp_06_devtools
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
Creating an app-package involves adopting some new `devtools` [habits]{style="font-weight: bold; font-size: 1.0em;"}, and the initial contents of `moviesApp` hopefully helped demonstrate the purpose of each function. 
