...
The differences between developing an R package and a Shiny app can be boiled down to a handful of [habits]{style="font-weight: bold; color: #CA3C4A;"}, each of which calls a `devtools` function:

::: {.column-margin}

I'll use [this font style]{style="font-weight: bold; color: #CA3C4A;"} to indicate each habit and accompanying function.

:::

1. [Load]{style="font-weight: bold; color: #CA3C4A;"} all the functions and data in your app-package with `load_all()`

2. [Document]{style="font-weight: bold; color: #CA3C4A;"} the app-package functions and data with `document()`

3. [Install]{style="font-weight: bold; color: #CA3C4A;"} the app-package with `install()`
...
`load_all()` is the most common `devtools` function used during development. [Load the package when anything changes in the `R/` folder.]{style="font-weight: bold; font-size: 1.10em; color: #CA3C4A;"} 
...
[Document a package whenever changes are made to the `roxygen2` syntax or `DESCRIPTION`.]{style="font-weight: bold; font-size: 1.10em; color: #CA3C4A;"} 
...
[Install a package after the initial setup, after major changes to the code, documentation, or dependencies, and before committing or sharing.]{style='font-weight: bold; font-size: 1.10em; color: #CA3C4A;'}
