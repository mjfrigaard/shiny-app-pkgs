...
After creating a `DESCRIPTION` file with the mandatory fields, moving the `.R` files into the `R/` folder (and--if we're using RStudio--configuring the project build tools in `.Rproj`). Now we're ready to start developing our app-package with [`devtools`](https://devtools.r-lib.org/).
...
## [`devtools`]{style="font-size: 1.10em"} (and friends!)
...
Let's assume we're continuing with a Shiny project from the [previous branch](https://github.com/mjfrigaard/sap/tree/03.1_description)  of `sap` (the files and folders are below).
...
However, the `.Rproj` file in this branch is still configured to work with a Shiny project.[^dev-2]

[^dev-2]: If you created your Shiny app using the **New Project Wizard**, your `.Rproj` file has been configured to work with an RStudio project, not an R package. 
