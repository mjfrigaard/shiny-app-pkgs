...
The previous chapter described the contents of R packages, how to create new app-packages, and how to convert an existing shiny app project into an app-package. The key points are summarized below: 
...
co_box(
  color = "b", 
  fold = FALSE,
  header = "Summary of Chapter 2: Packages",
  contents = "
...
If you'd like a refresher on the files and folders created in chapter 1, I've also provided a summary of these topics below:
...
After creating a `DESCRIPTION` file with the seven mandatory fields and placing all of the `.R` files in the `R/` directory, we'll need to test its ‘functionality’ with the [`devtools` package](https://devtools.r-lib.org/) (which we'll install and load below).
...
> ["One package to rule them all."]{style="font-weight: bold; font-style: italic; color: #FF5200;"} - **Not** the `devtools` development team
...
Let's assume we're continuing with the app project we converted manually in the [`04_description` branch](https://github.com/mjfrigaard/projApp/tree/04_description)  of `projApp` (the files and folders are below).

```{r}
#| label: co_box_description_branch
#| echo: false
#| results: asis
#| eval: true
co_box(
  color = "o",
  header = "Why this branch?", 
  contents = "We're backing up to the [`04_description` branch](https://github.com/mjfrigaard/projApp/tree/04_description) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file. 
  
If your package was created or converted with `usethis::create_package()`, these fields are automatically added without explaining their role or purpose.", 
  fold = FALSE
)
```
...
### [`projApp.Rproj`]{style="font-size: 1.05em"}
...
## Package development [habits]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"}
...
co_box(
  ...
  header = "Important!",
  contents = "This section shouldn't be considered a replacement for the ['Whole Game' chapter in R packages (2 ed)](https://r-pkgs.org/whole-game.html) (which I highly recommend reading).", 
  fold = FALSE
)
```

The differences between developing an R package and a shiny app can be boiled down to a handful of habits. These [habit]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"} call various `devtools` functions: 

1. [Load]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"} all the functions and data in your app-package with `load_all()`

2. [Document]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"} the app-package functions and data with `document()`

3. [Install]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"} the app-package with `install()`
...
### [Load]{style="font-weight: bold; color: #6F1AB6"}
...
`load_all()` is the most common `devtools` function we'll use during development because we should [load the package when anything changes in the `R/` folder.]{style="font-weight: bold; font-size: 1.10em; color: #6F1AB6"} 
...
### [Document]{style="font-weight: bold; color: #6F1AB6"} 
...
[Document the package whenever changes are made to any `roxygen2` syntax (or settings).]{style="font-weight: bold; font-size: 1.10em; color: #6F1AB6"} 

### [Install]{style="font-weight: bold; color: #6F1AB6"}  
...
[Install a package after the initial setup, after major changes to the code, documentation, or dependencies, and before committing or sharing.]{style='font-weight: bold; font-size: 1.10em; color: #6F1AB6'}
...
co_box(
  ...
  contents = "
The code for this section can be found in the [[`06_devtools`](https://github.com/mjfrigaard/projApp/tree/06_devtools)] branch of the [[`projApp`](https://github.com/mjfrigaard/projApp)] repo.
...
```{r}
#| label: git_box_projApp_06_devtools
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
### [Check?]{style="font-weight: bold; color: #6F1AB6"}  
...
[After adding a bug fix or feature, check a package and keep any notes, warnings, or errors from accumulating.]{style="font-weight: bold; font-size: 1.10em; color: #6F1AB6"}

:::

### Recap 

Creating an app-package involves adopting some new `devtools` [habits]{style="font-weight: bold; font-size: 1.0em; color: #6F1AB6"}, and the initial contents of `projApp` hopefully helped demonstrate the purpose of each function. 
...
After installing and loading `devtools`: 
  ...
  1. [Load the package whenever changes occur in the `R/` folder]{style='font-weight: bold; font-size: 1.0em; color: #6F1AB6'}.\n 
      - [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd>]{style='font-weight: bold; font-size: 0.75em'} load all the code in the package.\n
  
  2. [Document the package whenever changes are made to any `roxygen2` syntax (or settings)]{style='font-weight: bold; font-size: 1.0em; color: #6F1AB6'}.\n 
  ...
  3. [Install the package after the initial setup, after major changes to the code, documentation, or dependencies, and before committing or sharing]{style='font-weight: bold; font-size: 1.0em; color: #6F1AB6'}.\n  
