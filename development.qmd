...
[R Package Habits: load, document, and install]{style="font-size: 1.25em; font-style: italic; color: #02577A;"}
...
co_box(
  ...
  header = "TLDR &emsp; ![](img/devtools.png){width='45' height='50'}", 
  fold = TRUE,
  contents = "
...
## Building [`moviesApp`]{style="font-size: 0.90em"}

Let's assume we're continuing with the app project we converted manually in the [`03.1_description` branch](https://github.com/mjfrigaard/moviesApp/tree/03.1_description)  of `moviesApp` (the files and folders are below).
...
We're backing up to the branch we created by manually editing the [`DESCRIPTION` file](https://github.com/mjfrigaard/moviesApp/tree/03.1_description/DESCRIPTION) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file.[^dev-1]
...
### [Running `devtools::check()`]{style="font-size: 1.25em"}
