...
Let's assume we're continuing with the app project we converted manually in the [`03a_description` branch](https://github.com/mjfrigaard/moviesApp/tree/03a_description)  of `moviesApp` (the files and folders are below).
...
#| eval: false 
#| code-fold: false
moviesApp/ # 03a_description branch
  ├── DESCRIPTION
  ├── R
...
We're backing up to the branch we created by [manually editing the `DESCRIPTION` file](https://github.com/mjfrigaard/moviesApp/tree/03a_description) to show the connection between the `devtools` functions and specific fields in the `DESCRIPTION` file.[^dev-1]
...
```{r}
#| label: git_box_moviesApp_06b_devtools
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
