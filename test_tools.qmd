...
For example, I've stored the code used to create a ['tidy' version](https://github.com/mjfrigaard/shinyrPkgs/blob/12a_tests-fixtures/tests/testthat/fixtures/make-tidy_ggp2_movies.R) of the `ggplot2movies::movies` data along with the output dataset in `tests/testthat/fixtures/`:
...
I tend to use functions like `test_logger()` enough to justify placing them in a testing utility file ([`R/testthat.R`](https://github.com/mjfrigaard/shinyrPkgs/blob/13_tests-modules/R/testthat.R)) below `R/`. Including testing functions in the `R/` folder also ensures they're documented (and any dependencies become part of your app-package).[^test_tools-helpers-r-dir]
...
I've included a small UI function (`text_logo()`) in the `R/` folder of `shinyrPkgs`: 
