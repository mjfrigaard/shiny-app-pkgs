...
This chapter introduces tools to help write clean and efficient tests. We'll cover using the [`ensure` package](https://simonpcouch.github.io/ensure/) to help write tests, where to store test data, snapshots, and more. Most of methods I'll cover below are thoroughly described in [R Packages, 2ed](https://r-pkgs.org/testing-design.html#sec-testing-design-principles) and the `testthat` [documentation](https://testthat.r-lib.org/articles/third-edition.html), but within the context of a standard R package.[^test_tools-documentation] 
...
#| results: asis
#| eval: true
shinypak_apps(regex = "^16", branch = "16.1_test-data")
```

## Help writing tests {#sec-test-tools-help}

I recommend using the [`ensure` package](https://simonpcouch.github.io/ensure/) to help write tests in your app-package. If you're already using an LLM to help write code, `ensure` is a nice addition to your LLM toolbox because of it's focus on testing. 
...
The `ensure` package does a great job when writing basic unit tests, but as you'll see, it doesn't have a solid understanding of the reactivity in your application (but you should). The test code created by `ensure` often needs some editing. 
...
We can continue trying to find something produced by  `ggplot2::ggplot_build(p)`, but this test confirms the "*scatter_plot creates a ggplot object*", so why not just use `ggplot2::is.ggplot()`:
...
I've provided this example because it illustrates some important limitations when using `ensure` (or any LLM) to help write code: **Don't confuse volume with precision.** LLMs are great at generating lots of code to answer questions when a the better answer is concise and accurate. In this case, it arguably would've taken less time to do the research and write the correct test (instead of debugging the one created by `ensure`). 

It's also worth pointing out that if the test *did* pass, the expectations wouldn't match the test description (these belong in a "*scatter_plot x and y limits match true range of values*" test).
...
Code outside of `test_that()` usually serves a specific purpose (load or manipulate data, set options, create folders or files, etc). The topics below demonstrate how to include these behaviors in tests without placing code outside the test scope.[^bdd-chapter]
