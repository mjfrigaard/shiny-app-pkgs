# [`roxygen2`]{style="font-size: 1.05em;"} {.unnumbered}
...
::: {#fig-no_man_pages}
![no `man/` pages](img/no_man_pages.png){#fig-no_man_pages width='100%' fig-align='center'}
...
We'll address this message by going over the fundamentals of the `roxygen2` syntax--these basics should be included for all the functions in your app-package. I'll also touch on a few helpful `roxygen2` tags that are specific to the app-packages. 

The current state of `pkgApp` is stored in the [`main` branch](https://github.com/mjfrigaard/pkgApp/tree/main) (it's identical to the [`06_create-package` branch of `projApp`](https://github.com/mjfrigaard/projApp/tree/06_create-package)). 
...
(*the `rsconnect/` folder from deploying `projApp` has has been removed*)

I strongly encourage checking out the [`roxygen2` documentation](https://roxygen2.r-lib.org/articles/rd.html) and [R Packages](https://r-pkgs.org/man.html#roxygen2-basics) for more information on documenting the code (and data) in your app-package. 

## [`roxygen2`]{style="font-size: 1.10em"} basics

```{r}
#| label: co_box_roxygen2_pkgApp
#| echo: false
#| results: asis
#| eval: false
co_box(
  color = "r",
  fold = FALSE,
  header = "GitHub [`pkgApp`]:[`02-roxygen2`]",
  contents = "
  This shiny app project can be found in the `02-roxygen2` branch of the [`pkgApp`](https://github.com/mjfrigaard/pkgApp/tree/02-roxygen2) repo.
  
From the `main` branch, this branch was created using:
  
  `\`\`\bash
  git checkout -B 02-roxygen2
  git push -u origin 02-roxygen2
  `\`\`\
...
[`roxygen2`](https://roxygen2.r-lib.org/) connects the package code (i.e., the`.R` files in the `R/` folder) to it's documentation files (i.e., the`.Rd` files in the `man/` folder):

::: {#fig-roxygen2_connects_r_man}
![`roxygen2`](img/roxygen2_connects_r_man.png){#fig-roxygen2_connects_r_man width='55%' fig-align='center'}
...
R documentation (`.Rd`) files have a formatting style similar to [(La)TeX](https://www.latex-project.org/) (but `roxygen2` saves us from having to learn this syntax). If we open `man/scatter_plot.Rd`, we see it contains the following: 

:::: {.column-body-outset-right}

::: {#fig-rd_title_description}
![`.Rd` file](img/rd_title_description.png){#fig-rd_title_description width='100%' fig-align='center'}
...
::: {#fig-tag_rd_link}
![`@tag` links to `.Rd` file](img/tag_rd_link.png){#fig-tag_rd_link width='100%' fig-align='center'}
...
::: {#fig-help_with_md}
![`.Rd` file](img/help_with_md.png){#fig-help_with_md width='100%' fig-align='center'}
...
::: {#fig-aes_examples_example layout-nrow=2}

![**Run examples** hyperlink](img/aes_examples.png){#fig-aes_examples width='100%' fig-align='center'}

![Executed `@examples` code](img/aes_examples_example.png){#fig-aes_examples_example width='100%' fig-align='center'}
...
::: {#fig-rd_examples_doc}
![`@examples` in `.Rd` preview](img/rd_examples_doc.png){#fig-rd_examples_doc width='100%' fig-align='center'}
...
::: {#fig-mod_var_input_ui_seealso}
![`@seealso` in `mod_var_input_ui()`](img/mod_var_input_ui_seealso.png){#fig-mod_var_input_ui_seealso width='80%' fig-align='center'}

Link `mod_var_input_ui()` to `mod_var_input_server()`
:::
...
::: {#fig-seealso_link_01}
![`@seealso` link](img/seealso_link_01.png){#fig-seealso_link_01 width='100%' fig-align='center'}
...
::: {#fig-family_link_01}
![`@family` link](img/family_link_01.png){#fig-family_link_01 width='100%' fig-align='center'}

Link from `var_input` module and group `scatter_display` module
:::
...
::: {#fig-family_link_02}
![`@seealso` and `@family` connections](img/family_link_02.png){#fig-family_link_02 width='100%' fig-align='center'}

Connecting inputs and outputs, grouping modules, and linking utility functions
:::
...
#### [`movies_ui()`]{style="font-size: 0.90em;"}
...
::: {#fig-movies_ui_rd}
![`movies_ui()`](img/movies_ui_rd.png){#fig-movies_ui_rd width='100%' fig-align='center'}

UI function documentation
:::



#### [`movies_server()`]{style="font-size: 0.90em;"}
...
::: {#fig-movies_server_rd}
![`movies_server()`](img/movies_server_rd.png){#fig-movies_server_rd width='100%' fig-align='center'}
...
::: {#fig-movies_app_rd}
![`movies_app()`](img/movies_app_rd.png){#fig-movies_app_rd width='100%' fig-align='center'}
...
The code for the next section is in the  [[`02_roxygen2`](https://github.com/mjfrigaard/pkgApp/tree/02_roxygen2)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.

:::


```{r}
#| label: co_box_push_02-roxygen2
#| echo: false
#| results: asis
#| eval: false
co_box(
  ...
  header = "GitHub [`pkgApp`]:[`02-roxygen2`]",
  contents = "
  The code for this section was pushed to the  [[`02-roxygen2`](https://github.com/mjfrigaard/pkgApp/tree/02-roxygen2)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.
...
The `roxygen2` documentation for `pkgApp` is saved in the [`02-roxygen` branch](https://github.com/mjfrigaard/pkgApp/tree/02-roxygen). As we can see, calling the `roxygen2` comments and tags generated `.Rd` files in the `man/` folder for each function in the `R/` folder:
...
The goal of good documentation is to make it as easy as possible for others (and future you) to understand what your function does and how to use it. 
...
co_box(
  ...
  - **Give more info with `@section`**s: Use `@section` to give in-depth explanations and more specific details about how a function works, including any information about the function `@usage` or it's `@param`s.
