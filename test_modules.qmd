...
The feature and scenario for these changes are provided below: 
...
Inside `testServer()`, if we're testing a module function that collects the reactive values, we need to wrap those values in `reactive()` in the `args()` argument:
