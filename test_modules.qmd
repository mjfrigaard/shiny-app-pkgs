...
We can combine the BDD functions with `testServer()` to test *reactive interactions* between modules. For example, to confirm that the drop-down feature requirement (**F1.2**) is working (i.e., that user-inputs are updating in the application), we need to test two changes:
...
```{verbatim}
#| code-fold: false
Feature 1.2: Dropdown menus (continuous x & y axes, categorical point colors)
...
testthat::describe(
  ...
  testthat::it(# <2>
    "Scenario: Scatter plot initial x, y, color values 
      ...
      Then the scatter plot should show 'IMDB Rating' on the x-axis
      And the scatter plot should show 'Audience Score' on the y-axis
      And the points on the scatter plot should be colored by 'MPAA Rating'", # <2>
    code = {                     
    ...
    shiny::testServer(app = mod_var_input_server, expr = {       # <3>
      ...
      test_vals <- list(y = "imdb_rating",                      # <4>
                        x = "audience_score",
                        z = "mpaa_rating",
                        alpha = 0.75,
                        size = 3,
      ...
      # change inputs
      session$setInputs(y = "imdb_rating",                       # <5>
                        x = "audience_score",
                        z = "mpaa_rating",
                        alpha = 0.75,
                        size = 3,
...
```
1. Feature description (**F1.2**)   
2. Test scope for inputs  
...
```
1. Feature requirement description 
2. Scenario test scope    
