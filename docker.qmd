...
We'll be using the `shinyrPkgs` example from the previous chapter as an example (the folder/file structure is below):
...
When the container starts, set Shiny to listen on port `8180`, then launch the Shiny app from our `shinyrPkgs` package. The port we exposed in the `Dockerfile` should match the `shiny.port` option.
...
> options('shiny.port'=8180,shiny.host='0.0.0.0');library(shinyrPkgs);shinyrPkgs::launch_app()
