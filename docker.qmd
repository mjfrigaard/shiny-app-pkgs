...
We can write a `Dockerfile` (a text file containing a series of instructions) inside our app-package that defines how to build the Docker image. The image is a snapshot representing our Shiny app and its dependencies. Docker containers are instantiated with the `build` command, which reads and executes the instructions inside the `Dockerfile` and creates a runtime instance of the image.
...
flowchart TB
    subgraph AP["App-Package"]
        subgraph DockFile["<code>Dockerfile</code>"]
            Instructions["Instructions for<br>building image"]
        end
    end
        subgraph DockImg["Docker Image"]
            Image["Dependencies for Shiny app"]
        end

        subgraph DockCont["Docker Container"]
            R["R version"]
            ShinyDep["Shiny server"]
        end

    DockFile -->|"<code>docker build</code> <em>creates instance of image</em>"| DockImg
    DockImg --> |"<em>Executes instance of image</em>"|DockCont
```
