...
The `inst/` folder is versatile tool that we can use to store a variety of files we need in our application (logos or images, CSS styling, JavaScript functions, HTML, etc). W.

For example, we can use `inst/` to store alternate image files and configure the UI to display a different layout. This method ensures our app has the exact same functionality, but a different UI layout.[^layouts-git] 
