...
In `inst/dev/`, we've place the application modules in [`app.R` file](https://github.com/mjfrigaard/shinyAppPkg/blob/07_dev-ggp2movies/inst/dev/app.R):

1. The `mod_variables_ui()` development module contains choices for the names in the `tidy_movies` data:
...
2. The 'development' version of the display module imports the `tidy_movies.fst` data file and an option for removing missing values.
  
```{r}
#| eval: false 
#| code-fold: true 
#| code-summary: 'show/hide mod_scatter_plot'
mod_scatter_plot_ui <- function(id) {
  ...
  mod_scatter_plot_server <- function(id, var_inputs) {
    moduleServer(id, function(input, output, session) {
      # load alternate data
      all_data <- fst::read_fst("tidy_movies.fst")
  
      
      graph_data <- reactive({ # <3> 
        if (input$missing) { # <3> 
          graph_data <- tidyr::drop_na(data = all_data)  # <3> 
        } else {  # <3> 
      ...
      observe({ # <5>
        ...
        bindEvent(graph_data(), inputs()) # <5>
    })
  }
```
1. Alternate help text for `ggplot2movies` data  
2. Check-box to remove missing values     
3. Build reactive data based on missing checkbox input    
4. Bind `missing` input to update when UI changes   
5. Bind plot output to update with variable inputs and 'missing' checkbox output  

By including a call to `library(shinyAppPkg)` at the top of `app.R`, the `scatter_plot()` function is available to use in the development app module.

#### [`dev/` UI and server]{style="font-size: 0.95em;"} 

`app.R` also contains development app UI and server functions. 

In `appUI()`, development modules are paired with package modules (`mod_variables_ui()` with the `mod_var_input_server()`), and we've include an alternate image (`bootstrap.png`).[^export-modules-6]
...
2. Refer to alternate image  

```{r}
#| eval: false 
#| code-fold: true 
#| code-summary: 'show/hide appServer()'
appServer <- function(input, output, session) {
  
  selected_vars <- mod_var_input_server("vars") # <1>

  mod_scatter_plot_server("plot", var_inputs = selected_vars) # <2>
  
}
```
1. `mod_var_input()` from `R/` folder  
2. Defined in `dev/inst/app.R`

#### [Launching `dev/`]{style="font-size: 0.95em;"} app

Finally, we'll the development version of the app is launched with it's own standalone app function (stored in `R/ggplot2movies_app.R`). The `ggplot2movies_app()` function looks a lot like `movies_app()`, but without the `display_type()` helper and the `appDir` argument set to the location of the development app (which we provide with `system.file()`).
...
#'
#' @export
ggp2_movies_app <- function(test = TRUE) {
