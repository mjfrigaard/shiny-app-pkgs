...
In `inst/dev/`, we've placed the application modules in an [`app.R` file](https://github.com/mjfrigaard/shinyAppPkg/blob/07_dev-ggp2movies/inst/dev/app.R):

1. `mod_variables_ui()` contains choices for the names in the `tidy_movies` data:
...
2. The 'development' version of the display module imports the `tidy_movies.fst` data file and includes an option for removing missing values.
...
Finally, the development app is launched with it's own standalone function (stored in `R/ggplot2movies_app.R`). The `ggplot2movies_app()` function is similar to `movies_app()`, but without the `display_type()` helper, and the `appDir` argument is set to the location of the development files (which we provide with `system.file()`).
