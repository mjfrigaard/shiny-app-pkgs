...
If we viewed the files at the end of the path for `moviesApp` displayed above, we'd see the files and folders for an *installed* package. Below are folder trees for our source package and our installed package:[^path-package]
...
The installed version of `moviesApp` has some of the same files as the 'source' version we've been working with (i.e., `NAMESPACE` and `DESCRIPTION`), but it also might surprise you that many of the source files aren't included in the installed version (i.e., the `.R` and `.Rd` files, the `man/` folder).
