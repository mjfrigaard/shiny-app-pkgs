...
The `inst/` folder can also be used to store files we're using in alternate versions of our application. For example, if I wanted to test a differenet layout using the [`bslib` package](https://rstudio.github.io/bslib/index.html), I could include a `bslib` argument in the `movies_ui()` function and include the alternate image file (in `inst/www/bootstrap.png`):
...
#### [`ggplot2movies`]{style="font-size: 1.05em;"} app
...
We can create a home for this development version in `inst/dev/`, then place the application code in an [`app.R` file](https://github.com/mjfrigaard/pkgApp/blob/07_dev-ggp2movies/inst/dev/app.R). We'll also include the alternate image (`bootstrap.png`) and data file (created with `data-raw/ggp2movies.R`):
...
To run this development version of `movies_app()`, I'll create `R/dev_movies_app.R`: 
...
#' @export
#' 
dev_movies_app <- function(test = TRUE, run = "win") {
  switch(EXPR = run,
...
`dev_movies_app()` looks a lot like `movies_app()`, but the `appDir` argument is the location of the development application, which we provide with `system.file()`.

[<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd> / <kbd>D</kbd> / <kbd>B</kbd>]{style="font-style: italic; font-weight: bold; font-size: 1.10em"}

After loading, documenting, and installing `pkgApp`, we can run the development version using `dev_movies_app()`: 

:::: {.column-body-outset-right}

::: {#fig-07_inst_ggp2movies_app}
![`dev_movies_app()`](img/07_inst_ggp2movies_app.png){#fig-07_inst_ggp2movies_app width='100%' fig-align='center'}
