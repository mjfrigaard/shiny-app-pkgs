...
This chapter covers using `reactiveValues()` and `session$userData` in your application to store values and objects. Determining whether to use `reactiveValues()` or `session$userData` will depend on the purpose you want them to serve in your application.


```{r}
#| label: git_box_spec_topic-reactiveValues-userData
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
  fig_pw = '65%', 
  branch = "spec_topic-reactiveValues-userData", 
  repo = 'moviesApp')
```
...
Calling `reactiveValues()` will create *"an object for storing reactive values."* [^reactives-shiny-doc-reactive-values]
Below is a demonstration of using `reactiveValues()` to capture the values returned from the `var_input` module and passed to the `scatter_display` module in `movies_app()`:
...
Change `.dev` in `mod_var_input_server()` to `TRUE` in `movies_server()`, then load and run the application. The updated `movies_server()` function should look like this: [^reactives-debugging-ref]
...
## Testing `reactiveValues()`

The module tests for `test-mod_scatter_display.R` had to be redesigned to handle the `reactiveValues()` input. You can view the full test file in [this branch](https://github.com/mjfrigaard/moviesApp/blob/spec_topic-reactiveValues-userData/tests/testthat/test-mod_scatter_display.R), but to briefly summarize: 
