...
flowchart LR
 subgraph Function["Function Scope"]
        ...
        Browser -- Execution Paused --> Debugger("View variables/values")
    ...
    style Exit fill:#4CB7DB,stroke:none,rx:10,ry:10
...
In the [previous chapter](external.qmd) we developed an application using the `ggplot2movies` data.[^ggp2-callback] In the following sections we're going to use the interactive debugger to see the inner workings of the 'Remove missing' checkbox.
