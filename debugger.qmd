...
flowchart TD
    subgraph Function["<strong>Function Scope</strong>"]
        ...
        subgraph BrowserStep["<strong>browser() Scope<strong>"]
            Encounter("Encounter<br><code>browser()</code>") --> Step
            
            subgraph Debugger["<strong>Interactive Debugger</strong>"]
                Step[\"Inspect variables"/]
            end
            Step --> ExitBrowser("Exit <br><code>browser()</code>")
            
        end
        ExitBrowser --> Resume(["Resume<br>execution"])
...
flowchart TD
    subgraph Function["<strong>Function Scope</strong>"]
        Running(["Being<br>execution"])
        Running --> EncounterObs
        subgraph ObserveStep["<strong>observe() Scope</strong>"]
            EncounterObs("Encounter<br><code>observe()</code>") --> Browse
            subgraph Browser["<strong><code>browser()</code> Scope</strong>"]
                Browse("Encounter<br><code>browser()</code>") --> Step
                subgraph Debugger["<strong>Interactive Debugger</strong>"]
                 Step[\"Inspect variables<br>and reactives"/]
            end
                Step --> ExitBrowser("Exit <br><code>browser()</code>")
            end
            ExitBrowser --> ExitObserve("Exit <br><code>observe()</code>")
            
        end
        ExitObserve --> Resume(["Resume<br>execution"])
    end
