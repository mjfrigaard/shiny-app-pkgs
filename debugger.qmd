...
flowchart TD
    subgraph FunctionScope [<strong>Function Scope</strong>]
        ...
        subgraph Debugger ["<strong>Debugger</strong>"]
            ...
            Step1("Inspect<br>Variables")
            Step1 --> Step2("Step Through<br>Code")
            Step2 --> Debug{"End<br>Debugging?"}

            Debug -->|"No"| Step1
            Debug -->|"Yes"| Exit(Exit Debugger)
...
%%{init: {'theme': 'neutral', 'themeVariables': { 'fontFamily': 'monospace', "fontSize":"13px"}}}%%
flowchart TD
    subgraph Function["<strong>Function Scope</strong>"]
        Running("Execution")
        Running --> Encounter
        
        subgraph BrowserStep["<strong>browser()<strong>"]
            Encounter("Encounter<br><code>browser()</code>") --> Step1
            
            subgraph Debugger["<strong>Debugger</strong>"]
                Step1("View<br>variables/values") --> Step2("Execute code<br>line-by-line")
            end
            Step2 --> ExitBrowser("Exit the<br><code>browser()</code>")
            
        end
        ExitBrowser --> Resume("Resume execution")
...
```{mermaid}
%%| fig-cap: 'Reacting `browser`ing in an `observe`er'
%%| fig-align: center
%%{init: {'theme': 'neutral', 'themeVariables': { 'fontFamily': 'monospace', "fontSize":"13px"}}}%%
flowchart TD
    subgraph Server["server() Scope"]
        StartReact("Reactive Execution")
        StartReact --> ObserveExecution("browser()</code><br>invalidates observer")
        subgraph Observer["<strong>observe() Scope</strong>"]
            ObserveExecution("browser()</code><br>invalidates observer")  
            ObserveExecution --> Browser
            subgraph Browse["<strong>browser() Scope</strong>"]
                Browser("<code>observe</code> scope<br>execution paused")
                Browser --> Step
                subgraph Debugger["<strong>Debugger</strong>"]
                    Step("View variables<br>& reactives")
                end
                    ExitBrowser("Exit<br><code>browser()</code>")
                    Step --> ExitBrowser
                
            end
                ExitObserver("Exit<br><code>observe()</code>")
                ExitBrowser --> ExitObserver
        end
            ExitObserver --> EndReact("Resume Reactive<br>Execution")
