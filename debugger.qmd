...
flowchart TD
    ...
    style Start fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Browser fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Resume fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Step fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Debug fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Exit fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Debugger fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
```
...
Running -->|"Encounter"| Browser --> |"Enter"|Step -->|"Exit"|Resume
    ...
    style Running fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Browser fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Step fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Resume fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Debugger fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style Browse fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
...
```{r}
#| label: launch_ggp2_test_mode
#| eval: false 
#| code-fold: false
launch_app("ggp2", options = list(test.mode = TRUE))
```
...
The **Console** tells us where the application is `Listening` and displays the interactive debugger prompt: 
...
The output in the **Console** tells us the placement of the `observe()` call has suspended the execution of the application *before* the `tidy_movies.fst` data are loaded and the graph is rendered in the UI.
...
As we 'step through' the function, the **Editor** in Positron ![](images/positron.png){height=20} highlights the line [in yellow]{style="background-color: #FAFFC1; font-weight: bold;"}:

![Line number in **Editor** (click to enlarge)](images/debug_positron_highlight_next_exec.png){fig-align='center'}
...
If we view the next calls just outside the `observe()` scope, we see it's using the application inputs (i.e., `var_inputs()`). This is available both as the reactive and the actual values:
