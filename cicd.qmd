...
The **style** widget contains each step in the workflow file. If we click on it we can see each step executed in real-time. Below we can see the dependency lock file from `pak`:
...
When the workflow has finished, we can pull the style changes to our local branch and view styled files:[^rlib-actions-examples] 
...
We can see the workflow running on the Action tab of the moviesApp repository: 
