...
co_box(
  ...
  header = "TLDR: Resources and external files", fold = TRUE,
  contents = "

<br>

**Adding external files/resources:**
...
While developing, we access files from the IDE's file explorer. However, as noted in the [data](data.qmd) chapter, these aren't the files in our *installed* package. We can use  `system.file()` and `fs::dir_tree()` to print a folder tree of our *installed* package files:
...
To use `www/` from `inst/`, call `addResourcePath()` in your UI function with `system.file("www", package = "sap")`, then reference files as `src = "www/filename.png"`. See [addResourcePath() documentation](https://shiny.posit.co/r/reference/shiny/latest/addResourcePath.html) for full details.

After loading, documenting, and installing the package:
...
The `mode` parameter controls how the app launches: `mode = "run"` (default) uses blocking mode, which is appropriate for development and deployment contexts. For rapid iteration during development, `mode = "start"` launches the app non-blocking so your console remains available (see the [Launch chapter](launch.qmd) for details).
...
flowchart LR
    ...
    style display_type fill:#333,color:#FFF,stroke:#000,stroke-width:1px,rx:25,ry:25
    ...
    %% tidy-movies/R
    style app fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:3,ry:3
    ...
    style dev_mod_scatter_server color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    style dev_mod_scatter_ui color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    style dev_mod_vars_ui color:#fff,fill:#7fd9c7,stroke:#4CBB9D,stroke-width:1px,rx:12,ry:12
    %% files
    style data fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style img fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
...
#' @import shiny
#' 
launch_app <- function(app, options = list(), mode = "run", run = "p", ...) {
