...
We used `usethis::use_pkgdown_github_pages()` to configure our app-package, so we'll use `build_site_github_pages()` to build our site. 
...
Because our R package contains a Shiny app, we probably should include links to the landing page for your app, provide detailed examples of how it works, and link to any additional resources or documentation. The `navbar` components can also be customized with titles, sections, and article names. In `_pkgdown.yml`, the `articles` are listed under `components`, with a `text` title (`<TITLE>`):
