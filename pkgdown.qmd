...
The YAML syntax and corresponding `pkgdown` output are below:
...
Back in the **Actions** tab, we see the **pages build and deployment** workflow has been created, but with a 'bot' tag:

![](images/pkgdown_pages_build_deployment_workflow.png){width='100%'}

This is because although we committed and pushed changes to the `17_pkgdown` branch (`#10`), the website is being served from the `gh-pages` branch (`#11`. This means each time we push changes to `17_pkgdown`, a corresponding **pages build and deployment** workflow will be created to build and deploy the site:
...
**`pkgdown`** is handy for creating beautiful, functional websites for your app-package. Package sites help share your app-package with others in a more engaging and informative way.
  ...
  -  We can change our `pkgdown` site theme, set colors and fonts, organize the navigation bar, and add custom sections and pages. For an app-package, this means we can create a landing page for our Shiny app, provide detailed articles on how it works, and link to any additional resources or documentation.
  ...
  -  `build_site_github_pages()` goes through your app-package (the `roxygen2` function documentation and examples, vignettes, README) and assembles a coherent, navigable site.
  
  - **Deploying Your Site**:

  -  After building a `pkgdown` site, we can use GitHub Actions to upload the generated website contents to GitHub Pages, which hosts our app-package site directly from a GitHub repository.
