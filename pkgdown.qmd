...
### `man/` -> [Reference](https://mjfrigaard.github.io/shinyrPkgs/reference/index.html)
...
### `vignettes` -> [Articles](https://mjfrigaard.github.io/shinyrPkgs/articles/shinyrPkgs.html)
...
Reading 'vignettes/shinyrPkgs.Rmd'
Writing 'articles/shinyrPkgs.html'
...
[^get-started-vignette]: I created this vignette with `usethis::use_vignette("shinyrPkgs")` and included instructions for launching the various apps in `shinyrPkgs`.
...
[^exported-funs]: Functions with `@keywords internal` aren't listed in the package index, but can be accessed with `pkg:::fun()` (like the `test_logger()` function in `shinyrPkgs`).
...
-   Git is set up and configured (similar to a new user) with user credentials (`<username>` and `<username>@pm.me`), but also includes:
    ...
    -   adding remote origin to point to the GitHub repository where the `shinyrPkgs` package resides (`remote add origin`).
...
-   The changes are added, committed, and then force-pushed to the `gh-pages` branch:
    -   The content is updated to reflect the latest version of the website associated the `shinyrPkgs` R package  
...
This automated deployment process is essential for maintaining up-to-date documentation or website content for R packages (like `shinyrPkgs`) without manual intervention, making it easier for developers to focus on development while ensuring that users always have access to the latest information.

View the package website [here.](https://mjfrigaard.github.io/shinyrPkgs/)
