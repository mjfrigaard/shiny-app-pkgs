...
[**Re**-creating a toy app-package]{style="font-size: 1.25em; font-style: italic; color: #02577A;"}
...
This chapter is modeled on the [Whole Game](https://r-pkgs.org/whole-game.html) chapter in [R Packages, 2ed](https://r-pkgs.org/whole-game.html). We'll go through the development of a small shiny app-package.[^demo-monthApp] 
...
The `monthAppPkg` example we're going to be developing has been adapted from [Mastering Shiny.](https://mastering-shiny.org/scaling-packaging.html)[^whole-game-month-app]  Rather than duplicating how the `devtools`/`usethis` functions are used while writing R packages, I'm going to focus on the areas that differ from Shiny application development (each section is covered in more detail in the following chapters). The final result will be a Shiny application with all the `devtools` features and functionality. 
