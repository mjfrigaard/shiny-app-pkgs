...
This chapter resembles the [Whole Game](https://r-pkgs.org/whole-game.html) chapter in R Packages, 2ed. We'll go through the development of a small shiny app-package.[^demo-monthApp] Rather than duplicating how the `devtools`/`usethis` functions are used while writing R packages, I'm going to focus on the areas that differ from shiny application development (each section is covered in more detail in the following chapters). The final result is a shiny application with the necessary components of a functioning R package.

[^demo-monthApp]: The [example app](https://github.com/hadley/monthApp) comes from the [Packages chapter of Mastering Shiny](https://mastering-shiny.org/scaling-packaging.html)[^whole-game-month-app]
