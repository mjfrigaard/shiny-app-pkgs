...
This chapter resembles the [Whole Game](https://r-pkgs.org/whole-game.html) chapter in R Packages, 2ed. We'll go through the development of a small [app-package](https://github.com/hadley/monthApp)[^whole-game-month-app]. Rather than merely duplicating how to use the `devtools`/`usethis` functions and package development workflow, I'll focus on areas that differ from shiny application development. Each section is covered in more detail in the following chapters. The final result is a shiny application with the necessary components of a functioning R package.


[^whole-game-month-app]: The application we're going to be developing comes from the [packages chapter of Mastering Shiny](https://mastering-shiny.org/scaling-packaging.html). 

## Toy app-package: monthAppPkg

I've stored the code for this application in the `monthAppPkg` to avoid confusing it with the *actual* application repo for this chapter. 

## [`use_description()`]{style='font-size: 1.05em;'}

Every package needs a `DESCRIPTION`--calling `devtools::load_all()` without a `DESCRIPTION` will result in an error. The quickest way to create a description file is with `usethis::use_description()`.[^whole-game-use-desc] I've provided values to the `fields` list so this file isn't generated with the boilerplate contents.
...
The [documentation](https://usethis.r-lib.org/reference/use_description.html) for `use_description()` mentions putting this in your `.Rprofile` if you develop a lot of R packages (or app-packages!) 
...
We're advised again to use explicit namespacing (i.e., `pkg::fun()`), but we can avoid this by importing Shiny's functions into our package namespace. 
