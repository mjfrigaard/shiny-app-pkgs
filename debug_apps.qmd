...
For example, `launch_app()` wraps `shinyApp()`, which uses `movies_ui()` and `movies_server()`:
...
`movies_ui()` and `movies_server()` internally call their respective UI and server module functions:
