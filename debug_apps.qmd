...
co_box(
  color = "b", 
  look = "default", hsize = "1.10", size = "0.90",
  header = "![](images/bug-icon.png){width='8%'}TLDR: Debugging apps",
  fold = TRUE,
  contents = "
  
- Combining `reactiveValuesToList()` and `lobstr::tree()` with `verbatimTextOutput()` and `renderPrint()` lets us effectively debug and inspect a module’s reactive inputs.
