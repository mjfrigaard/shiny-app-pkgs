...
mod_scatter_display_server <- function(id, var_inputs, aes_inputs) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    inputs <- reactive({
      plot_title <- tools::toTitleCase(aes_inputs()$plot_title) # <1>
        list(
          x = var_inputs()$x, # <2>
          y = var_inputs()$y,
          z = var_inputs()$z, # <2>
...
```
1. Convert title to character  
2. Variable reactive inputs  
3. Aesthitic reactive inputs 
