...
This chapter covers using `reactiveValues()` and `session$userData` in your application to store values, data, or functions. Determining whether to use `reactiveValues()` or `session$userData` will depend on the purpose you want them to serve different in your application.
...
Below is a demonstration of using a `reactiveValues()` object to capture values passed from the `var_input` module to the `scatter_display` module in `movies_app()`:

### App server

This branch has an alternative version of `movies_server()` with the following changes: 
...
-   Both module server functions have `.dev` arguments set to `FALSE`

The steps below will show the structure of `rVals` and reactives in the application.
...
When the application launches, you should see the following: 
...
`mod_var_input_server()` has been simplified to return the output from `reactiveValuesToList()`. The same output is also being rendered in the sidebar when `.dev` is set to `TRUE`:[^reactives-debugging-ref]
...
[**Notice `rVals` has `:Classes 'ReactiveValues', 'R6'`**]{style="font-size: 1.10em;"}
...
Now remove the comments from the `renderPrint()` lines in `movies_server()` to view the structure of `rVals$inputs()`, then load and run the application. The new `movies_server()` function should now look like this:
...
The `rVals$inputs()` being rendered in `movies_server()` are the returned values from the variable input module.

When `rVals` is passed to `mod_scatter_display_server()`, the reactive `inputs()` object is built using the `rVals$inputs()` list:
...
An important thing to note is that when we change an input in the UI, **all** of the values change (i.e., in `rVals$inputs()` and in the `inputs()` object inside the display module).

You can also view these outputs using `movies_app(run = 'b', bslib = TRUE)`.

## `session$userData`

Unlike `reactiveValues()`, `session$userData` is not inherently reactive. This makes it ideal for storing objects that don't require or trigger reactivity. 

### Non-reactive object 

Assume I have a non-reactive object that I want to pass inside the server (and modules), but don't need it to update or change, we can store these in `session$userData`.  

The example below is a `make_dev_ggp2_movies()` function that creates a version of `ggplot2movies::movies`:
...
This function is designed to take a path or URL (i.e., a `con`nection) as an input and return a dataset that can be used in the `dev/` application. 

### App server

In the `inst/dev/app.R` file, the following changes have been made to `devServer()`:

  -   `session$userData` stores the contents of `make_dev_ggp2_movies()` in `data_fun`  
...
We're using the `mod_var_input_server()` from the `R/` folder, so we know it's displaying the contents from `reactiveValuesToList()` in the sidebar.
...
The differences in structure explain why `reactiveValues()` creates objects to control an application's reactive 'state', which can change and will trigger reactive updates in the UI. 

On the other hand, `session$userData` can store objects that should persist across different reactive contexts, but don't need reactive updating (and won't trigger reactivity).
...
`dev_mod_scatter_server()` includes arguments for `rVals`, `data_fun`, `con`, and `.dev`.
  ...
  -   `data_fun` is `session$userData$make_dev_ggp2_movies` (in this case, it's a URL for a .csv version of `ggplot2movies::movies`)

  -   `con` is the path or URL to the `data_fun` in `session$userData`
...
The structure of `data_fun` from `session$userData$make_dev_ggp2_movies` will be printed to the UI if the `.dev` argument is set to `TRUE`
...
co_box(
  ...
  contents = "
#### **`reactiveValues()` vs. `session$userData`**
