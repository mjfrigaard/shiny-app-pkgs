...
``` sh
✔ Setting active project to '/projects/apps/shinyAppPkgs'
✔ Creating '.github/'
...
After saving these changes to [`.github/workflows/style.yaml`](https://github.com/mjfrigaard/shinyAppPkgs/blob/16.1_gha-style/.github/workflows/style.yaml), we'll add, commit, and push the changes to GitHub, 
...
To https://github.com/mjfrigaard/shinyAppPkgs.git
...
From https://github.com/mjfrigaard/shinyAppPkgs
...
We need to make sure our `rsconnect` secrets are stored with the GitHub repository (so we can access them from within the workflow file). To do this, we're going to return to the **Settings** tab on the `shinyAppPkgs` repository, expand **Secrets and variables** under **Security**, and select **Actions**:[^repo-secrets]
...
![**Secrets and variables** for `shinyAppPkgs` repository](images/gh_action_secrets_repo.png){width='80%' fig-align='center'}
...
![**Repository secrets** in `shinyAppPkgs`](images/gh_action_all_repo_secrets.png){width='95%' fig-align='center'}
...
We can include the 'Shiny App Deployment' workflow file in `shinyAppPkgs` with `usethis::use_github_action("shiny-deploy")`:[^shiny-depoy]
...
This example workflow file is designed to deploy a Shiny application to a server. We're going to make a few changes to `shiny-deploy.yaml` so it will deploy the application stored in the [`16.2_gha-shiny`](https://github.com/mjfrigaard/shinyAppPkgs/tree/16.2_gha-shiny) branch.
...
Making sure my application will deploy locally with the code I plan on putting in a workflow file makes me confident it run when it's triggered. Below we'll adapt the `.github/workflows/shiny-deploy.yaml` file to deploy `shinyAppPkgs`. 
...
We can see the workflow running on the **Actions** tab of the `shinyAppPkgs` repository: 
...
## Shiny & Docker workflow {#sec-gha-shiny-docker}
...
The `deploy.R` script should be placed in the root folder of `shinyAppPkgs`. By default, this will deploy the application launched with `app.R`. We'll change this to the application launched with `ggp2_movies_app()` by changing the contents of app.R to the following: 
...
withr::with_options(new = list(shiny.autoload.r = FALSE), code = {
  if (!interactive()) {
    ...
    pkgload::load_all()
    shinyAppPkgs::ggp2_movies_app(options = list(test.mode = FALSE))
  }
})
```
...
After installing the dependencies, the `shinyAppPkgs.tar.gz` is installed from `*source*`:
...
- The workflows in this chapter are only triggered by changes to the `16.1_gha-style`, `16.2_gha-shiny`, and `16.3_gha-docker` branches of the `shinyAppPkgs` repository.
