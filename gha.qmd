...
### A `style` workflow {#sec-gha-style}
...
After saving these changes to [`.github/workflows/style.yaml`](https://github.com/mjfrigaard/moviesApp/blob/16.1_gha-style/.github/workflows/style.yaml), we'll add, commit, and push the changes to GitHub, 
...
## A Shiny workflow {#sec-gha-shiny}
...
This example workflow file is designed to deploy a Shiny application to a server. We're going to make a few changes to `shiny-deploy.yaml` so it will deploy the application stored in the [`16.2_gha-shiny`](https://github.com/mjfrigaard/moviesApp/tree/16.2_gha-shiny) branch.
...
## A Shiny/Docker workflow {#sec-gha-shiny-docker}
...
The `deploy.R` script should be placed in the root folder of `moviesApp`. By default, this will deploy the application launched with `app.R`. We'll change this to the application launched with `ggp2_movies_app()` by changing the contents of app.R to the following: 
...
After the dependencies, the `moviesApp.tar.gz` is installed from `*source*`:
...
co_box(
  ...
  fold = FALSE,
  contents = "
In the Shiny workflow example above we captured our application dependencies the `renv.lock` file and `renv/` folder, but when we used Docker *and* Shiny with Github Actions, we didn't activate `renv`. 
  
We get away with this because when the workflow runs, we can see `renv` is still used to capture our dependencies (line 13 in the output below), we see a message about the packages listed in the `Suggests` field or our `DESCRIPTION`:  
  
![Docker dependencies](images/gh_docker_dependencies.png){width='100%' fig-align='center'}
  
Not having these packages installed doesn't interfere with the application deploying because the packages listed in `Suggests` are used in`tests/`.
...
- The workflows in this chapter are only triggered by changes to the `16.1_gha-style`, `16.2_gha-shiny`, and `16.3_gha-docker` branches of the `moviesApp` repository.
