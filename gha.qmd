...
GitHub Actions are a powerful tool for automating workflows, such as testing, building, and deploying Shiny apps. Workflows (YAML files) are executed on GitHub's servers and be configured to respond to specific events (e.g., pushing code, creating a pull request, etc.).

[**Continuous Integration & Deployment (CI/CD)**]{style="font-size: 1.30em;"}
...
co_box(
  color = "b",
  look = "default", hsize = "1.10", size = "1.05",
  header = "CI/CD & Shiny App-Packages",
  fold = TRUE,
  contents = " 
...
flowchart TD
  workflow(["Workflow YAML file"]) -->|"<em>Triggered by<em>"| event("Pull request, commit, etc.")
...
1. [An event is any activity that triggers a workflow, such as a pull request, a new issue or pushed commits.]{style="font-size: 0.95rem;"}[^triggers]
...
2. [If `name` is not provided, GitHub will use the file path.]{style="font-size: 0.95rem;"}
...
flowchart TD
  ...
  event --> |"<em>Executes</em>"| Jobs{"Jobs"}
```
...
3. [The [identifier](https://docs.github.com/en/actions/using-jobs/using-jobs-in-a-workflow#setting-an-id-for-a-job) (`check` in this case) is used to reference the runner, in this example it's `ubuntu-latest`.]{style="font-size: 0.95rem;"}
...
flowchart
  ...
  subgraph VM["Virtual Machines"]
    subgraph runner02[Runner 2]

    end
    subgraph runner01[Runner 1]

    end
  end

  event --> |"<em>Executes</em>"| Jobs{"Jobs"}
  Jobs --> runner01 & runner02
```
...
Each **step** can be a shell script or an [**action**](https://docs.github.com/en/actions/about-github-actions/understanding-github-actions#actions), and are executed in the order they are written. For example, building an R package, running tests, or launching apps. 
...
6. [`steps` define a series of [tasks or actions](https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#jobsjob_idsteps) that the `job` will execute. These `steps` check out the code, set up the R environment, and install a local R package.]{style="font-size: 0.95rem;"}
...
1.   [Triggered event **`on`** a `push` to the `21.2_gha-shiny` branch]{style="font-size: 0.90rem;"}    
2.   [Change **`name`** to `shiny`]{style="font-size: 0.90rem;"}
...
The job will set up the runner and use our stored `GITHUB_TOKEN`.
...
The first four steps are **actions** (the path to the action is passed to `uses`, so `actions/checkout@v4` uses the YAML file from the [actions/checkout repo](https://github.com/actions/checkout))
...
[*`use-public-rspm` is the public RStudio package manager and `r-version: renv` indicates the R version is captured with `renv`.*]{style="font-size: 0.90rem;"}
...
The following step is a script (i.e., `run:`) and it installs `rsconnect`:
...
8.   [`Install rsconnect` uses R to install the `rsconnect`]{style="font-size: 0.90rem;"}       

:::

Next we [map](https://docs.github.com/en/actions/writing-workflows/workflow-syntax-for-github-actions#env) our account username (`mjfrigaard`) and app name (`shinyDeployApp`):
...
9.   [Sets up the environment variables `APPNAME`, `ACCOUNT`, and `SERVER` for authorization]{style="font-size: 0.90rem;"}      
...
flowchart TD
  ...
  Push -->|"<em>Triggers</em>"| Shiny[["<strong>Job</strong>: <code>shiny</code>"]]
  subgraph ubuntu["<strong>Runner</strong>"]
        Checkout("<strong>Action</strong>: Checkout repo")
        SetupPandoc("<strong>Action</strong>: Setup Pandoc")
        SetupR("<strong>Action</strong>: Set up R env")
        SetupRenv("<strong>Action</strong>: Set up (<code>renv</code>)")
        InstallRsconnect("<strong>Script</strong>: Install <code>rsconnect</code>")
        Deploy("<strong>Script</strong>: Authorize and Deploy App")
    end
  Shiny -->|"<em>Runs on: Ubuntu Latest</em>"| ubuntu
  Checkout --> SetupPandoc --> SetupR
  SetupR --> SetupRenv --> InstallRsconnect
  InstallRsconnect --> Deploy
```
...
- The workflows in this chapter are only triggered by changes to the `21.1_gha-style`, `21.2_gha-shiny`, and `16.3_gha-docker` branches of the `sap` repository.
