...
You'll rarely want to export everything, but it might be helpful if the primary audience for your app-package is other developers within your organization. See the **Low-key `@export`s with `@keywords internal`** box below for exporting functions without including them in your package index.
...
#### [`Depends`]{style="font-size: 1.05em"}
...
#### [`Imports`]{style="font-size: 1.05em"} 
...
#### [`Suggests`]{style="font-size: 1.05em"}
...
Function-level access is managed using namespace-qualified (or explicit namespacing). The `NAMESPACE` includes add-on packages or functions with an `@import` and `@importFrom` tag.[^imports-rpkgs-importFrom-import]

#### [`pkg::fun()`]{style="font-size: 1.05em"}

Refer to add-on package functions using `pkg::fun()` syntax in the code below `R/`.  

#### [`@importFrom`]{style="font-size: 1.05em"}
...
- [x] Include the add-on package to the `Imports` field with `usethis::use_package()`   

- [x] Refer to add-on functions using explicit namespacing (i.e., `pkg::fun()`) in the code beneath `R/`  

We have some special considerations for the imports in `moviesApp`:

  - [x] A substantial portion of the code in `moviesApp` comes from `shiny`, so we'll remove the explicit namespacing and place the `@import` tag in `R/movies_app.R`[^ms-imports]
...
- [x] `.data` can't be exported using `::`, so we'll include `@importFrom` for the use of `.data` in `R/scatter_plot.R`  
...
-   Managing the `NAMESPACE` ensures your app-package works when it's installed and loaded on another machine   
<br>
    -   R reads the package namespace to find the functions and objects it uses[^ns-lookup]
...
- Prefer `@importFrom` over `@import`[^r-ext-imports] [^r-pkgs-imports]
...
In order for app-package to work, users needs to have access to any of the add-on packages that are called in the code below `R/`. Knowing [when, why, how and what happens]{style="font-weight: bold;"} to imports is important because it helps you decide how to fit these habits into your package development workflow. 
