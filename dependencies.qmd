...
The error above is telling us that despite having documentation for `launch_app()` in the `R/` folder *and* generating the corresponding `.Rd` file in `man/`, `launch_app()` isn't being **exported** from `moviesApp`.

The exact cause of the error above becomes more apparent when we try to explicitly namespace `launch_app()` from `moviesApp`:
...
#| eval: false
#| code-fold: false
Error: 'launch_app' is not an exported object from 'namespace:moviesApp'
```
...
Let's start by exporting the `launch_app()` function from `moviesApp` by placing the `@export` tag above the function in `R/launch_app.R`:
...
The `NAMESPACE` file now contains a single export (`launch_app`), and when we enter `moviesApp::` in the **Console**, we see the `launch_app()` function help file in the tab completion.
...
![`moviesApp` namespace](images/dep_exported_launch_app.png){#fig-03_dependencies_namespace width=80%}

The `launch_app()` is now part of the `moviesApp` namespace
:::
...
But if a user were to click on the **Index** for `moviesApp` (at the bottom of the help file) `scatter_plot` would not be listed:
...
We'll pause here to notice a few things about what `@export` does. After loading, documenting, and installing `moviesApp`, the `NAMESPACE` is updated with `export(launch_app)`, and the **Console** automatically calls `library(moviesApp)`.

`ls()` returns "*the names of the objects in the specified environment*, so we can use it to confirm the `launch_app()` function is the only export from `moviesApp`:
...
What about the functions from add-on/third party packages `launch_app()` relies on, like `ggplot2`?
...
![`moviesApp::scatter_plot()`](images/dep_scatter_plot_namespace.png){#fig-deps_scatter_plot_exported width='95%' fig-align='center'}
...
A substantial portion of the code in `moviesApp` comes from `shiny`, so we'll remove the explicit namespacing and place the `@import` tag in `R/launch_app.R`[^ms-imports]
...
%%| fig-width: 6.5
%%| fig-align: center
%%| fig-cap: "`devtools::document` does not change the `DESCRIPTION`"
...
flowchart TD
  ...
  X3 --> note[<code>roxygen2</code> doesn't change the <code>DESCRIPTION</code> file]
  ...
  style note fill:#FFDD67,stroke:none,rx:10,ry:10;
```
...
Let's confirm we're still only exporting `launch_app()` and `scatter_plot()` from `moviesApp`:
