...
We've already confirmed that `ggplot2` isn't included in the `search()` list (and hence, are not attached with `pkgApp`)
...
My advice on exports is to [balance simplicity and utility when deciding on exported functions.]{style="font-size: 1.05em; font-weight: bold; color: #FF5200;"}.
...
I’ve also [emphasized]{style="font-weight: bold; color: #FF5200;"} the parts I’ve found worth committing to memory.
...
Below is a collection of relevant information for each method you're likely to come across when searching for advice on this topic: 
...
> "*Remember `usethis::use_package()` is helpful for [adding dependencies to `DESCRIPTION`]{style="color: #FF5200;"}*" - [Dependencies: In Practice](https://r-pkgs.org/dependencies-in-practice.html#fn2)
...
> "*A handy function [for your interactive workflow]{style="color: #FF5200;"} is `usethis::use_import_from()`*" - [In code below R/](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports-r-code)
...
Knowing [when and why]{style="font-weight: bold;"} to use each method above is just as important as knowing [how and where]{style="font-weight: bold;"} to import add-on packages and functions. I've tried to distill these decisions into actionable steps below:
...
> "*If a package [only needs a few objects from another package]{style="color: #FF5200;"} it can use a fully qualified variable reference (`foo::f`) in the code instead of a formal import.*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

This is the case for the two exported functions from `pkgApp`  (i.e., `ggplot2`, `rlang`). Another reason to use the `pkg::fun()` syntax is if there is a potential name conflict:

> *Using the `foo::f` form will be necessary when a package needs to use a function of the [same name from more than one namespace.]{style="color: #FF5200;"}*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)
...
> "*...the `package::function()` calling style is also our default recommendation for how to use your dependencies in the code below R/, [because it eliminates all ambiguity.]{style="color: #FF5200;"}*" - [R packages](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace-motivation)
...
> "*The `Imports` field lists packages whose namespaces are imported from (as specified in the `NAMESPACE` file) but which do not need to be attached. [Namespaces accessed by the `::` and `:::` operators must be listed here.]{style="color: #FF5200;"}*" - [Package Dependencies](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Package-Dependencies)
...
> "*It is common for a package to be listed in `Imports` in `DESCRIPTION`, but not in `NAMESPACE`. The converse is not true. [Every package mentioned in `NAMESPACE` must also be present in the `Imports` or `Depends` fields.]{style="color: #FF5200;"}*" - [Confusion about Imports](https://r-pkgs.org/dependencies-in-practice.html#confusion-about-imports)

On when to use `Imports` vs. `Depends`:

> *Unless there is a good reason otherwise, [you should always list packages in `Imports` not `Depends`]{style="color: #FF5200;"}.*" - [Whether to Import or Depend](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace)

### [`@import/@importFrom` (`NAMESPACE`)]{style="font-size: 0.90em;"}

The official R documentation states 'variables exported from other packages' need to be imported: 

> "*Variables exported from [other packages with namespaces need to be imported explicitly using the directives `Imports` and `importFrom`]{style="color: #FF5200;"}*" [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

It also recommends using `@importFrom` over `@import` in most cases:

> "*Using `importFrom` selectively rather than `Imports` is good practice and recommended notably [when importing from packages with more than a dozen exports and especially from those written by others (so what they export can change in future)]{style="color: #FF5200;"}.*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 

'Formal imports' are also referred to as more efficient than using `pkg::fun()` in the following section (a 'formal import' means the add-on package function uses `import()` or `importFrom()` in the `NAMESPACE`): 

> "*[`foo::f`] is slightly [less efficient than a formal import]{style="color: #FF5200;"} and also loses the advantage of recording all dependencies in the `NAMESPACE` file"* - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 


[R Packages, 2ed](https://r-pkgs.org/), recommends minimizing the use of both `@import` and `@importFrom` (*`aaapkg` represents a hypothetical package listed in the `Imports` field of the `DESCRIPTION` file*)

> "*Specifically, we recommend that you [default to not importing anything from `aaapkg` into your namespace]{style="color: #FF5200;"}. This makes it very easy to identify which functions live outside of your package, which is especially useful when you read your code in the future. This also eliminates any concerns about name conflicts between `aaapkg` and your package.*" [Package is listed in Imports](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports)

However, [R Packages, 2ed](https://r-pkgs.org/) also notes the following exceptions to this rule:

> "*Sometimes you make such [heavy use of so many functions from another package that you want to import its entire namespace]{style="color: #FF5200;"}.*" 
> 
> "*You can’t call an [operator from another package]{style="color: #FF5200;"} via ::, so you must import it.*" - [Package is listed in Imports](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports)

If you happen to read the advice from the footnote, you'll also find somewhat conflicting advice: 

> "*[Always use a `NAMESPACE` to specify imports]{style="color: #FF5200;"} so that your package code isn't harmed by other peoples' use of `Depends`.*" - [Whether to Import or Depend: footnote reference ](https://r-pkgs.org/dependencies-mindset-background.html#fn6)
:::
...
I've found the best guidance on when and why to use `pkg::fun()`, `@importFrom`, and the `Imports` field in the `DESCRIPTION` comes from the `roxygen2` documentation:
...
We should also include `@importFrom` for the use of `.data` in `R/scatter_plot.R`:
...
Finally, we need to list each add-on package to the `Imports` field in the `DESCRIPTION` using `usethis::use_package()`:
...
What happens to imported packages and functions can get a bit technical, but I've done my best to include what I consider to be the necessary distinctions between using explicit namespacing (`pkg::fun()`), `@importFrom`, `@import`, and the `Imports` field in the `DESCRIPTION.` I strongly encourage reading the [namespaces section of Advanced R, 2ed](https://adv-r.hadley.nz/environments.html#namespaces) for a deeper understanding of these topics.
...
> "*Evaluating `foo::f` will cause package `foo` to be [loaded, but not attached]{style="color: #FF5200;"}, if it was not loaded already*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 
...
> ```
> 
> *The code inside your package can [assume that aaapkg is installed whenever pkg is installed]{style="color: #FF5200;"}.*
...
> "*The main difference is that a package you list in `Imports` will [just be loaded when you use it]{style="color: #FF5200;"}, whereas a package you list in `Depends` [will be attached when your package is attached]{style="color: #FF5200;"}.* - [Whether to Import or Depend](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace)
...
We've listed five packages in the `Imports` field of the `DESCRIPTION` file--are these packages on the search list?
...
And we can still access add-on package functions using the `pkg::fun()` syntax: 
