...
flowchart TD
    ...
    subgraph RepEnv["<strong>Reproducible Env</strong>"]
    ...
    style Dir fill:#f9f9f9,stroke:#999,stroke-width:2px,rx:3,ry:3
    style RepEnv fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:3,ry:3
    style Init fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Snap fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style Lock fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Lib fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
```
...
flowchart TD
    ...
    style RPkg fill:#f9f9f9,stroke:#999,stroke-width:1px,rx:3,ry:3
    style SapPkg fill:#e8f4f1,stroke:#4CBB9D,stroke-width:1px,rx:3,ry:3
    style Users fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
    style fun fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style sap fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
```
...
A great place to start is our `app.R` file. See the [current app.R](https://github.com/mjfrigaard/sap/blob/06.1_pkg-exports/app.R) for reference. Ideally, we'll want to replace library() calls once we ensure the required functions are available in `sap` with proper namespace declarations.
...
flowchart
  ...
  style R fill:#f9f9f9,stroke:#999,stroke-width:2px
  style NS fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
  style Man fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
  style Tag fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style Document fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
  style Load fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px
  style Exported fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style RdFile fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
...
We can use the add-on/third-party package functions `sap` relies on by explicitly namespacing them (i.e., `pkg::fun()`). See [GitHub](https://github.com/mjfrigaard/sap/blob/06.1_pkg-exports/R/scatter_plot.R) for examples of namespaced function calls in `scatter_plot()`.
...
#' @param x_var character variable for x-axis
#' @param y_var character variable for y-axis
#' @param col_var character variable for color aesthetic
#' @param alpha_var numeric transparency (0-1)
#' @param size_var numeric point size
#'
#' @return A ggplot object
#' @examples
#' scatter_plot(df = mtcars, x_var = "mpg", y_var = "hp",
#'   col_var = "cyl", alpha_var = 0.7, size_var = 3)
#' @export
```
...
Functions included via `pkg::fun()` are loaded in the namespace. Non-exported functions remain accessible via `pkg:::fun()`. See [R Packages, 2ed](https://r-pkgs.org/dependencies-in-practice.html#what-to-export) for guidance.
...
flowchart TD
  ...
  style DESCRIPTION fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
  style R fill:#f9f9f9,stroke:#999,stroke-width:2px
  style NAMESPACE fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
  style UsePkg fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style Imports fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style Tag fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style Import fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style ImportFrom fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style NSImport fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style NSImportFrom fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style Document fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px

```

**NOTE**: `devtools::document()` updates the `NAMESPACE` with any `@import`, `@importFrom` or `@export` tags. However, no changes are made to the `DESCRIPTION` file.
:::
