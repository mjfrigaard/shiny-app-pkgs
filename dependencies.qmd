...
The first step in managing dependencies is identifying which add-on packages `pkgApp` relies on.[^depends-pkg-ns] Our goal is to limit the dependencies to only those critical to the functioning of our app, because each additional dependency is a potential point of failure (should this package become unavailable or significantly change).

[^depends-pkg-ns]: I've made this process somewhat easier by explicitly namespacing all of the add-on package functions in `pkgApp` (i.e., with `pkg::fun()`).
...
-   Listing packages in `Imports` ensures they will be installed when `pkgApp` is installed (i.e. with `install.packages("pkgApp")`).   
<br>
    -   Users of `pkgApp` won't automatically have access to the packages listed under `Imports` (unless they're already installed).
...
<br>
    -   [You'll *almost* always list dependencies under `Imports` because these are the packages needed to use `pkgApp`]{style="font-weight: bold; color: #043b67;"}.[^desc-suggests]

#### What 

-   `library(pkgApp)` loads the namespace of imported packages, but they are not attached to the `search()` path.[^desc-add-on-access] 


:::

[^desc-suggests]: Packages used for developing or testing `pkgApp` can be included under `Suggests`, because these are **not** installed along with `pkgApp`. Packages in `Depends` will be installed and attached to the search list when `pkgApp` is installed.
...
-   Using `pkg::fun()` makes calls to add-on packages explicit and easy to differentiate from the native functions developed in `pkgApp`   
<br>
...
-   Managing the `NAMESPACE` ensures pkgApp works when it's installed and loaded on another machine (regardless of other installed and attached packages)   
<br>
...
[^use-import-from]: You can also consolidate all `@import` and `@importFrom` tags into a single package doc file (i.e., `R/[pkgApp]-package.R`) by calling `usethis::use_package_doc()`.

In order for `pkgApp` to work, users needs to have access to any of the add-on packages that are called in the code below `pkgApp/R/`. Knowing [Why, when, how and what happens]{style="font-weight: bold;"} to imports is important because it helps you decide how to fit these habits into your package development workflow. 

## [Imports]{style="font-weight: bold;"} workflow in [`pkgApp`]{style="font-size: 1.05em;"}
...
We're already using `pkg::fun()` in most of the functions in `pkgApp`, so we need to remove them from shiny functions (and add `@import`).
...
After adding all add-on packages to the `DESCRIPTION` with `usethis::use_package()`, then deciding if/where to use `@importFrom` and `@import`, we'll load, document, and install `pkgApp`:
