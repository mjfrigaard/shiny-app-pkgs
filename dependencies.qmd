...
We will use the [`05_roxygen2`](https://github.com/mjfrigaard/moviesApp/tree/05_roxygen2)  branch of  `moviesApp` from the end of the previous chapter. Below is a folder tree of its contents:
...
The error tells me that despite having documentation for `movies_app()` in `R/` *and* generating the corresponding `.Rd` file in `man/`, the `movies_app()` function isn't being **exported** from `moviesApp`.
...
![`moviesApp` namespace](img/05_dependencies_tab_compl_movies_app.png){#fig-03_dependencies_namespace width=60%}
...
![`moviesApp::scatter_plot()`](img/05_dependencies_tab_compl_scatter_plot.png){#fig-03_dependencies_tab_compl_movies_app width='55%' fig-align='center'}
