...
I've made this process somewhat easier by explicitly namespacing all of the add-on package functions in `pkgApp` (i.e., with `package::function()`). You'll learn more about explicit namespacing in the following sections. 
...
If we want to use functions from the packages `pkgApp` relies on, we need to explicitly namespace these functions from their original package namespaces (i.e., `package::function()`): 
...
co_box(
  ...
  contents = "When a user loads `pkgApp` with `library(pkgApp)`, any add-on package functions used in an export is available to users if they use `package::function()` (or if they load the package themselves with `library()`).", 
  fold = FALSE
)
```
...
1. Use a 'fully qualified variable reference' (i.e., the `package::function()` syntax) (*we're already using this method with `movies_app()` and `scatter_plot()`*).

2. List the package in the `Import` field of the `DESCRIPTION` file
...
### [`package::function()`]{style="font-size: 0.90em;"}
...
[R Packages, 2ed](https://r-pkgs.org/) recommends using '`package::function()`' for all calls to add-on package functions:
...
Using '`package::function()`' can seem tedious at first, but fortunately we have tools like tab-completion (if you're using the Posit Workbench IDE) and packages like [`sinew`](https://yonicd.github.io/sinew/) (which we'll cover in a later chapter)

### [`Import` (`DESCRIPTION`)]{style="font-size: 0.90em;"}
...
[R Packages, 2ed](https://r-pkgs.org/) recommends defaulting to `package::function()` for calls to any add-on packages in the `R/` folder because it makes these calls more explicit:
...
>
> *"If the repetition of the package name becomes annoying you can `@importFrom` and drop the [`package::function()`]"* - [Importing functions](https://roxygen2.r-lib.org/articles/namespace.html#functions)
...
#| eval: false 
#| code-fold: true 
#| code-summary: 'show/hide output from usethis::use_package()'
...
![Updated `NAMESPACE` with `import` and `importFrom](img/03_dependencies_namespace_import_importFrom.png){#fig-03_dependencies_namespace_import_shiny width='50%' align='center'}
