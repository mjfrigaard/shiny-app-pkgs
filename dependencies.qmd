...
%%| fig-width: 6.5
%%| fig-align: center
%%| fig-cap: 'The `NAMESPACE` and `DESCRIPTION` handle package dependencies'
%%{init: {'theme': 'base', 'themeVariables': { 'fontFamily': 'monospace'}}}%%
flowchart TD
    sap["We use <code>fun()</code> in <code>foo()</code>"]
    Pkg("R Package <code>pkg</code> has function <code>fun()</code>")
    Users("Users install/load <code>sap</code> to use <code>foo()</code>")

    Pkg -->|import <code>pkg</code>| sap
    sap -->|export <code>foo</code>| Users
    
    style sap fill:#89D6FB,color:#000000,stroke:none,rx:10,ry:10;
    style Pkg fill:#8dd38d,color:#000000,stroke:none,rx:10,ry:10;
    style Users fill:#8dd38d,color:#000000,stroke:none,rx:10,ry:10;

```


Multiple packages can help you manage the dependencies in your app-package, and these are covered in the [Dependency hell](entanglement.qmd) chapter. This chapter will pick up where we left off with the [`05_roxygen2`](document.qmd)  branch of  `sap`.
...
The first step in managing dependencies is identifying which add-on packages `sap` relies on.[^depends-pkg-ns] Our goal is to limit the dependencies to only those critical to the functioning of our app, because each additional dependency is a potential point of failure (should this package become unavailable or significantly change).
...
![`sap` namespace](images/dep_exported_launch_app.png){#fig-03_dependencies_namespace width=100%}
...
flowchart
  Tag("Tag function 
	  with <code>@export</code>")
  LoadAll("Run <code>load_all()</code>")
  Document("Run <code>document()</code>")
  Exported("Function exported 
            to <code>NAMESPACE</code>")
  NAMESPACE("<code>NAMESPACE</code> 
            updated with 
            <code>export(launch_app)</code>")
  RdFile("<code>.Rd</code> files 
	     created in <code>man/</code>")
  ...
  style Tag fill:#8dd38d,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
  style LoadAll fill:#89D6FB,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
  style Document fill:#89D6FB,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
  style Exported fill:#FFB64C,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
  style NAMESPACE fill:#FFB64C,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
  style RdFile fill:#FFB64C,color:#000000,stroke:#333,stroke-width:1px,rx:10,ry:10
```
...
After loading, documenting, and installing `sap`, the `NAMESPACE` is updated with the `export()` [directive](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-NAMESPACE-file): 
...
#### [`pkg::fun()`]{style="font-size: 1.0em"} {#sec-depends-pkg-fun-syntax .unnumbered}

Refer to add-on package functions using `pkg::fun()` syntax in the code below `R/`.  

#### [`@importFrom`]{style="font-size: 1.0em"} {#sec-depends-import-from .unnumbered}
...
#### [`@import`]{style="font-size: 1.00em"} {#sec-depends-import .unnumbered}
...
flowchart TD
  ...
  style NamespaceUpdated fill:#FFB64C,stroke:none,rx:10,ry:10;
  style RdFileUpdated fill:#FFB64C,stroke:none,rx:10,ry:10;
```
...
## More information on imports {.unnumbered}

Below are handful of questions and answers I've encountered regarding package imports:

***Question: How can I include an add-on package to my `DESCRIPTION` file?***

Answer: `usethis::use_package()` automatically adds a package in the `Imports` section, and has options for specifying the minimum version.

***Question: Will users of my app-package have access to the packages listed in the `Imports` field of my `DESCRIPTION` file?***

Answer: `library(pkg)` loads the namespace of imported packages, but they are not attached to the `search()` path.[^desc-add-on-access] 
...
Answer: using `pkg::fun()` makes calls to add-on packages explicit and easy to differentiate from the native functions developed in `sap`.[^rpks-pkg-fun-syntax]
...
***Question: What does the `NAMESPACE` do when my package is installed by a user?***

Answer: managing the `NAMESPACE` ensures your app-package works when it's installed and loaded on another machine, because R will read your package namespace to find what it imports and exports.[^ns-lookup]
...
***Question: Where should I place the `@importFrom` tag in the code below R/?***

Answer: place the `@importFrom pkg fun` tag directly above the code using the add-on function. You can also consolidate all `@import` and `@importFrom` tags into a single package doc file (i.e., `R/[sap]-package.R`) by calling `usethis::use_package_doc()`.

***Question: Should I be using `@importFrom` or `@import` from?***

Answer: prefer `@importFrom` over `@import`, but try to avoid using either[^r-ext-imports] [^r-pkgs-imports].
...
Answer: Imports are described briefly in R Packages, 2ed[^r-pkgs-fun-lookup] and covered in-depth in Advanced R, 2ed.[^advr-imports] 
