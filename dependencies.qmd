...
flowchart TD
    sap["Our app-package uses <code>fun()</code>\n in function <code>foo()</code>"]
    Pkg("R Package <code>pkg</code> with \nfunction <code>fun()</code>")
    Users("Users load <code>sap</code> and \nuse <code>foo()</code>")

    Pkg -->|Imports <code>pkg</code>| sap
...
flowchart TD
  ...
  NAMESPACE("<code>NAMESPACE</code> updated with\n <code>export(launch_app)</code>")
...
flowchart TD
  Imports("List package under <code>Imports</code>\n in <code>DESCRIPTION</code>")
  Reference("Reference <code>package::function()</code>\n in code")
  LoadAll("Run <code>devtools::load_all()</code>")
  Document("Run <code>devtools::document()</code>")
...
-   Using `pkg::fun()` makes calls to add-on packages explicit and easy to differentiate from the native functions developed in `sap`[^rpks-pkg-fun-syntax]
