...
Dependencies are handled using `roxygen2` tags (to generate the `NAMESPACE`) and three fields in the `DESCRIPTION` file (`Suggests`, `Imports`, or `Depends`). Multiple packages can help you manage the dependencies in your app-package, and these are covered in the [Dependency hell](entanglement.qmd) chapter.


This chapter will pick up where we left off with the [`05_roxygen2`](document.qmd)  branch of  `shinyAppPkgs`.
...
ℹ Updating shinyAppPkgs documentation
ℹ Loading shinyAppPkgs
...
* installing to library ‘/path/to/local/install/shinyAppPkgs-090c61fc/R-4.2/x86_64-apple-darwin17.0’
...
The first step in managing dependencies is identifying which add-on packages `shinyAppPkgs` relies on.[^depends-pkg-ns] Our goal is to limit the dependencies to only those critical to the functioning of our app, because each additional dependency is a potential point of failure (should this package become unavailable or significantly change).

[^depends-pkg-ns]: I've made this process somewhat easier by explicitly namespacing all of the add-on package functions in `shinyAppPkgs` (i.e., with `pkg::fun()`).
...
Ideally, we'll want to replace these calls to `libary()`, but first we have to make sure the functions we're using in these packages will be available in `shinyAppPkgs`.
...
The error above is telling us that despite having documentation for `launch_app()` in the `R/` folder *and* generating the corresponding `.Rd` file in `man/`, `launch_app()` isn't being **exported** from `shinyAppPkgs`.

The exact cause of the error above becomes more apparent when we try to explicitly namespace `launch_app()` from `shinyAppPkgs`:
...
#| eval: false
#| code-fold: false
Error: 'launch_app' is not an exported object from 'namespace:shinyAppPkgs'
```
...
### Export [`launch_app()`]{style="font-size: 1.00em;"}

Let's start by exporting the `launch_app()` function from `shinyAppPkgs` by placing the `@export` tag above the function in `R/launch_app.R`:
...
In `app.R`, we'll replace the calls to `library()` with a single call to `library(shinyAppPkgs)`
...
The `NAMESPACE` file now contains a single export (`launch_app`), and when we enter `shinyAppPkgs::` in the **Console**, we see the `launch_app()` function help file in the tab completion.
 
:::: {.column-body-outset-right}

::: {#fig-dep_exported_namespace layout-ncol=2}

![updated `NAMESPACE`](images/dep_exported_namespace.png){#fig-dep_exported_namespace width=80%}

![`shinyAppPkgs` namespace](images/dep_exported_launch_app.png){#fig-03_dependencies_namespace width=80%}

The `launch_app()` is now part of the `shinyAppPkgs` namespace
:::
...
![](images/depends_keywords_internal_namespace.png){width='80%' fig-align='center'}
  
But if a user were to click on the **Index** for `shinyAppPkgs` (at the bottom of the help file) `scatter_plot` would not be listed:
  
![](images/depends_pkg_index.png){width='65%' fig-align='center'}
...
We'll pause here to notice a few things about what `@export` does. After loading and documenting our package, the `NAMESPACE` is updated with `export(launch_app)`, and the **Console** automatically calls `library(shinyAppPkgs)`.
...
%%| fig-width: 6.5
%%| fig-align: center
%%| fig-cap: 'The `@export` tag makes a function available to anyone using our package'
...
`library(shinyAppPkgs)` attaches `shinyAppPkgs` to the search list. We can view all the attached packages in the string returned from `search()`:

```{verbatim}
#| eval: false
#| code-fold: false
"package:shinyAppPkgs" %in% search()
```
...
We can use the add-on/third-party package functions `shinyAppPkgs` relies on, but we need to explicitly namespace these functions from their original package namespaces (i.e., using  `pkg::fun()`): 
...
co_box(
  ...
  contents = "
***What happens to add-on/third party functions that are exported from `shinyAppPkgs`?***
  
When a user loads `shinyAppPkgs` with `library(shinyAppPkgs)`, any add-on/third-party package functions used in exports are available to users if they use `pkg::fun()` (or if they load the package themselves with `library()`).", 
  fold = TRUE
)
```

Access to add-on/third-party package functions has implications for the other functions in `shinyAppPkgs`--for example, the `scatter_plot()` function uses `ggplot2` functions. But we're not exporting `scatter_plot()`, so when we attempt to run the examples, we see the following error: 
...
[Examples for 'shinyAppPkgs::scatter_plot']{style="font-weight: bold; font-size: 1.15em;"}
...
Let's add the `@export` tag to `R/scatter_plot.R` so it's exported from `shinyAppPkgs`.
...
After loading, documenting, and installing `shinyAppPkgs`, the `NAMESPACE` is updated with the `export()` [directive](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-NAMESPACE-file): 
...
The contents of the updated `NAMESPACE` file are below (and `shinyAppPkgs::` in the **Console** now displays the `scatter_plot()` help file in the tab completion):

:::: {.column-body-outset-right}

::: {#fig-deps_scatter_plot_exported layout-ncol=2}

![`@export` the `scatter_plot` function](images/dep_scatter_plot_exported.png){#fig-deps_scatter_plot_exported width='95%' fig-align='center'}

![`shinyAppPkgs::scatter_plot()`](images/dep_scatter_plot_namespace.png){#fig-deps_scatter_plot_exported width='95%' fig-align='center'}

`scatter_plot()` is now part of the `shinyAppPkgs` namespace
:::
...
We've already confirmed that `ggplot2` isn't attached with `shinyAppPkgs` (and hence, it is not included in the `search()` list)
...
co_box(
  ...
  contents = "
***What about functions that _aren't_ exported from `shinyAppPkgs`?***
...
When determining which functions to export, consider the question: "*When a user installs and loads `shinyAppPkgs`, what functions do I want to be available?*" 
...
Packages listed under `Depends` are essential for our app-package to work. These packages will be attached *before* our package when `library(shinyAppPkgs)` is called.
...
A substantial portion of the code in `shinyAppPkgs` comes from `shiny`, so we'll remove the explicit namespacing and place the `@import` tag in `R/launch_app.R`[^ms-imports]
...
``` sh
✔ Setting active project to '/Users/mjfrigaard/projects/apps/shinyAppPkgs'
...
After adding all add-on packages to the `DESCRIPTION` with `usethis::use_package()`, then deciding if/where to use `@importFrom` and `@import`, we'll load, document, and install `shinyAppPkgs`:
...
Confirming we still have full functionality in `shinyAppPkgs`
:::
...
<!--style note fill:#fa7b3c,stroke:#E74A2F,stroke-width:2px;-->
...
Let's confirm we're still only exporting `launch_app()` and `scatter_plot()` from `shinyAppPkgs`:
...
This demonstrates that none of these packages are attached with `shinyAppPkgs`. However, the `rlang` and `shiny` packages are included in the `loadedNamespaces()` (because we included them with `@import`/`@importFrom`). 
...
We can still access the add-on package functions in `shinyAppPkgs` using the `pkg::fun()` syntax: 
...
-   Using `pkg::fun()` makes calls to add-on packages explicit and easy to differentiate from the native functions developed in `shinyAppPkgs`[^rpks-pkg-fun-syntax]
...
- You can also consolidate all `@import` and `@importFrom` tags into a single package doc file (i.e., `R/[shinyAppPkgs]-package.R`) by calling `usethis::use_package_doc()`.
...
#| code-fold: false
#| eval: false
── R CMD check results ────────────────────────── shinyAppPkgs 0.0.0.9000 ────
Duration: 7.4s
