...
Lets start by exporting the `movies_app()` function from `pkgApp` by placing the `@export` tag above the function in `R/movies_app.R`:
...
![`movies_app()`](img/03_dependencies_exported_movies_app.png){width=100%}
...
![updated `NAMESPACE`](img/03_dependencies_namespace.png){#fig-03_dependencies_namespace width=40%}

![`pkgApp` namespace](img/03_dependencies_tab_compl_movies_app.png){#fig-03_dependencies_namespace width=60%}
...
We're going to pause a moment here to notice a few things about what `@export` does. After loading, documenting, and installing `pkgApp`, the `NAMESPACE` is updated with `export(movies_app)`, and the **Console** automatically calls `library(pkgApp)`.

`ls()` returns "*the names of the objects in the specified environment*, so we can use it to confirm the `movies_app()` function the only export from `pkgApp`:
...
What about the functions from add-on packages `movies_app()` relies on (like (`shiny`, `ggplot2` etc.))?

Let's check to see if two of these packages (`ggplot2` and `stringr`) also attached to the `search()` list: 

```{r}
#| eval: false
#| code-fold: false
c("package:ggplot2", "package:stringr") %in% search()
```
...
![We can use `ggplot2` if we explicitly namespace it's functions ](img/03_dependencies_using_ggplot2_with_ns.png){width=100%}
...
co_box(
  ...
  contents = "When a user loads `pkgApp` with `library(pkgApp)`, any add-on package functions used in an export is available to users if they use `pkg::fun()` (or if they load the package themselves with `library()`).", 
  fold = FALSE
)
```

This has implications for the other functions in `pkgApp`--for example, the `scatter_plot()` function uses `ggplot2` functions. But we're not exporting `scatter_plot()`, so when we attempt to run the examples, we see the following error: 

:::: {.column-body-outset-right}

::: {#fig-03_dependencies_scatter_plot_examples layout-ncol=2}

![Examples in `scatter_plot()`](img/03_dependencies_scatter_plot_examples_rd.png){#fig-03_dependencies_scatter_plot_examples width=40%}



![Error in `scatter_plot()` examples](img/03_dependencies_scatter_plot_examples_error.png){#fig-03_dependencies_scatter_plot_examples width=60%}
...
The message in the top of the **Help** pane is informative, because it tells us that despite `scatter_plot()` being functional when we run `movies_app()`, it's not part of the package namespace (and thus, not accessible to users in the help file). 
...
Lets add the `@export` tag to `R/scatter_plot.R` so it's exported from `pkgApp`.
...
The contents of the updated `NAMESPACE` file is below (and `pkgApp::` in the **Console** now displays the `scatter_plot()` help file in the tab completion):

:::: {.column-body-outset-right}

::: {#fig-03_dependencies_namespace_scatter_plot layout-ncol=2}

![`@export` the `scatter_plot` function](img/03_dependencies_namespace_scatter_plot.png){#fig-03_dependencies_namespace_scatter_plot width='45%' fig-align='center'}

![`pkgApp::scatter_plot()`](img/03_dependencies_tab_compl_scatter_plot.png){#fig-03_dependencies_tab_compl_movies_app width='55%' fig-align='center'}
...
![`scatter_plot()` examples](img/03_dependencies_scatter_plot_examples.png){#fig-03_dependencies_scatter_plot_examples width='100%' fig-align='center'}
...
We've already confirmed `ggplot2` and `rlang` aren't included in the `search()` list (and hence, are not attached with `pkgApp`)

```{r}
#| eval: false 
#| code-fold: false
pkgs <- c("package:rlang", "package:ggplot2")
pkgs %in% search()
```
...
However, we can access these functions because their packages are included in the loaded namespaces (which we can view with `loadedNamespaces()`)

```{r}
#| eval: false 
#| code-fold: false
pkgs <- c("rlang", "ggplot2")
...
-   Extending the ‘problem’ statement to the broadest possible definition would assume users will want access to all the functions in `pkgApp`--this would give them the ability to customize, extend, or integrate the contents of `pkgApp` with other tools. It's rare you'll want to export *everything* from your app-package, but it can be helpful if your primary audience is other developers within your organization.

My advice on exports is to [aim for a balance between simplicity and utility when deciding on exported functions]{style="font-size: 1.05em; font-weight: bold; color: #772953;"}.

::: {.column-margin}

![New Git Branch](img/new_branch_ico.png){width='70%'}

The code for this section is in the  [[`03_pkg-exports`](https://github.com/mjfrigaard/pkgApp/tree/03_pkg-exports)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.
...
When `pkgApp` is documented (i.e., **[<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>D</kbd>]{style='font-size: 0.70em;'}**), `roxygen2` tags are used to update the `NAMESPACE` with any imports or exports. However, the `DESCRIPTION` file must be edited independently:
...
![`roxygen2` does not connect the `NAMESPACE` to the `DESCRIPTION`](img/03_dependencies_roxygen2_no_connect_desc.png){#fig-roxygen2_connects width='65%' align='center'}

`roxygen2` & `NAMESPACE` = function-level access vs. `DESCRIPTION` = package-level dependencies
:::

The differences between the dependencies listed in the `NAMESPACE` directives and the `Imports` field in the `DESCRIPTION` file can be a [common point of confusion](https://r-pkgs.org/dependencies-in-practice.html#confusion-about-imports), which is understandable if you've consulted the [official R documentation](https://cran.r-project.org/doc/manuals/r-release/R-exts.html) (specifically the [Package Dependencies](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Package-Dependencies) and [Package namespaces](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Package-namespaces) topics). 
...
I've also added [emphasis]{style="color: #E74A2F;"} to the parts I've found are worth committing to memory.
...
1. Use a 'fully qualified variable reference' (i.e., the `pkg::fun()` syntax) (*we're already using this method with `movies_app()` and `scatter_plot()`*).

2. List the package in the `Imports` field of the `DESCRIPTION` file

3. Include an `@import` or `@importFrom` tag from `roxygen2` (this will add `import()` or `importFrom()` in the `NAMESPACE`) 
...
:::: {.callout-note collapse="false" icon=false title="How and where to import add-on package functions?"}

::: {.panel-tabset}

### [`pkg::fun()`]{style="font-size: 0.90em;"}

The official R documentation refers to the `pkg::fun()` syntax as a 'fully-qualified reference':

> "*A fully-qualified reference to the function `f` in package `foo` is of the form `foo::f`.*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

[R Packages, 2ed](https://r-pkgs.org/) recommends using '`pkg::fun()`' for all calls to add-on package functions:

> "*Our recommended default is to call external functions using the `package::function()` syntax*" - [R Packages 2ed, In code below `R/`](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports)
...
The official R documentation assumes you'll be editing the `DESCRIPTION` manually, but [R Packages, 2ed](https://r-pkgs.org/) recommends using the `usethis::use_package()` function to include add-on packages to the `Imports` field of the `DESCRIRPTION` file:

> "*Remember `usethis::use_package()` is helpful for [adding dependencies to `DESCRIPTION`]{style="color: #E74A2F;"}*" - [Dependencies: In Practice](https://r-pkgs.org/dependencies-in-practice.html#fn2)


### [`@import/@importFrom` (`NAMESPACE`)]{style="font-size: 0.90em;"}

The official R documentation assumes you'll be editing the `NAMESPACE` manually (i.e., with `import()` and `importFrom()` directives), but [R Packages, 2ed](https://r-pkgs.org/) assumes you'll be building your app-package iteratively, so the authors recommend using the `usethis::use_import_from()`:

> "*A handy function [for your interactive workflow]{style="color: #E74A2F;"} is `usethis::use_import_from()`*" - [In code below R/](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports-r-code)
...
We're already using `pkg::fun()` in the two exported functions from `pkgApp` (`movies_app()` and `scatter_plot()`), so let's cover when and why to list these packages and functions as imports.
...
Knowing [when and why]{style="font-weight: bold;"} to use each method above is just as important as knowing [how and where]{style="font-weight: bold;"} to import add-on packages and functions, and I've tried to distill this decision into actionable steps below:

:::: {.callout-note collapse="false" icon=false title="When and why to import add-on package functions?"}

::: {.panel-tabset}

### [`pkg::fun()`]{style="font-size: 0.90em;"}

If we're only using a handful of functions from an add-on package, using the `pkg::fun()` is fine:

> "*If a package [only needs a few objects from another package]{style="color: #E74A2F;"} it can use a fully qualified variable reference (`foo::f`) in the code instead of a formal import.*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

This is the case in `pkgApp` for the two exported functions (i.e., `ggplot2`, `rlang`), but we should also use the `pkg::fun()` syntax if there is a potential name conflict:

> *Using the `foo::f` form will be necessary when a package needs to use a function of the [same name from more than one namespace.]{style="color: #E74A2F;"}*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

[R Packages, 2ed](https://r-pkgs.org/) recommends defaulting to `pkg::fun()` for calls to any add-on packages in the `R/` folder because it makes these calls more explicit:

> "*...the `package::function()` calling style is also our default recommendation for how to use your dependencies in the code below R/, [because it eliminates all ambiguity.]{style="color: #E74A2F;"}*" - [R packages](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace-motivation)

### [`Imports` (`DESCRIPTION`)]{style="font-size: 0.90em;"}

The official R documentation indicates any add-on package functions using `pkg::fun()` should be listed in the `DESCRIPTION`  (specifically, the `Imports` field).

> "*The `Imports` field lists packages whose namespaces are imported from (as specified in the `NAMESPACE` file) but which do not need to be attached. [Namespaces accessed by the `::` and `:::` operators must be listed here.]{style="color: #E74A2F;"}*" - [Package Dependencies](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Package-Dependencies)
...
> "*It is common for a package to be listed in `Imports` in `DESCRIPTION`, but not in `NAMESPACE`. The converse is not true. [Every package mentioned in `NAMESPACE` must also be present in the `Imports` or `Depends` fields.]{style="color: #E74A2F;"}*" - [Confusion about Imports](https://r-pkgs.org/dependencies-in-practice.html#confusion-about-imports)

On when to use `Imports` vs. `Depends`:

> *Unless there is a good reason otherwise, [you should always list packages in `Imports` not `Depends`]{style="color: #E74A2F;"}.*" - [Whether to Import or Depend](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace)

### [`@import/@importFrom` (`NAMESPACE`)]{style="font-size: 0.90em;"}

The official R documentation states 'variables exported from other packages' need to be imported: 

> "*Variables exported from [other packages with namespaces need to be imported explicitly using the directives `Imports` and `importFrom`]{style="color: #E74A2F;"}*" [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports)

But it also recommends using `@importFrom` over `@import` in most cases:

> "*Using `importFrom` selectively rather than `Imports` is good practice and recommended notably [when importing from packages with more than a dozen exports and especially from those written by others (so what they export can change in future)]{style="color: #E74A2F;"}.*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 

'Formal imports' are also referred to more efficient than using `pkg::fun()` in the following section (a 'formal import' means the add-on package function uses `import()` or `importFrom()` in the `NAMESPACE`): 

> "*[`foo::f`] is slightly [less efficient than a formal import]{style="color: #E74A2F;"} and also loses the advantage of recording all dependencies in the `NAMESPACE` file"* - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 


[R Packages, 2ed](https://r-pkgs.org/), recommends minimizing the use of both (*`aaapkg` represents a hypothetical package listed in the `Imports` field of the `DESCRIPTION` file*)

> "*Specifically, we recommend that you [default to not importing anything from `aaapkg` into your namespace]{style="color: #E74A2F;"}. This makes it very easy to identify which functions live outside of your package, which is especially useful when you read your code in the future. This also eliminates any concerns about name conflicts between `aaapkg` and your package.*" [Package is listed in Imports](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports)

However, [R Packages, 2ed](https://r-pkgs.org/) also notes the following exceptions to this rule:

> "*Sometimes you make such [heavy use of so many functions from another package that you want to import its entire namespace]{style="color: #E74A2F;"}.*" 
> 
> "*You can’t call an [operator from another package]{style="color: #E74A2F;"} via ::, so you must import it.*" - [Package is listed in Imports](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-in-imports)

If you happen to read the advice from the footnote, you'll also find somewhat conflicting advice: 

> "*[Always use a `NAMESPACE` to specify imports]{style="color: #E74A2F;"} so that your package code isn't harmed by other peoples' use of `Depends`.*" - [Whether to Import or Depend: footnote reference ](https://r-pkgs.org/dependencies-mindset-background.html#fn6)
:::
...
I think the best guidance on when and why to use `pkg::fun()`, `@importFrom`, and the `Imports` field in the `DESCRIPTION` comes from the `roxygen2` documentation:
...
![Updated `NAMESPACE` with `Imports` and `importFrom](img/03_dependencies_namespace_import_importFrom.png){#fig-03_dependencies_namespace_import_shiny width='50%' align='center'}

![Updated `DESCRIPTION` with all  `Imports`](img/03_dependencies_description_imports.png){#fig-03_dependencies_namespace_import_shiny width='50%' align='center'}
...
![`movies_app()` works](img/03_dependencies_exported_movies_app.png){#fig-03_dependencies_namespace_import_shiny width='50%' align='center'}

![Examples in `?scatter_plot`](img/03_dependencies_scatter_plot_examples.png){#fig-import_check_app_examples width='50%' align='center'}
...
This section gets a bit technical, but I've done my best to include what I consider to be the necessary distinctions between using explicit namespacing (`pkg::fun()`), `@importFrom`, `@import`, and the `Imports` field in the `DESCRIPTION.`

:::: {.callout-note collpase="false" icon=false title="What happens to imported add-on package functions?"}

::: {.panel-tabset}

### [`pkg::fun()`]{style="font-size: 0.90em;"}

> "*Evaluating `foo::f` will cause package `foo` to be [loaded, but not attached]{style="color: #E74A2F;"}, if it was not loaded already*" - [Specifying imports and exports](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Specifying-imports-and-exports). 
...
> ```
> 
> *The code inside your package can [assume that aaapkg is installed whenever pkg is installed]{style="color: #E74A2F;"}.*
...
> "*The main difference is that a package you list in `Imports` will [just be loaded when you use it]{style="color: #E74A2F;"}, whereas a package you list in `Depends` [will be attached when your package is attached]{style="color: #E74A2F;"}.* - [Whether to Import or Depend](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-namespace)
...
So we can still access these functions using the `pkg::fun()` syntax: 
...
![`ggplot2` functions are still available if we explicitly namespace ](img/03_dependencies_using_ggplot2_with_ns.png){width=100%}
...
The code for this section is in the  [[`04_pkg-imports`](https://github.com/mjfrigaard/pkgApp/tree/04_pkg-imports)] branch of the [[`pkgApp`](https://github.com/mjfrigaard/pkgApp)] repo.
