...
Note that the only items in the new Shiny app project are `app.R` and the `shinyrPkgs.Rproj` file.
...
The `shinyrPkgs` project now contains the following files:
...
I've moved the `utils.R` file into the `R/` folder in `shinyrPkgs`:
...
View the code for this application in the [`shinyrPkgs` branches](https://github.com/mjfrigaard/shinyrPkgs/branches/all).
```
...
The deployed files of `shinyrPkgs` are below: 

```{bash}
#| eval: false 
#| code-fold: false
shinyrPkgs/ # 02.3_proj-app branch
├── DESCRIPTION
├── R/
│   ├── mod_scatter_display.R
│   ├── mod_var_input.R
...
The `rsconnect/` folder has been removed from the [`02.3_proj-app`](https://github.com/mjfrigaard/shinyrPkgs/tree/02.3_proj-app) branch.
...
- Lockfile written to '~/path/to/shinyrPkgs/renv.lock'.
```
