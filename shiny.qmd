...
We will replace the application in the `app.R` with the application from the [Building Web Applications with Shiny](https://rstudio-education.github.io/shiny-course/) course. I’ve chosen to use this app for the following reasons:
...
The header below is placed in the top of the `app.R` file
...
::: {.callout-note title="`mod_var_input_server()`" collapse='false' icon='false'}
...
::: {.callout-note title="`mod_scatter_display_server()`" collapse='false' icon='false'}
