...
Note that the only items in the new Shiny app project are `app.R` and the `shinyAppPkgs.Rproj` file.
...
The `shinyAppPkgs` project now contains the following files:
...
I've moved the `utils.R` file into the `R/` folder in `shinyAppPkgs`:
...
View the code for this application in the [`shinyAppPkgs` branches](https://github.com/mjfrigaard/shinyAppPkgs/branches/all).
```
...
The deployed files of `shinyAppPkgs` are below: 

```{bash}
#| eval: false 
#| code-fold: false
shinyAppPkgs/ # 02.3_proj-app branch
├── DESCRIPTION
├── R/
│   ├── mod_scatter_display.R
│   ├── mod_var_input.R
...
The `rsconnect/` folder has been removed from the [`02.3_proj-app`](https://github.com/mjfrigaard/shinyAppPkgs/tree/02.3_proj-app) branch.
...
- Lockfile written to '~/path/to/shinyAppPkgs/renv.lock'.
```
