...
If you're new to Shiny, or you have an app you'd like to add features to, I highly recommend using the Shiny Assistant.
...
Shiny Assistant is great for trying different UI components, layouts, and styling (color palettes, themes, fonts, etc.). We're going to walk through using the assistant for building an Shiny app-package, but we'll limit the number of prompts to five to keep this chapter brief. However, I still think you'll find the Assistant is a helpful tool to have in your Shiny toolbox.

## Prompt 1: Build an app

Let's start with trying to reproduce some of the applications we have in `sap`. I typically have data before I have an idea for a dashboard or app, so we'll pass the structure of the `movies` data to the Assistant with instructions on what we'd like to see:
...
https://raw.githubusercontent.com/mjfrigaard/sap/refs/heads/23_llm-shiny-assist/inst/shiny-assist/movies/movies_sample.csv
...
This prompt doesn't change the app display, but we're given a summary of the changes in the chat:
...
This results in an error in the Shiny Assistant UI:

![Shiny Assistant: error](images/shiny_assist_prompt_04.png){fig-align="center" width="100%"}

## Prompt 5: Error

The error above tells us the `bslib` package is not being loaded before `runApp()` is being called, so we'll give some direction on how to structure the file names and the call to `shinyApp()`:

**Prompt**: <code> An error occurred because the `page_sidebar` function could not be found. The `app.R` file needs to contain the `source()` calls for the modules and the call to `shinyApp()`. Also, rename the `ui.R` and `server.R` to `app_ui.R` and `app_server.R`. </code>
...
But this alone will not remove the error. We need to explicitly run the calls to `library()` and `source()` in the `app.R` file:

![Shiny Assistant: `library()` and `source()` calls](images/shiny_assist_library_source_calls.png){fig-align="center" width="100%"}

However, we see the following error when trying to call our standalone app function: 

![Shiny Assistant: `runApp()` error](images/shiny_assist_run_app_error.png){fig-align="center" width="100%"}

Debugging in Shiny Assistant is a little tricky, but we can see this is happening because we're trying to explicitly call `shinyApp()` (and the UI works by call `runApp()`).
...
Now that we have our modules, app functions and documentation, we'll add a [`thematic` theme](https://rstudio.github.io/thematic/), confirm it works, then download the app files.
...
Click on the cloud icon in the upper right corner to download the application files:

![Download files](images/shiny_assist_download.png){fig-align="center" width="70%"}

## Running locally

I've placed the downloaded files in the `inst/shiny-assist/movies` folder in `sap`, and after some minor changes to the files/folder organization, we can see the app-package structure starting to take shape:
...
The `app.R` file no longer needs to `source()` the modules and app functions if they are placed in an `R/` folder.
...
+ **`ggplot2` (`aes_string()`) warning**: as noted above, 

+ **`roxygen2` documentation**: the `@importFrom` and `@import` tags...

+ **module structure**: the Assistant came up with a different module structure than we have in our movies app in `sap`...
