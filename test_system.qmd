...
This chapter covers using `shinytest2` and `testthat` to perform system tests on each feature in your app-package. I'll also briefly cover set up with `chromote` (the package that gives us access to the Chromium headless browser).
...
```{r}
#| label: git_box_12d_tests-modules
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
The contents of `test-shinytest2.R` are below:
...
We can use this initial test as a template for writing our feature scenarios. 
...
With this approach you can create the test file as soon as you have a feature described (and come back later to fill in the `Scenario` and test).

An alternative approach is to use nested `describe()` functions and include the `Scenario`'s `Then` steps into the test (because these are what will actually be tested):
...
If I want to test a feature for one of the alternative applications I have in `moviesApp`, I can pass their location to the `app_dir` argument of `AppDriver$new()`. In the test below, the scenario describes changing the x, y, and color inputs and removing the missing values from the graph: 
