...
INFO [2024-05-20 04:19:56] [ START fixture = tidy_ggp2_movies.rds]
INFO [2024-05-20 04:19:56] [ START data = shinyAppPkgs::movies]
⠙ |          2 | scatter_plot                                 
INFO [2024-05-20 04:19:56] [ END data = shinyAppPkgs::movies]
✔ |          2 | scatter_plot
══ Results ═══════════════════════════════════════════════════════════════════════
...
### [`shinytest2`]{style="font-size: 1.05em;"} {#sec-tests-system-shinytest2}
...
Warning (test-shinytest2.R:23:3): {shinytest2} recording: shinyAppPkgs-feature-01
Adding new file snapshot: 'tests/testthat/_snaps/shinyAppPkgs-feature-01-001_.png'

Warning (test-shinytest2.R:23:3): {shinytest2} recording: shinyAppPkgs-feature-01
Adding new file snapshot: 'tests/testthat/_snaps/shinyAppPkgs-feature-01-001.json'
...
test_that("{shinytest2} recording: shinyAppPkgs-feature-01", {
  app <- AppDriver$new(name = "shinyAppPkgs-feature-01", height = 887, width = 1241) # <1>
...
If we want to test a feature for one of the alternative applications in `shinyAppPkgs`, we can pass their location to the `app_dir` argument of `AppDriver$new()`. In the test below, the scenario describes changing inputs for x, y, and color, and removing the missing values from the graph: 
...
describe(
  ...
  describe(
    ...
    app <- AppDriver$new(app_dir = system.file("dev", package = "shinyAppPkgs"), # <1>
...
## [`test.mode`]{style="font-size: 1.05em;"} {#sec-tests-system-test-mode}
...
└── shinytest2
    ├── feature-01-001.json
    ├── feature-01-001_.png
    ├── shinyAppPkgs-feature-01-001.json
    └── shinyAppPkgs-feature-01-001_.png
...
co_box(
  color = "g", 
  look = "default", hsize = "1.25", size = "1.10",
