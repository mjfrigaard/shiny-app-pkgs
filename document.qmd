...
co_box(
  ...
  header = "TLDR &emsp; ![](images/roxygen2.png){width='45' height='50'} ", 
  fold = TRUE,
  contents = "
...
The current state of `shinyAppPkgs` is stored displayed in the folder tree below: 
...
```
(*the `rsconnect/` folder from deploying `shinyAppPkgs` has been removed*)
...
Documenting the functions below `R/` will address message, and I strongly encourage checking out the [`roxygen2` vignettes](https://roxygen2.r-lib.org/articles/rd.html) and [R Packages, 2ed chapter](https://r-pkgs.org/man.html#roxygen2-basics) for more information on documenting your app-package. 
...
%%| fig-width: 6.5
%%| fig-align: center
%%| fig-cap: '`roxygen2` creates `man/` documentation'
...
To view how the `roxygen2` syntax will appear in the `.Rd` file, I'll document `shinyAppPkgs`:
...
![`.Rd` documentation file](images/doc_scatter_plot_rd.png){width='90%' fig-align='center'}
...
![`roxygen2` tag syntax in `.Rd` file](images/doc_scatter_plot_rd.png){width='100%' fig-align='center'}
...
![Help file](images/doc_scatter_plot_help.png){width='90%' fig-align='center'}
...
The syntax for an example plot with out `scatter_plot()` utility function is below:
...
When documenting modules, think of the audience as someone looking to understand the execution path through the application. In our standalone `shinyAppPkgs()` functions,  the inputs are collected with the `var_input` module, then passed to the `scatter_display` module to render in the UI.
...
The `roxygen2` documentation for `shinyAppPkgs` is saved in the [`02-roxygen` branch](https://github.com/mjfrigaard/shinyAppPkgs/tree/02-roxygen). As we can see, calling `devtools::document()` generates the `.Rd` files in the `man/` folder for each function in the `R/` folder:
