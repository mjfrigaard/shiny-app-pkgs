...
This chapter covers documenting the functions in your app-package. The output from the previous `install()` returned the following message regarding the documentation:

::: {style='font-weight: bold; font-size: 1.15em;'}

> *`No man pages found in package  ‘shinyAppPkg’`*

:::

We'll address this by introducing the basic fundamentals of the `roxygen2` syntax (i.e., the 'bare minimum' to include for each function in your app-package). I'll also touch on a few helpful `roxygen2` tags specific to shiny modules and standalone app functions. 
...
co_box(
  ...
  header = "![](img/roxygen2.png){width='8%'} &emsp; TLDR", 
  fold = TRUE,
  contents = "
...
The current state of `shinyAppPkg` is stored in the [`main`](https://github.com/mjfrigaard/shinyAppPkg/tree/main) branch[^main-06_create-package]. 

[^main-06_create-package]: The [`main`](https://github.com/mjfrigaard/shinyAppPkg/tree/main) branch of `shinyAppPkg` is identical to the [`06_create-package`](https://github.com/mjfrigaard/shinyAppProj/tree/06_create-package)  branch of `shinyAppProj`.
...
(*the `rsconnect/` folder from deploying `shinyAppProj` has been removed*)

I strongly encourage checking out the [`roxygen2` documentation](https://roxygen2.r-lib.org/articles/rd.html) and [R Packages](https://r-pkgs.org/man.html#roxygen2-basics) for more information on documenting your app-package. 
...
Roxygen: list(markdown = TRUE)
                                                        # <1>
```
1. Always leave at least one empty final line in your `DESCRIPTION` file.
...
![`.Rd` file](img/04_rox2_rd_title_description.png){#fig-04_rox2_rd_title_description width='90%' fig-align='center'}
...
co_box(
  color = "b", hsize = "1.10",
  header = "`.Rd` file syntax", fold = TRUE,
  contents = "
...
![`.Rd` file](img/04_rox2_help_with_md.png){#fig-04_rox2_help_with_md width='90%' fig-align='center'}
...
![**Run examples** hyperlink](img/04_rox2_aes_examples.png){#fig-04_rox2_aes_examples width='90%' fig-align='center'}

![Executed `@examples` code](img/04_rox2_aes_examples_example.png){#fig-04_rox2_aes_examples width='90%' fig-align='center'}
...
![`@examples` in `.Rd` preview](img/04_rox2_rd_examples_doc.png){#fig-04_rox2_rd_examples_doc width='90%' fig-align='center'}
...
![`@seealso` link](img/04_rox2_mod_var_input_ui_seealso.png){#fig-04_rox2_mod_var_input_ui_seealso width='60%' fig-align='center'}
...
::: {#fig-04_rox2_seealso_link_01 layout="[30,-1,69]" layout-valign="top"}
...
![`movies_ui()`](img/04_rox2_movies_ui_rd.png){width='90%' fig-align='center'}
...
![`movies_server()`](img/04_rox2_movies_server_rd.png){width='90%' fig-align='center'}
...
![`movies_app()`](img/04_rox2_movies_app_rd.png){width='90%' fig-align='center'}
...
The `roxygen2` documentation for `shinyAppPkg` is saved in the [`02-roxygen` branch](https://github.com/mjfrigaard/shinyAppPkg/tree/02-roxygen). As we can see, calling `devtools::document()` generates the `.Rd` files in the `man/` folder for each function in the `R/` folder:
