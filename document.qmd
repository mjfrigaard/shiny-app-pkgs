...
When documenting modules, think of the audience as someone looking to understand the execution path through the application. In our standalone `moviesApp()` functions,  the inputs are collected with the `var_input` module, then passed to the `scatter_display` module to render in the UI.
