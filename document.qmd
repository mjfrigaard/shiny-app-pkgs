...
&emsp; &emsp; [<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>L</kbd> / <kbd>D</kbd>]{style='font-weight: bold; font-size: 0.75em;'}
  ...
  6. Use `@inheritParams` to avoid repeating shared parameter docs across module functions; filter with an explicit list or `-param` to exclude server-only arguments\n

  7. Start tag values on the same line as the tag -- `roxygen2` 8.0.0 warns when the value begins on the next line\n

  8. Run `roxygen2::needs_roxygenize('.')` to check whether `.Rd` files are out of date before committing\n
...
The structure of `sap` at the start of this chapter is shown below:[^devtools-branch]
...
9. App resources
10. Image file
11. Renamed to `scatter_plot.R` in the [File name alignment](#sec-doc-file-name) section below
...
### File name alignment {#sec-doc-file-name}
...
When we created our app-package with `create_package()`, support for markdown formatting in our package help files was automatically included by adding `Config/roxygen2/markdown: TRUE` to the `DESCRIPTION` file:
...
License: GPL-3
Encoding: UTF-8
Config/roxygen2/markdown: TRUE #<2>
```
1. Updated package version
2. Markdown support (`Config/roxygen2/` replaces the older `Roxygen: list(markdown = TRUE)` field)

If you have an existing package with the legacy `Roxygen:` and `RoxygenNote:` fields, running `devtools::document()` will migrate them to the `Config/roxygen2/` format automatically. If the field is missing entirely, follow the instructions in the [`roxygen2` documentation](https://roxygen2.r-lib.org/articles/markdown.html#turning-on-markdown-support) for enabling markdown.
...
co_box(
  color = "r",
  look = "default", hsize = "1.05", size = "0.95",
  header = "Multi-line tag values (roxygen2 8.0.0)",
  fold = TRUE,
  contents = "
Starting a tag value on the line *below* the tag now triggers a warning:

\`\`\`r
#' @return
#' A ggplot2 plot object   # <-- warning: value starts on next line
\`\`\`

Put the value on the same line as the tag:

\`\`\`r
#' @return A ggplot2 plot object
\`\`\`

Continuation lines (indented text that wraps a long description) are still fine and do not trigger a warning.
  "
)
```

### Inheriting parameters {#sec-roxygen2-inheritparams}

When multiple functions share parameters, `@inheritParams` pulls the documentation from a source function so you only have to write it once:
...
In `sap`, every module UI and server function accepts an `id` parameter. Rather than copying the same `@param id` line into each function, document it once on `mod_var_input_ui()` and inherit it everywhere else:
...
**Filtering inherited parameters** is new in `roxygen2` 8.0.0. If you only need a subset of the source function's parameters, list them explicitly:
...
To inherit everything *except* specific parameters, prefix them with `-`:

```{r}
#| eval: false
#| code-fold: false
#' @inheritParams mod_var_input_ui -var_inputs
```

This filtering is especially useful in Shiny modules, where the server function shares `id` with the UI function but not the reactive arguments (like `var_inputs`) that are server-side only.
...
flowchart TB
  ...
  style Code fill:#f9f9f9,stroke:#999,stroke-width:2px,rx:10,ry:10
  style Roxy2 fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px,rx:10,ry:10
  style Doc fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:2px,rx:10,ry:10
  style Parse fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px,rx:10,ry:10
  style Man fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:10,ry:10
  style Root fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px,rx:10,ry:10
  style Rd fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
  style NS fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
```
...
### Checking documentation status {#sec-roxygen2-needs-roxygenize}

`roxygen2` 8.0.0 introduced [`needs_roxygenize()`](https://roxygen2.r-lib.org/reference/needs_roxygenize.html), which checks whether the `.Rd` files in `man/` are out of date relative to the source files in `R/` (without running a full `devtools::document()` pass):

```{r}
#| eval: false
#| code-fold: false
roxygen2::needs_roxygenize(".")
```

This will return `TRUE` if any `.Rd` file needs regeneration and `FALSE` if everything is current, which makes it useful in two places:

1. **Pre-commit hooks** will fail fast if documentation was forgotten:

```{r}
#| eval: false
#| code-fold: false
if (roxygen2::needs_roxygenize(".")) {
  stop("Documentation is out of date. Run devtools::document() before committing.")
}
```

2. **CI checks** will verify that committed docs match their source:

```{r}
#| eval: false
#| code-fold: false
if (roxygen2::needs_roxygenize(".")) {
  stop("man/ is out of sync with R/. Re-run devtools::document() and commit the result.")
}
```

Because `needs_roxygenize()` compares file modification times rather than regenerating output, it's much faster than running `devtools::document()` and adds no meaningful overhead to either hook.[^hooks]

[^hooks]: Git hooks are scripts that run automatically at specific points in the Git workflow (e.g., before a commit with `pre-commit`, or before a push with `pre-push`). They live in `.git/hooks/` and can be used to enforce checks — like ensuring documentation is current — before changes are recorded or shared. Read more about [Git hooks](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks).
...
The markdown formatting is helpful in sections (`@section`), where we can use headings (`###`), bold text, or lists to structure detailed explanations.
...
flowchart LR
    ...
    style mod_var_input_ui fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:1px
    style mod_var_input_server fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:1px
    style mod_scatter_display_server fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:1px
    style tag fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style ns fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
```
...
For the variable input/display modules:
...
flowchart TD
    subgraph AppCom["<strong>App Components</strong>"]
        launch_app("Standalone Function:<br>launch_app()")
    end
    
    subgraph Graph["<strong>Display Module</strong>"]
    ...
    style AppCom fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style Vars fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style Graph fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style Utils fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style launch_app fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:1px
    style mod_var_input_ui fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style mod_var_input_server fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style mod_scatter_display_ui fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style mod_scatter_display_server fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style scatter_plot fill:#333,color:#fff,stroke:#000,stroke-width:1px
...
Below are `roxygen2` documentation examples for the variable input module. See the [full documented module on GitHub](https://github.com/mjfrigaard/sap/blob/05_roxygen2/R/mod_var_input.R) for complete implementation with `@examples`.

**`mod_var_input_ui()`**:

Creates a UI for selecting variables and attributes for a scatter plot. Namespace all input IDs with `shiny::NS()` and return a `shiny::tagList()` with UI elements (dropdowns for x/y/color, sliders for alpha/size, text input for title).
...
The server-side documentation describes 1) the reactive expression the UI returns, 2) the connection to the `mod_scatter_display_server()` via `@seealso`.
...
Below are `roxygen2` documentation examples for the graph display module. See the [full documented module on GitHub](https://github.com/mjfrigaard/sap/blob/05_roxygen2/R/mod_scatter_display.R) for complete implementation with `@examples`.

**`mod_scatter_display_ui`**:

Creates a UI for displaying the scatter plot and metadata. All input IDs are namespaced with `shiny::NS()`. References the utility function (`scatter_plot()`) with `@seealso`:
...
The display server module includes `@section`s describing the reactive expression (`var_inputs`) and lists all the reactive inputs. Also includes references to the input module  (`mod_var_input_server()`) and utility function (`scatter_plot()`).
...
Our [`scatter_plot()`](https://github.com/mjfrigaard/sap/blob/05_roxygen2/R/scatter_plot.R) utility function includes descriptions of the inputs (with `@param`) and a `@section` describing how it's used. It also includes a hyperlink to the `ggplot2` documentation. 

**`scatter_plot()`**:

References the module server function it's used in (`mod_scatter_display_server()`)
...
The [`movies_ui()`](https://github.com/mjfrigaard/sap/blob/05_roxygen2/R/movies_ui.R) doesn't have any arguments--it contains the theme, image, and application title:
...
[`movies_server()`](https://github.com/mjfrigaard/sap/blob/05_roxygen2/R/movies_server.R) is written to be passed to the `server` argument in `shinyApp()`:
...
flowchart TD
    ...
    subgraph Vars["<strong>Input Module</strong>"]
        mod_var_input["Module:<br><code>mod_var_input.R</code>"]
    end
    
    subgraph Graph["<strong>Display Module</strong>"]
        mod_scatter_display["Module:<br><code>mod_scatter_display.R</code>"]
    end
    ...
    launch_app --> UI
    launch_app --> Server
    UI --"call UI module function"--> mod_var_input
    UI --"call UI module function"--> mod_scatter_display
    Server --"calls server module function"--> mod_var_input
    Server --"calls server module function"--> mod_scatter_display
    mod_scatter_display --"calls"--> scatter_plot
    ...
    style Utils fill:#e8f4f1,stroke:#4CBB9D,stroke-width:2px
    style launch_app fill:#4CBB9D,color:#fff,stroke:#2d8a7a,stroke-width:1px
    style UI fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style Server fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style mod_var_input fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style mod_scatter_display fill:#7fd9c7,color:#fff,stroke:#4CBB9D,stroke-width:1px
    style scatter_plot fill:#333,color:#fff,stroke:#000,stroke-width:1px
    
```

The `@export` tag controls which functions become part of our package's public API. Any function tagged with `@export` is written to the `NAMESPACE` file by `devtools::document()` and is available to users after `devtools::install()`.

In app-packages, the standalone app function is the one function users need to call directly, so it should always carry `@export`. Internal functions (like `movies_ui()`, `movies_server()`) and the individual module functions don't need to be exported, unless you intend for users to call them outside the app. 

Using `@export` with discretion keeps the package namespace clean and signals which functions are internal and only for implementation.
...
The complement to `@export` for internal functions is `@noRd`. Adding `@noRd` to a `roxygen2` block tells `devtools::document()` to skip generating an `.Rd` file for that function. 

This is useful when we want to keep `roxygen2` comments for our own reference (parameter types, internal notes) without creating a help page that would show up in the package index. 

Functions with no `roxygen2` block at all behave the same way, so `@noRd` is most valuable when we already have comments and want to be explicit that the function is intentionally undocumented:
...
#' Build the movies UI
#'
#' @param ... passed to `shiny::fluidPage()`
#'
#' @noRd
movies_ui <- function(...) {
  # internal — not exported, no help page generated
}
```

Together, `@export` and `@noRd` give precise control over the `NAMESPACE` and the `man/` folder: 

* `@export` promotes a function to the public API

* `@noRd` keeps internal comments tidy without polluting the documentation index.

### Usage {#sec-roxygen2-usage}

`roxygen2` auto-generates a `@usage` block from the function signature, which is exactly what you want for functions with meaningful parameters. For a no-argument wrapper like `launch_app()`, however, the auto-generated line would simply be:
...
That adds nothing a reader couldn't already see from the function name. Setting [`@usage NULL`](https://roxygen2.r-lib.org/articles/rd.html#usage) suppresses the auto-generated line entirely, keeping the focus on the `@details` and `@section` blocks that actually explain how the function works. 

It's the right choice any time the signature itself is uninformative and the prose explanation does the real work.

```{r}
#| eval: false 
#| code-fold: show 
#| code-summary: 'show/hide launch_app documentation'
...
co_box(
  ...
  - **DESCRIPTION format**: Markdown support is now declared as `Config/roxygen2/markdown: TRUE`; the older `Roxygen: list(markdown = TRUE)` and `RoxygenNote:` fields are migrated automatically on the next `devtools::document()` run.
  ...
  - **Tag values on the same line**: roxygen2 8.0.0 warns when a tag value starts on the line below the tag. Put values on the same line as the tag; continuation lines that wrap a long description are still fine.

  - **Inheriting parameters (`@inheritParams`)**: Use `@inheritParams source_fun` to pull parameter docs from another function instead of duplicating them. Filter with an explicit argument list (`@inheritParams source_fun id`) or exclude specific params with a `-` prefix (`@inheritParams source_fun -var_inputs`).
  ...
  - **Checking documentation status**: `roxygen2::needs_roxygenize('.')` returns `TRUE` if any `.Rd` file is out of date, making it useful in pre-commit hooks and CI checks without the overhead of a full `devtools::document()` run.
