...
The current state of `shinyrPkgs` is stored displayed in the folder tree below: 
...
```
(*the `rsconnect/` folder from deploying `shinyrPkgs` has been removed*)
...
To view how the `roxygen2` syntax will appear in the `.Rd` file, I'll document `shinyrPkgs`:
...
![`.Rd` documentation file](images/doc_scatter_plot_man_rd.png){width='90%' fig-align='center'}
...
When documenting modules, think of the audience as someone looking to understand the execution path through the application. In our standalone `shinyrPkgs` functions,  the inputs are collected with the `var_input` module, then passed to the `scatter_display` module to render in the UI.
...
The `roxygen2` documentation for `shinyrPkgs` is saved in the [`02-roxygen` branch](https://github.com/mjfrigaard/shinyrPkgs/tree/05_roxygen2). As we can see, calling `devtools::document()` generates the `.Rd` files in the `man/` folder for each function in the `R/` folder:
