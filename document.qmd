...
The current state of `sap` is stored displayed in the folder tree below: 
...
When documenting modules, think of the audience as someone looking to understand the execution path through the application. In our standalone `sap` functions,  the inputs are collected with the `var_input` module, then passed to the `scatter_display` module to render in the UI.
