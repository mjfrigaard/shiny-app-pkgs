...
-  `golem` 'gives away' many helper functions with boilerplate tests for free
...
- The testing infrastructure is set up using the custom `golem::use_recommended_tests()` function, which creates the necessary `testthat` folders we covered in the [Test suite](test_suite.qmd) chapter [^golem-start-tests], but also adds a collection of boilerplate tests.
...
- **External files**: adding external files is handled with `golem_add_external_resources()`, which uses the same methods we covered in the [External files](external.qmd) chapter.[^golem-add-external-files]  
<br>
...
- **Posit Connect**: arguments and options for deploying your application using `rsconnect::deployApp()`. [^golem-deploy-rsconnect-options]

- **Docker**: golem has multiple options for creating Docker files: 
...
-   `gap_0.0.0.9000.tar.gz`: this is our app-package to deploy in the Docker container.
