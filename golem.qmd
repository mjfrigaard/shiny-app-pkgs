...
This chapter walks through building a version of the `moviesApp` with the [`golem` framework](https://thinkr-open.github.io/golem/). The resulting app-package (`gap`) is in the [`15_golem` branch](https://github.com/mjfrigaard/moviesApp/tree/15_golem).
...
After checking out the `15_golem` branch, be sure to load, document, and install the application.
...
<br><br>
  - [x] `extdata/` contains the external data files. [^golem-inst-extdata] 
<br><br>
...
- [x] **Data**: the data in `golem` app-packages function like the data folder and files in a standard R package we covered in the [Data](data.qmd) chapter.
<br><br>
...
<br><br>
    - [x] `tidy_movies.R` reads in the `ggplot2movies::movies` data and creates the `inst/extdata/tidy_movies.fst` data.
...
7. Created with: `golem::use_recommended_tests()`
8. Created with: `golem::use_utils_ui(with_test = TRUE)` 
9. Created with: `golem::use_utils_server(with_test = TRUE)`
10. Created with: `golem::add_module(name = 'scatter_display', ` `with_test = TRUE)`
11. Created with: `golem::add_module(name = 'var_input',`  `with_test = TRUE)`
12. Test recording from `shinytest2::record_test()`   
...
-   [x] The tests for `scatter_plot()` is in the `test-utils_mod_scatter_display.R` file.
...
INFO [2023-11-07 13:30:24] [ END data = movies.rda]
```
...
[ FAIL 0 | WARN 0 | SKIP 0 | PASS 1 ]
INFO [2023-11-07 13:34:42] [ END input = initial inputs returned()]
```

-   [x] `test-mod_scatter_display.R`
...
[ FAIL 0 | WARN 0 | SKIP 0 | PASS 2 ]
INFO [2023-11-07 13:36:51] [ END data = movies.rda]
```
...
-   [x] `test-shinytest2.R` is the initial resulting test from `shinytest2::record_test()` covered in [System tests](test_system.qmd)
...
These let us know the initial snapshots are saved in `tests/testthat/_snaps/`
...
-   [x] `test-app-feature-01.R` contains feature tests, and the snapshot warnings display the `Scenario` descriptions:
