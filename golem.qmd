...
This chapter walks through building a version of the `shinyAppPkgs` with the [`golem` framework](https://thinkr-open.github.io/golem/). The resulting app-package (`gap`) is in the [`22_golem` branch](https://github.com/mjfrigaard/shinyAppPkgs/tree/22_golem).
...
[^golem-inst-app]: the `app/` folder is used to add external resources to the application (similar to the previous versions of `shinyAppPkgs`).
...
#| eval: false 
#| code-fold: false
# in the 22_golem branch of shinyAppPkgs
...
For comparison, this is the `shinyAppPkgs` dependency tree (note that using `devtools`/`usethis` doesn't make our app-package depend on these packages).

```{r}
#| eval: false 
#| code-fold: false
# in the 09d_inst-prod branch of shinyAppPkgs
