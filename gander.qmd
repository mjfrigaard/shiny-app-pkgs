...
Having graph code in a separate utility function makes it easier to debug/write tests. Writing examples in the `roxygen2` documentation also makes it possible to view minor changes to the graph without dealing with the reactivity inside the application:
...
`gander` only returned a few changes to the `ggplot2` code, but these changes reduced a lot of time because the `ggplot2::theme()` arguments are be difficult to remember, and usually take multiple iterations to get correct: 
