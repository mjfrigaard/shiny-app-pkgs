...
`gander` is incredibly helpful for making small adjustments to `ggplot2` graphs. I noticed the text size was too small in the graphs on the **Distribution** tabs, so I asked the chatbot to make the font size change dynamically with the display. 

> **Prompt:** ***"I want the font in the ggplot2 graph to adjust with the screen size"***

This only made a few changes to the `ggplot2` code, but the `ggplot2::theme()` can be difficult to remember: 
