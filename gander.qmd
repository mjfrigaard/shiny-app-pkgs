...
Configure the keyboard shortcut following the instructions on the [package website](https://simonpcouch.github.io/gander/index.html#installation) (RStudio ![](images/rstudio-icon.png){height=30} or Positron ![](images/positron.png){height=30}). I use the recommended [<kbd>Ctrl</kbd>+<kbd>Cmd</kbd>+<kbd>G</kbd>]{style='font-weight: bold;'} (or [<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>G</kbd>]{style='font-weight: bold;'} on Windows).
...
## Addin and keyboard shortcut {#sec-gander-dev-addin} 
...
### Providing context {#sec-gander-suffix} 
...
### [`system`]{style="font-weight: bold; font-size: 1.15em;"}

::: {style="font-size: 1.10em; color: #282b2d;"}

{{< include _gander_system.qmd >}}
...
### [`user`]{style="font-weight: bold; font-size: 1.15em;"}

::: {style="font-size: 1.10em; color: #282b2d;"}

{{< include _gander_user.qmd >}}
...
### [`assistant`]{style="font-weight: bold; font-size: 1.15em;"}

::: {style="font-size: 1.10em; color: #282b2d;"}

{{< include _gander_assistant.qmd >}}

:::

:::

## What _is_ `gander` doing? {#sec-gander-in-practice}

`gander` isn't a tool or skill--it *pre-injects* context into the prompt. The addin allows us to write a new prompt each time--making it more flexible than `chores` helpers--and `gander` automatically attaches the IDE/environment context.

```{mermaid}
%%| fig-cap: 'gander development'
%%| eval: true 
%%| echo: false
%%| fig-align: center
...
flowchart TD
    User(["User<br>prompt"]) --> Gander["<code>gander</code><br>assembles<br>context"]
    Selection(["Selected<br>code"]) --> Gander
    EnvSnapshot(["IDE/environment<br>(data, objects)"]) --> Gander
    Gander --> LLM["LLM via<br><code>ellmer</code>"]
    LLM --> Insert("Streams into<br>source file")

    style User fill:#e8f4f1,stroke:#4CBB9D,stroke-width:3px,color:#000
    style Selection fill:#e8f4f1,stroke:#4CBB9D,stroke-width:3px,color:#000
    style EnvSnapshot fill:#e8f4f1,stroke:#4CBB9D,stroke-width:3px,color:#000
    style Gander fill:#4CBB9D,stroke:#2d8a7a,stroke-width:2px,color:#fff
    style LLM fill:#7fd9c7,stroke:#4CBB9D,stroke-width:2px,color:#fff
    style Insert fill:#4CBB9D,stroke:#2d8a7a,stroke-width:2px,color:#fff
