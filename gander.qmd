...
## Graph adjustments  {#sec-gander-utils} 

I've found the `gander` addin to be incredibly helpful when developing visualizations in Shiny app-packages. For example, the **Compare** tab displays a `plotly` scatter plot.  I asked `gander` to convert the plotly code in the module server function into a utility function (`R/compare_plot.R`):
...
Having graph code in a separate utility function makes it easier to debug/write tests. Examples in the `roxygen2` documentation also make it easier to make minor changes to graph without worrying about the reactivity inside the application:

![](images/gander_examples_compare_plot.png){width='100%' fig-align='center'}

I also asked `gander` to make a small adjustments to the `ggplot2` graphs. I noticed the text size was too small in the graphs on the **Distribution** tabs:
...
## Downloadable report {#sec-gander-rmd-report} 
...
The `params` in the report will be passed from the new `mod_download_report` module, which we can ask gander to assist with by highlighting the contents of app's server function (`nav_server()`)
...
As we can see, this module conforms to our naming convention and follows the guidance from the official [generating reports](https://shiny.posit.co/r/articles/build/generating-reports/) documentation (i.e., it has a `downloadHandler()` with  `filename` and `content`, and passes the `params` to `rmarkdown::render()`).
...
I stopped here, but as you can see, `gander` can be used to continue developing utility functions for each graph and table in the application to use in the report (or we could build the outputs manually using the `params` in the `isnt/report_template.Rmd` file).
