...
## In practice {#sec-gander-in-practice}

`gander` *pre-injects* context into the prompt--making it more flexible than `chores` helpers. The addin allows us to write the prompt each time, and `gander` auto-attaches IDE/environment context.
...
flowchart TD
    User(["User<br>prompt"]) --> Gander["<code>gander</code><br>assembles context"]
    Selection(["Selected<br>code"]) --> Gander
    EnvSnapshot(["R environment<br>(data, objects)"]) --> Gander
    Gander --> LLM["LLM via <code>ellmer</code>"]
