...
I'll use color to differentiate shiny app [projects]{style="font-weight: bold; color: #5cb85c"}, R [packages]{style='font-weight: bold; color: #F37735;'}, and [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"}.

:::

-   [Project]{style="font-weight: bold; color: #5cb85c"}: I consider a [project]{style="font-weight: bold; color: #5cb85c"} to be any directory of files requiring R (the binary downloaded from [CRAN](https://cran.r-project.org/) with a version number and neat names like, '*R 4.3.1 "Beagle Scouts" released on ...*') to execute with an RStudio/Posit workbench project file (`.Rproj`).

-   [Package]{style='font-weight: bold; color: #F37735;'}: I'll be using the term [package]{style='font-weight: bold; color: #F37735;'} to describe a directory of functions, documentation, or data that can be installed and loaded into an R session. A [package]{style='font-weight: bold; color: #F37735;'} includes the necessary dependency management (`NAMESPACE`) and metadata files (`DESCRIPTION`) *and* has access to the package development tools in RStudio/Posit Workbench.

-   [App]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"}: an [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"} (which you have probably already guessed) is a [package]{style='font-weight: bold; color: #F37735;'} containing a shiny application. [App]{style="font-weight: bold; color: #428BCA"}-[packages]{style="font-weight: bold; color: #F37735"} have all of the functionality of a standard R [package]{style='font-weight: bold; color: #F37735;'}, but also contain the files and folders required to successfully develop, run, and deploy a shiny [app]{style="font-weight: bold; color: #428BCA"}.
...
The previous chapter covered a few practices to adopt during shiny development that improves the extensibility of your app [project]{style="font-weight: bold; color: #5cb85c"} to an [app]{style='font-weight: bold; color: #428BCA;'}-[package]{style='font-weight: bold; color: #F37735;'} (these are summarized in the callout box below).
...
There are multiple reasons for deciding to develop an [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"}: your app [project]{style="font-weight: bold; color: #5cb85c"} might have to be in a [package]{style='font-weight: bold; color: #F37735;'} structure due to specific organizational needs or practices around deploying applications, or maybe you've been convinced it's the best practice for improving the shareability/reproducibility of your app [project]{style="font-weight: bold; color: #5cb85c"}. 
...
2. You've already developed a shiny app [project]{style="font-weight: bold; color: #5cb85c"}, but now you need to convert it to an [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"}.

### R [package]{style='font-weight: bold; color: #F37735;'} vs. shiny [project]{style="font-weight: bold; color: #5cb85c"}
...
At first glance, the contents above can seem daunting when compared to the folders and files that currently exist in your app [project]{style='font-weight: bold; color: #5cb85c;'} (like `projApp` below): 
...
If you've been following the code in `projApp`, the quotes above should confuse you. The `projApp` shiny [project]{style='font-weight: bold; color: #5cb85c;'} has a `DESCRIPTION` file and an `R/` directory, but `projApp` is not a functioning [package]{style='font-weight: bold; color: #F37735;'}. 
...
As we've just learned, the presence of the `DESCRIPTION` file and an `R/` folder are insufficient to turn your app [project]{style='font-weight: bold; color: #5cb85c;'} into a [package]{style='font-weight: bold; color: #F37735;'} (or an [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"}).
...
> "*A [project]{style="font-weight: bold; color: #5cb85c"} needs **a `DESCRIPTION` file with specific fields**, a directory of `R/` files, and **a properly configured `.Rproj` file** to be a [package]{style='font-weight: bold; color: #F37735;'}.*" - Me

Let's see how each of these requirements works together to convert the contents of `projApp` from a [project]{style="font-weight: bold; color: #5cb85c"} into a [package]{style='font-weight: bold; color: #F37735;'}.
...
### [Project]{style="font-weight: bold; color: #5cb85c"} options 
...
#### Default [project]{style="font-weight: bold; color: #5cb85c"} options

**Tools** > **Project options** provide access to the [project]{style="font-weight: bold; color: #5cb85c"}-level options. For example, fields 2 - 4 are available under **General**, 5 - 8 affect the **Code** options, and the final two fields deal with **Sweave**. 
...
These are the default settings created when we selected the Shiny App [project]{style='font-weight: bold; color: #5cb85c;'} from the **New Project Wizard** in the last chapter.
...
When the new session starts, the new [project]{style="font-weight: bold; color: #5cb85c"}-level options activate the **Build** pane in the IDE. 
...
While this information isn't incorrect, we just demonstrated it's possible to create a [package]{style='font-weight: bold; color: #F37735;'} without the `usethis::create_package()` function. Still, if you're looking for a way to quickly create or convert your app [project]{style="font-weight: bold; color: #5cb85c"} into an [app-package]{style='font-weight: bold; color: #428BCA;'}, using `create_package()` is a great option. 
...
If you already have a shiny app [project]{style='font-weight: bold; color: #5cb85c'} that needs to be converted into an [app]{style="font-weight: bold; color: #428BCA"}-[package]{style="font-weight: bold; color: #F37735"} (like the app files stored in the [`03_projApp` branch](https://github.com/mjfrigaard/projApp/tree/03_projApp)), you can also use `create_package(path = getwd())` in your root folder, but I recommend using the following arguments:
...
When converting your existing shiny app [project]{style='font-weight: bold; color: #5cb85c'} into am [app]{style='font-weight: bold; color: #428BCA'}-[package]{style='font-weight: bold; color: #F37735'} with `usethis::create_package()`, don't use `'.'` in the `path` argument:
  
`\`\`\r
usethis::create_package('.')
`\`\`\
  
This will return the following warning about creating nested [projects]{style='font-weight: bold; color: #5cb85c'}, and ask if you want to proceed anyway:
...
We can avoid this warning altogether by passing the `getwd()` to the `path` argument, so I recommend cancelling the [project]{style='font-weight: bold; color: #5cb85c'} creation:
...
This chapter has covered the mandatory fields in the `DESCRIPTION` file and the [package]{style='font-weight: bold; color: #F37735;'} configuration fields in `.Rproj`. We also covered creating and converting [projects]{style='font-weight: bold; color: #5cb85c'} using the `usethis::create_package()` function.
...
-   `usethis::create_package()` can be used to create a new [package]{style='font-weight: bold; color: #F37735;'} and to convert an existing shiny [project]{style='font-weight: bold; color: #5cb85c;'} into a shiny [app]{style='font-weight: bold; color: #428BCA'}-[package]{style='font-weight: bold; color: #F37735'}.

-   The IDE reads RStudio (`.Rproj`) files and determines R session settings at the [project]{style='font-weight: bold; color: #5cb85c;'}-level (i.e., working directory, workspace, history, code formatting, etc.)
