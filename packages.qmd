...
co_box(
  ...
  header = "TLDR  &emsp; ![](images/rpkg_icon_no_bkgrnd.png){width='45' height='40'}", 
  fold = TRUE,
  contents = "
...
The `shinyAppPkgs` directory has: 
...
So, is `shinyAppPkgs` a functioning R package? 
...
When we open `shinyAppPkgs`, the **Build** pane is not displayed in the IDE:

::: {#fig-pkgs_no_build_pane}
![`shinyAppPkgs` IDE](images/pkgs_no_build_pane.png){#fig-pkgs_no_build_pane width="100%" fig-align="center"}
...
Let's see how each of these requirements work together to convert the contents of `shinyAppPkgs` from a Shiny project into a Shiny app-package.
...
Below is an example `DESCRIPTION` file for our new `shinyAppPkgs` package with the mandatory fields:[^pkgs-10]
...
![`shinyAppPkgs` IDE](images/pkgs_desc_no_build_pane.png){#fig-pkgs_desc_no_build_pane height="100%" width="100%" fig-align="center"}
...
![`shinyAppPkgs.Rproj` fields](images/pkgs_proj_options.png){#fig-pkgs_proj_options width="100%" fig-align="center"}

Field settings from `shinyAppPkgs.Rproj` file
:::
...
I've placed the `.Rproj` file from `shinyAppPkgs` with the example `.Rproj` file from [R Packages, (2ed)](https://r-pkgs.org/) side-by-side so you can compare them below:[^pkgs-11]
...
Comparison of `shinyAppPkgs.Rproj` file and `.Rproj` file in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project)
:::

I've circled the fields in the `.Rproj` file that illustrate it's configured to work with an R package. Note that in `shinyAppPkgs`, the **Project build tools** are initially set to (**None**) under **Build Tools**:

::: {#fig-pkgs_proj_opts_build_tools}
![`shinyAppPkgs.Rproj` build tools](images/pkgs_proj_opts_build_tools.png){#fig-pkgs_proj_opts_build_tools height="100%" width="100%" fig-align="center"}

Build tool settings in `shinyAppPkgs.Rproj` file
:::

Changing the **Project build tools** option to **Package** will set the default **Build Tools** options:[^pkgs-12]

[^pkgs-12]: The initial **Build Tools** settings (i.e., **(None)**) should help explain the absence of any R package development fields in the `shinyAppPkgs.Rproj` when we compared it to the version in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) (i.e., `BuildType`, `PackageUseDevtools`, `PackageInstallArgs`, and `PackageRoxygenize`). 
...
The links between the **Build Tools** options and fields in `shinyAppPkgs.Rproj` are in the figure below:

::: {#fig-pkgs_rproj_connect_build_tools}
![`shinyAppPkgs.Rproj` build tool fields](images/pkgs_rproj_connect_build_tools.png){#fig-pkgs_rproj_connect_build_tools width="100%" fig-align="center"}

Default build tool settings in `shinyAppPkgs.Rproj` file
:::

-   `BuildType: Package` tells the IDE `shinyAppPkgs` is an R package and triggers the **Build** pane.
...
After clicking **OK**, the IDE will automatically reboot, and the additional fields will be added to the bottom of the `shinyAppPkgs.Rproj`:
...
There you have it--`shinyAppPkgs` is a functional app-package!
...
`\`\`r
New project 'shinyAppPkgs' is nested inside an existing project
...
usethis::create_package(
  path = getwd(),
  fields = list(Package = 'shinyAppPkgs',
         Version = '0.0.0.9000',
         Title = 'Shiny App-Packages',
         Description = 'An R package with a collection of Shiny applications.',
         "Authors@R" = NULL,
...
#| eval: false 
#| code-fold: false
Overwrite pre-existing file 'shinyAppPkgs.Rproj'?
```
...
The `DESCRIPTION` file (shown below) has a few additional fields (`Encoding`, `Roxygen`, and `RoxygenNote`) we didn't include when we converted `shinyAppPkgs` above, but we will cover these in the upcoming chapters. 
