...
## Mastering Shiny Packages Chapter (a caveat)

::: {style='font-size: 1.05em; color: #696969;'}

This chapter (and arguably this entire book) was written because transitioning from 'shiny app development' into 'R package development' is complicated, and everyone enters these areas with varying experience levels and skill-sets. 

The information necessary for your particular application or development environment is likely spread across multiple resources (or it doesn't exist). 

For example, if you happen to download or clone the [`monthApp`](https://github.com/hadley/monthApp) example from the [Packages](https://mastering-shiny.org/scaling-packaging.html) Chapter in Mastering Shiny, you might notice a few things:

1. The [`DESCRIPTION` file](https://github.com/hadley/monthApp/blob/master/DESCRIPTION) contains additional fields that are not addressed.[^pkgs-1] 

2. The `monthApp.Rproj` file has been configured to work with package.[^pkgs-2]

3. Dependency management is discussed briefly, but not documented in the application. [^pkgs-3]
  
3. `monthApp` doesn't have any documentation[^pkgs-4] or tests. [^pkgs-5] 

[^pkgs-1]: [Mastering Shiny](https://mastering-shiny.org/scaling-packaging.html#a-package) recommends using the `usethis::use_description()` function, but not all of the `fields` in this function are required (and there are a few we can adapt or remove to save time).

[^pkgs-2]: The `.Rproj` file is the connection between the `DESCRIPTION` fields, the code in the `R/` folder, and the IDE's **Build** pane or “package development mode.” If you already have an `.Rproj` file in your shiny app project, you can activate these settings in **Tools** > **Project Options** > **Build Tools**.

[^pkgs-3]: The [`NAMESPACE` file](https://github.com/hadley/monthApp/blob/master/NAMESPACE) is empty (although the Chapter recommends [importing `shiny`](https://mastering-shiny.org/scaling-packaging.html#r-cmd-check)), and the `Imports` field is missing from the [`DESCRIPTION`](https://github.com/hadley/monthApp/blob/master/DESCRIPTION).

[^pkgs-4]: The [Package](https://mastering-shiny.org/scaling-packaging.html) Chapter mentions `roxygen2` (and that it's beyond the scope of this book), but for now just know `roxygen2` syntax is placed in the code below `R/` is to create the help files and the `NAMESPACE`.

[^pkgs-5]: Testing is also covered in [Mastering Shiny](https://mastering-shiny.org/scaling-testing.html), but `monthApp` doesn't have the testing infrastructure provided by [`testthat`](https://testthat.r-lib.org/) (and can be quickly created using `usethis::use_testthat()`).

None of the items above make the `monthApp` example from Mastering Shiny incomplete or incorrect--it's been written for an audience with a particular experience level and skill-set.[^pkgs-6] 

[^pkgs-6]: Mastering shiny doesn't make many assumptions about your experience with package development, but [Engineering Production-Grade Shiny Apps](https://engineering-shiny.org/) and the [`golem` package](https://thinkr-open.github.io/golem/) assumes readers "*are comfortable with building an R package.*"

The contents of this book are intended to broaden that audience.
...
I'll use color to differentiate [**Shiny projects**]{style="color: #00916e"}, [**R packages**]{style='color: #a31621;'}, and [**Shiny app-packages**]{style="color: #009ddc"}
...
-   [**R Package**]{style='color: #a31621;'}: I'll use the term [**R package**]{style='color: #a31621;'} to describe a directory of functions, documentation, or data that can be installed and loaded into an R session. An [**R package**]{style='color: #a31621;'} includes the necessary dependency management (`NAMESPACE`) and metadata files (`DESCRIPTION`) *and* has access to the package development tools in RStudio/Posit Workbench.

-   [**Shiny app-packages**]{style="color: #009ddc;"}: A [**shiny app-package**]{style="color: #009ddc;"} (which you have probably already guessed) is an [**R package**]{style='color: #a31621;'} containing a [**shiny app**]{style="color: #009ddc"}lication. [**shiny app-packages**]{style="color: #009ddc"} have all of the functionality of a standard [**R package**]{style='color: #a31621;'} but also contain the files and folders required to successfully develop, run, and deploy a [**shiny app**]{style="color: #009ddc"}.

### Motivation

There are multiple reasons for deciding to develop a [**shiny app-package**]{style="color: #009ddc;"}: your [**shiny app project**]{style="color: #00916e"} might need to be in an [**R package**]{style='color: #a31621;'} structure due to specific organizational practices around deploying applications, or maybe you've been convinced it will improve the shareability/reproducibility of your [**shiny app project**]{style="color: #00916e"}. 

Regardless of the reason, you've likely found yourself in one of two circumstances:

1. You want to develop a new app, but want it structured as an [**R package**]{style='color: #a31621;'}.  

2. You've already developed a [**shiny app project**]{style="color: #00916e"}, but now you need to convert it to a [**shiny app-packages**]{style="color: #009ddc;"}

The previous chapter covered a few practices to adopt during shiny development that improve the extensibility of your [**shiny app project**]{style="color: #00916e"} to an [**shiny app-package**]{style="color: #009ddc"}.[^pkgs-7]

[^pkgs-7]: Advanced shiny app project features are covered in the [Project Folders](https://mjfrigaard.github.io/shinyap/shiny.html#project-folders) section of the Shiny apps chapter.
...
### [R packages]{style='font-weight: bold; color: #a31621;'} vs. [Shiny app projects]{style="font-weight: bold; color: #00916e"}

Below is a folder tree with some of the typical files and folders found in [**R packages**]{style='color: #a31621;'}. At first glance, the contents can seem daunting when compared to the folders and files that currently exist in your [**shiny app project**]{style='color: #00916e;'} (like `projApp`):[^pkgs-8]

[^pkgs-8]: Fortunately, [**shiny app-package**]{style="color: #009ddc;"} don't require *all* the files and folders displayed above to gain the functionality and benefits of an [**R package**]{style='color: #a31621;'}
...
[**R packages**]{style='color: #a31621;'} can be easily shared, reused, and reproduced because they all have a familiar structure, and each folder and file plays an essential role in extending R’s capabilities.

This chapter will cover the minimum requirements for an [**R package**]{style='color: #a31621;'}, so you can handle both creating new [**shiny app-packages**]{style="color: #009ddc"} and converting existing [**shiny projects**]{style="color: #00916e"} into [**shiny app-packages**]{style="color: #009ddc"}.

## What makes an [R package]{style='font-weight: bold; color: #a31621;'}?

If you've done some research on [**R packages**]{style='color: #a31621;'}, you've probably encountering one (or both) of the following statements, 
...
We've created 1) a [**shiny app project**]{style='color: #00916e;'}, 2) with a `DESCRIPTION` file, 3) and an `R/` directory, but it's *not a functioning [**R package**]{style='color: #a31621;'}*. 

As we noted above, a functioning [**R package**]{style='color: #a31621;'} can be installed and loaded into an R session *and* has access to the **Build** pane in the IDE.
...
When opened, functioning [**R packages**]{style='color: #a31621;'} have access to the **Build** pane, which allows us to iterate between loading, installing, and testing quickly. But when `projApp` is opened, the **Build** pane is not displayed in the IDE:
...
As we've just learned, the presence of the `DESCRIPTION` file and an `R/` folder are insufficient to turn your [**shiny app project**]{style='color: #00916e;'} into a functioning [**R package**]{style='color: #a31621;'} (or an [**shiny app-packages**]{style="color: #009ddc;"}).

## What *really* makes an R [package]{style='font-weight: bold; color: #a31621;'}?

For our purposes, I'll rewrite the minimum requirements for creating a 'functional' [**R package**]{style='color: #a31621;'}:

> "*A project needs **a `DESCRIPTION` file with specific fields**, a directory of `R/` files, and **a properly configured `.Rproj` file** to be a package.*"

Let's see how each of these requirements works together to convert the contents of `projApp` from a [**shiny app project**]{style="color: #00916e"} into a [**shiny app-package**]{style='color: #a31621;'}.
...
The official R documentation[^pkgs-9] lists the following required fields for the `DESCRIPTION` in [**R packages**]{style='color: #a31621;'}:
...
Below is an example `DESCRIPTION` file for `projApp` with the mandatory fields:[^pkgs-10]
...
I’ve placed the `.Rproj` file from `pkgApp` with the example `.Rproj` file from R Packages (2ed), side-by-side so you can compare them below:[^pkgs-11]
...
I've circled the fields in the `.Rproj` file that illustrate it's configured to work with an [**R package**]{style='color: #a31621'}. Note that in `projApp`, the **Project build tools** are initially set to (**None**) under **Build Tools**:[^pkgs-12]

[^pkgs-12]: The initial **Build Tools** settings (i.e., **(None)**) should help explain the absence of any [**R package**]{style='color: #a31621;'} development fields in the `projApp.Rproj` when we compared it to the version in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) (i.e., `BuildType`, `PackageUseDevtools`, `PackageInstallArgs`, and `PackageRoxygenize`). 
...
-   `BuildType: Package` tells the IDE `projApp` is an [**R package**]{style='color: #a31621;'} and triggers the **Build** pane.
...
-   The fourth option (`PackageRoxygenize`) is available under **Generate documentation with Roxygen** > **Use roxygen to generate**:
    -   These options affect the documentation in an [**R package**]{style='color: #a31621;'}. To match the example from [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) above, make sure **Rd files**, **Collate field**, and **NAMESPACE file** are selected and click **OK**.
...
When the IDE reboots, I can see the **Build** pane has been added, and I can check the [**R package**]{style='color: #a31621;'} functionality by loading the code with **Build** > **Load All**
...
There you have it--`projApp` is a functional [**R package**]{style="color: #a31621"}!

## Functional [R packages]{style='font-weight: bold; color: #a31621;'}
...
In a functional [**R package**]{style='color: #a31621;'}: 
...
The items above will create a functional [**R package**]{style='color: #a31621;'}, but these are the first steps (we haven't developed anything yet!) on our way to an [**shiny app-packages**]{style="color: #009ddc;"}.

Think of the two items above as a two-part process: `devtools` requires specific the `DESCRIPTION` fields,[^pkgs-13] and the IDE requires `.Rproj` fields to trigger the **Build** pane.[^pkgs-14]

[^pkgs-13]: `devtools` *and* the [official R documentation](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-DESCRIPTION-file) require specific fields in the `DESCRIPTION` file
...
## Creating [**shiny app-packages**]{style='font-weight: bold; color: #009ddc;'} with [`create_package()`]{style="font-size: 0.95em"}

The Posit documentation[^pkgs-15] lists the following ways to create [**R packages**]{style='color: #a31621;'}, 

[^pkgs-15]: You can read more about package development in the Posit Workbench IDE in the [Writing R Packages](https://docs.posit.co/ide/user/ide/guide/pkg-devel/writing-packages.html) documentation
...
We now know his information isn't *technically* incorrect, because we just demonstrated it's possible to create an [**R package**]{style='color: #a31621;'} without the `usethis::create_package()` function. 

Still, if you're looking for a way to quickly create or convert your [**shiny app project**]{style="color: #00916e"} into a [**shiny app-package**]{style='color: #009ddc;'}, using `create_package()` is a great option.[^pkgs-16] I've provided some suggestions below for using `create_package()`.
...
### New [shiny app-packages]{style="font-weight: bold; color: #009ddc"}

If you want to create a new shiny [**shiny app-packages**]{style="color: #009ddc;"}, but haven’t written any code, you can create it with the `create_package()` function from the [`usethis` package](https://usethis.r-lib.org/reference/create_package.html).
...
Assuming your current working directory is where you want your new [**shiny app-packages**]{style="color: #009ddc;"}, call `usethis::create_package()` with the `path` argument set to `getwd()`
...
You're now free to develop `newApp`. Store and document any `.R` files in the `R/` folder, edit the `DESCRIPTION` file with details about the application, read through  [R Packages](https://r-pkgs.org/) and [Mastering Shiny](https://mastering-shiny.org/), and add the shiny code to complete your [**shiny app-package**]{style="color: #009ddc;"}.

### Converting existing [shiny app projects]{style="font-weight: bold; color: #00916e"}

If you already have a [**shiny app project**]{style='color: #00916e;'} that needs to be converted into an [**shiny app-package**]{style="color: #009ddc;"} (like the app files stored in the [`03_projApp` branch](https://github.com/mjfrigaard/projApp/tree/03_projApp)), you can also use `create_package(path = getwd())` in your root folder, but I recommend using the following arguments:
...
-   Two additional arguments from `use_description()` are passed to `create_package()`:
    ...
    -   **`check_name`**: verifies your [**shiny app-package**]{style="color: #009ddc;"} name is valid for CRAN, so we can set this to `FALSE` (unless you're planning on submitting to CRAN)
...
The [**shiny app-packages**]{style="color: #009ddc;"} structure is below:
...
This chapter has covered the mandatory fields in the `DESCRIPTION` file and the [**R package**]{style='color: #a31621;'} configuration fields in `.Rproj`. We also covered creating and converting [**shiny app projects**]{style='color: #00916e;'} using the `usethis::create_package()` function.
