...
I'll be using color to differentiate shiny app [projects]{style="font-weight: bold; color: #38B44A"}, R [packages]{style='font-weight: bold; color: #E74A2F;'}, and [app-packages]{style="font-weight: bold; color: #007bff"}.

:::

-   [project]{style="font-weight: bold; color: #38B44A"}: I consider a 'project' to be any directory of files requiring R (the binary downloaded from [CRAN](https://cran.r-project.org/), i.e., '*R 4.3.1 "Beagle Scouts" released on ...*') to execute with an RStudio/Posit workbench project file (`.Rproj`).

-   [package]{style='font-weight: bold; color: #E74A2F;'}: I'll be using the term 'package' to describe a directory of functions, documentation, and/or data can be installed and loaded into an R session. A [package]{style='font-weight: bold; color: #E74A2F;'} includes the necessary dependency management (`NAMESPACE`) and metadata files (`DESCRIPTION`) *and* has access to the package development tools in RStudio/Posit Workbench.

-   [app-package]{style="font-weight: bold; color: #007bff"}: an app-package (which you have probably already guessed) is a [package]{style='font-weight: bold; color: #E74A2F;'} containing a shiny application. [App-packages]{style="font-weight: bold; color: #007bff"} have all of the functionality of a standard R [package]{style='font-weight: bold; color: #E74A2F;'}, but also contain the files and folders required to successfully develop, run, and deploy a shiny application.
...
Posit Workbench (formerly RStudio) is a popular integrated development environment (IDE) that streamlines many of the R package development tasks. I've purposely connected Posit Workbench--specifically, the package development tools provided in the **Build** pane and `devtools`--to the definitions above for [package]{style='font-weight: bold; color: #E74A2F;'} and [app-package]{style='font-weight: bold; color: #007bff'} above. 
...
The previous chapter covered a few practices to adopt during shiny development that improves the extensibility of your app [project]{style="font-weight: bold; color: #38B44A"} to an [app-package]{style="font-weight: bold; color: #007bff"} (these are summarized in the callout box below).
...
## [App-packages]{style='font-weight: bold; color: #007bff'}: motivation

There are multiple reasons for deciding to develop an [app-package]{style='font-weight: bold; color: #007bff'}: your app [project]{style="font-weight: bold; color: #38B44A"} might have to be in a package structure due to specific organizational needs or practices around deploying applications; or maybe you've been convinced it's the best practice for improving the shareability/reproducibility of your app [project]{style="font-weight: bold; color: #38B44A"}. 

Regardless of the reason, you've likely found yourself in one of two circumstances:

1. You want to develop a new shiny app, but want it structured as an [app-package]{style='font-weight: bold; color: #007bff'}.  

2. You've already developed a shiny app [project]{style="font-weight: bold; color: #38B44A"}, but now you need to convert it to an [app-package]{style='font-weight: bold; color: #007bff'}

### R [package]{style='font-weight: bold; color: #E74A2F;'} vs. shiny [project]{style="font-weight: bold; color: #38B44A"}

Below is a folder tree with some of the typical files and folders found in R [packages]{style='font-weight: bold; color: #E74A2F;'}:
...
The reason R [packages]{style='font-weight: bold; color: #E74A2F;'} can be easily shared, reused, and reproduced is because they all have a familiar structure, and each folder and file plays an essential role in extending R’s capabilities. 

At first glance, the contents above can seem daunting when compared to the folders and files that currently exist in your app [project]{style='font-weight: bold; color: #38B44A;'} (like `projApp` below): 
...
Fortunately, [app-packages]{style="font-weight: bold; color: #007bff"} don't require *all* the files and folders displayed above to gain the functionality and benefits of an R [package]{style='font-weight: bold; color: #E74A2F;'}. 

This chapter will cover what's required in all R [packages]{style='font-weight: bold; color: #E74A2F;'}, so you'll be able to handle both creating new [app-packages]{style='font-weight: bold; color: #007bff;'} and converting existing shiny [projects]{style="font-weight: bold; color: #38B44A"} into [app-packages]{style='font-weight: bold; color: #007bff;'}

## What makes an R [package]{style='font-weight: bold; color: #E74A2F;'}?
...
If you've been following along with the code, these quotes should confuse you. The `projApp` shiny [project]{style='font-weight: bold; color: #38B44A;'} has a `DESCRIPTION` file and an `R/` directory, but `projApp` is not a functioning [package]{style='font-weight: bold; color: #E74A2F;'}. 

As we defined above, a functioning [package]{style='font-weight: bold; color: #E74A2F;'} can be installed and loaded into an R session, *and* has access to the **Build** pane in the IDE.
...
When opened, functioning [packages]{style='font-weight: bold; color: #E74A2F;'} have access to the **Build** pane, which allows us to quickly iterate between loading, installing, and testing. But when `projApp` is opened, the **Build** pane is not displayed in the IDE:
...
As we've just learned, the presence of the `DESCRIPTION` file and an `R/` folder are not sufficient to turn your app [project]{style='font-weight: bold; color: #38B44A;'} into a [package]{style='font-weight: bold; color: #E74A2F;'} (i.e., or an [app-package]{style='font-weight: bold; color: #007bff'}).

## What *really* makes an R [package]{style='font-weight: bold; color: #E74A2F;'}?

For our purposes, I'll rewrite the minimum requirements for creating a 'functional' R package:

> "*A [project]{style="font-weight: bold; color: #38B44A"} needs **a `DESCRIPTION` file with specific fields**, a directory of `R/` files, and **a properly configured `.Rproj` file** to be a [package]{style='font-weight: bold; color: #E74A2F;'}.*" - Me

Let's see how each of these requirements work together to convert the contents of `projApp` from a [project]{style="font-weight: bold; color: #38B44A"} into a [package]{style='font-weight: bold; color: #E74A2F;'}.

### [`DESCRIPTION`]{style="font-size: 0.95em"} fields

The [official R documentation](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-DESCRIPTION-file) lists the following required fields for the `DESCRIPTION` in R [packages]{style='font-weight: bold; color: #E74A2F;'}:
...
### [Project]{style="font-weight: bold; color: #38B44A"} options 
...
#### Default [project]{style="font-weight: bold; color: #38B44A"} options

**Tools** > **Project options** provide access to the [project]{style="font-weight: bold; color: #38B44A"}-level options. For example, fields 2 - 4 are available under **General**, fields 5 - 8 affect the **Code** options, and the final two fields deal with **Sweave**. 
...
These are the default settings that were created when we selected the Shiny App [project]{style='font-weight: bold; color: #38B44A;'} from the **New Project Wizard** in the last chapter.
...
I've circled the fields in the [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) `.Rproj` file that illustrate it's configured to work with a [package]{style='font-weight: bold; color: #E74A2F'}. Note that in `projApp`, the **Project build tools** are initially set to (**None**) under **Build Tools**:
...
This setting should help explain the absence of any [package]{style='font-weight: bold; color: #E74A2F;'} development fields in the `projApp.Rproj` when we compared it to the version in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) (i.e., `BuildType`, `PackageUseDevtools`, `PackageInstallArgs`, and `PackageRoxygenize`). 
...
-   `BuildType: Package` tells the IDE `projApp` is a [package]{style='font-weight: bold; color: #E74A2F;'} and triggers the **Build** pane.
...
-   The fourth option (`PackageRoxygenize`) is available under **Generate documentation with Roxygen** > **Use roxygen to generate**
    -   These options affect the documentation in your [package]{style='font-weight: bold; color: #E74A2F;'}. To match the example from [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) above, make sure **Rd files**, **Collate field**, and **NAMESPACE file** are selected and click **OK**
...
When the new session starts, the new [project]{style="font-weight: bold; color: #38B44A"}-level options will activate the **Build** pane in the IDE. 
...
When the IDE reboots, I can see the **Build** pane has been added, and I can check the [package]{style='font-weight: bold; color: #E74A2F;'} functionality by loading the code with **Build** > **Load All**
...
There you have it--`projApp` is a functional [package]{style="font-weight: bold; color: #E74A2F"}!

## Functional R [packages]{style='font-weight: bold; color: #E74A2F;'}
...
A functional R [package]{style='font-weight: bold; color: #E74A2F;'}: 
...
The items above will create a functional R [package]{style='font-weight: bold; color: #E74A2F;'}, but these are only the first steps (we haven't developed anything yet!) on our way to an [app-package]{style='font-weight: bold; color: #007bff'}.

Think of the two items above as two-part process: the `DESCRIPTION` fields are required by the [official R documentation](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-DESCRIPTION-file) (and hence, `devtools`), and the `.Rproj` fields are required by the IDE trigger the **Build** pane. For more information on the topics in this chapter, you should read
...
The [Posit documentation](https://docs.posit.co/ide/user/ide/guide/pkg-devel/writing-packages.html) lists the following ways to create R [packages]{style='font-weight: bold; color: #E74A2F;'}, 
...
While this information isn't incorrect, we did just demonstrate it's possible to create a [package]{style='font-weight: bold; color: #E74A2F;'} without the `usethis::create_package()` function. Still, if you're looking for a way to quickly create or convert your app [project]{style="font-weight: bold; color: #38B44A"} into an [app-package]{style='font-weight: bold; color: #007bff;'}, using `create_package()` is a great option. 
...
You're now free to develop `newApp`. Store and document any `.R` files in the `R/` folder, edit the `DESCRIPTION` file with details about the application, read through  [R Packages](https://r-pkgs.org/) and [Mastering Shiny](https://mastering-shiny.org/), and add the shiny code to complete your [app-package]{style='font-weight: bold; color: #007bff'}

### Converting existing app

If you already have a shiny app [project]{style='font-weight: bold; color: #38B44A'} that needs to be converted into an [app-package]{style='font-weight: bold; color: #007bff'} (like the app files stored in the [`03_projApp` branch](https://github.com/mjfrigaard/projApp/tree/03_projApp)), you can also use `create_package(path = getwd())` in your root folder, but I recommend using the following arguments:

#### [`DESCRIPTION`]{style="font-size: 0.95em"} arguments

-   **`fields`**: these are arguments passed to `usethis::use_description()`. If `fields` is empty, a boilerplate `DESCRIPTION` file is created (similar to [this one](https://r-pkgs.org/description.html#the-description-file)). 
...
-   Two additional arguments from `use_description()` are passed to `create_package()`:
    ...
    -   **`check_name`**: verifies your [app-package]{style='font-weight: bold; color: #007bff'} name is valid for CRAN, so we can set this to `FALSE` (unless you're planning on submitting to CRAN)
...
When converting your existing shiny app [project]{style='font-weight: bold; color: #38B44A'} into am [app-package]{style='font-weight: bold; color: #007bff'} with `usethis::create_package()`, don't use `'.'` in the `path` argument:
  
`\`\`\r
usethis::create_package('.')
`\`\`\
  
This will return the following warning about creating nested [projects]{style='font-weight: bold; color: #38B44A'}, and ask if you want to proceed anyway:
...
We can avoid this warning altogether by passing the `getwd()` to the `path` argument, so I recommend cancelling the [project]{style='font-weight: bold; color: #38B44A'} creation:
...
The [app-package]{style='font-weight: bold; color: #007bff'} structure is below:
...
This chapter has covered the mandatory fields in the `DESCRIPTION` file and the [package]{style='font-weight: bold; color: #E74A2F;'} configuration fields in `.Rproj`. We also covered creating and converting [projects]{style='font-weight: bold; color: #38B44A'} using the `usethis::create_package()` function.
...
-   **The `DESCRIPTION` file** ultimately controls whether you have a functional shiny [package]{style='font-weight: bold; color: #E74A2F;'}
    -   The mandatory fields are: `Package`, `Version`, `License`, `Description`, `Title`, `Author`, and `Maintainer`

-   `usethis::create_package()` can be used to create a new [package]{style='font-weight: bold; color: #E74A2F;'} and to convert an existing shiny [project]{style='font-weight: bold; color: #38B44A;'} into a shiny [app-package]{style='font-weight: bold; color: #007bff'}

-   The IDE reads RStudio (`.Rproj`) files and determines R session settings at the [project]{style='font-weight: bold; color: #38B44A;'}-level (i.e., working directory, workspace, history, code formatting, etc.)

    -   [Package]{style='font-weight: bold; color: #E74A2F;'} development settings can be accessed via **Project Options** > **Build Tools**
