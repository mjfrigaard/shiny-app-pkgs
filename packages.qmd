...
![](images/rpkg_icon.png){fig-align="right"}
...
co_box(
  ...
  header = "![](images/rpkg_icon.png){width='10%'} &emsp; TLDR", 
  fold = TRUE,
  contents = "
...
I'll use color to differentiate Shiny app [projects]{style="font-weight: bold; color: #00916e"}, R [packages]{style='font-weight: bold; color: #ad343e;'}, and Shiny [app-packages]{style="font-weight: bold; color: #00a6ed"}

:::

-   **Shiny** [Projects]{style="font-weight: bold; color: #00916e"}: I consider a Shiny app [project]{style="font-weight: bold; color: #00916e"} to be any directory of files requiring R (the binary downloaded from [CRAN](https://cran.r-project.org/) with a version number and neat names like, '*R 4.3.1 "Beagle Scouts" released on ...*') to execute with an RStudio/Posit workbench project file (`.Rproj`) and an application (contained in an `app.R` file or `ui.R`/`server.R` files).

-   **R** [Packages]{style='font-weight: bold; color: #ad343e;'}: I'll use the term R [package]{style='font-weight: bold; color: #ad343e;'} to describe a directory of functions, documentation, or data that can be installed and loaded into an R session. An R [package]{style='font-weight: bold; color: #ad343e;'} includes the necessary dependency management (`NAMESPACE`) and metadata files (`DESCRIPTION`) *and* has access to the package development tools in RStudio/Posit Workbench.

-   **Shiny** [app-packages]{style="font-weight: bold; color: #00a6ed;"}: A Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"} (which you have probably already guessed) is an R [package]{style='font-weight: bold; color: #ad343e;'} containing a Shiny [app]{style="font-weight: bold; color: #00a6ed"}lication. Shiny [app-packages]{style="font-weight: bold; color: #00a6ed"} have all of the functionality of a standard R [Package]{style='font-weight: bold; color: #ad343e;'} but also contain the files and folders required to successfully develop, run, and deploy a Shiny app.

### Motivation

There are multiple reasons for deciding to develop a Shiny [app-package]{style="font-weight: bold; color: #00a6ed"}: your Shiny app [project]{style="font-weight: bold; color: #00916e"} might need to be in an R [package]{style='font-weight: bold; color: #ad343e;'} structure due to specific organizational practices around deploying applications, or maybe you've been convinced it will improve the shareability/reproducibility of your Shiny [project]{style="font-weight: bold; color: #00916e"}. 

Regardless of the reason, you've likely found yourself in one of two circumstances:

1. You want to develop a new app, but want it structured as an R [package]{style='font-weight: bold; color: #ad343e;'}.  

2. You've already developed a Shiny app [project]{style="font-weight: bold; color: #00916e"}, but now you need to convert it to a Shiny [app-packages]{style="font-weight: bold; color: #00a6ed;"}

The previous chapter covered a few practices to adopt during Shiny development that improve the extensibility of your Shiny app [project]{style="font-weight: bold; color: #00916e"} and make it easier to convert into a Shiny [app-package]{style="font-weight: bold; color: #00a6ed"}.[^pkgs-7]

[^pkgs-7]: Shiny app project features are covered in [this section](shiny.qmd#project-folders) of the Shiny chapter.
...
### R [packages]{style='font-weight: bold; color: #ad343e;'} vs. Shiny [projects]{style="font-weight: bold; color: #00916e"}

Below are folder trees with some of the typical files and folders found in R [packages]{style='font-weight: bold; color: #ad343e;'} (on the left) and the files that currently exist in `moviesApp` (on the right), our Shiny app [project]{style='font-weight: bold; color: #00916e;'}. 
...
R [packages]{style='font-weight: bold; color: #ad343e;'} can be easily shared, reused, and reproduced because they all have a familiar structure, and each folder and file plays an essential role in extending R’s capabilities.[^pkgs-8]

[^pkgs-8]: Fortunately, Shiny [app-packages]{style="font-weight: bold; color: #00a6ed;"} don't require *all* the files and folders displayed in the folder tree to gain the functionality and benefits of an R [package]{style='font-weight: bold; color: #ad343e;'}. 

This chapter will cover the minimum requirements for an R [package]{style='font-weight: bold; color: #ad343e;'}, so you can handle both creating new Shiny [app-packages]{style="font-weight: bold; color: #00a6ed"} and converting existing Shiny [ projects]{style="font-weight: bold; color: #00916e;"} into Shiny [app-packages]{style="font-weight: bold; color: #00a6ed"}.

## What makes an R [package]{style='font-weight: bold; color: #ad343e;'}?

If you've done some research on R [packages]{style='font-weight: bold; color: #ad343e;'}, you've probably encountered one (or both) of the following statements, 
...
We've created a Shiny app [project]{style='font-weight: bold; color: #00916e;'} (`moviesApp`) with a `DESCRIPTION` file and an `R/` directory, but it's not a functioning R [package]{style='font-weight: bold; color: #ad343e;'}. 

As we noted above, a functioning R [package]{style='font-weight: bold; color: #ad343e;'} can be installed and loaded into an R session *and* has access to the **Build** pane in the IDE.
...
When opened, functioning R [packages]{style='font-weight: bold; color: #ad343e;'} have access to the **Build** pane, which allows developers to quickly load, install, and test their packages. 
...
![`moviesApp` IDE](images/pkgs_no_build_pane.png){#fig-pkgs_no_build_pane height="100%" width="100%" fig-align="center"}
...
As we've just learned, the presence of the `DESCRIPTION` file and an `R/` folder are insufficient to turn a Shiny app [project]{style='font-weight: bold; color: #00916e;'} into a functioning R [package]{style='font-weight: bold; color: #ad343e;'} (or a Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"}).

## What *really* makes an R [package]{style='font-weight: bold; color: #ad343e;'}? {#sec-packages-essential-ingredients}
...
Let's see how each of these requirements work together to convert the contents of `moviesApp` from a Shiny [project]{style="font-weight: bold; color: #00916e"} into a Shiny [app-package]{style='font-weight: bold; color: #00a6ed;'}.

### [`DESCRIPTION`]{style="font-size: 0.95em"} fields

```{r}
#| label: git_box_moviesApp_04_description
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
The official R documentation[^pkgs-9] lists the following required fields for the `DESCRIPTION` in R [packages]{style='font-weight: bold; color: #ad343e;'}:
...
The default settings were created when we selected the Shiny app [project]{style='font-weight: bold; color: #00916e;'} from the **New Project Wizard** in the last chapter.
...
I've circled the fields in the `.Rproj` file that illustrate it's configured to work with an R [package]{style='font-weight: bold; color: #ad343e'}. Note that in `moviesApp`, the **Project build tools** are initially set to (**None**) under **Build Tools**:
...
[^pkgs-12]: The initial **Build Tools** settings (i.e., **(None)**) should help explain the absence of any R [package]{style='font-weight: bold; color: #ad343e;'} development fields in the `moviesApp.Rproj` when we compared it to the version in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) (i.e., `BuildType`, `PackageUseDevtools`, `PackageInstallArgs`, and `PackageRoxygenize`). 
...
-   `BuildType: Package` tells the IDE `moviesApp` is an R [package]{style='font-weight: bold; color: #ad343e;'} and triggers the **Build** pane.
...
-   The fourth option (`PackageRoxygenize`) is available under **Generate documentation with Roxygen** > **Use roxygen to generate**:
    -   These options affect the documentation in an R [package]{style='font-weight: bold; color: #ad343e;'}. To match the example from [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) above, make sure **Rd files**, **Collate field**, and **NAMESPACE file** are selected and click **OK**.
...
```{r}
#| label: git_box_moviesApp_05_rproj
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
::: {#fig-pkgs_shiny_app_build_pane}
![Build pane in IDE](images/pkgs_shiny_app_build_pane.png){#fig-pkgs_shiny_app_build_pane height="100%" width="100%" fig-align="center"}
...
When the IDE reboots, I can see the **Build** pane has been added, and I can check the R [package]{style='font-weight: bold; color: #ad343e;'} functionality by loading the code with **Build** > **Load All**
...
There you have it--`moviesApp` is a functional [**R package**]{style="color: #ad343e"}!
...
In a functional R [package]{style='font-weight: bold; color: #ad343e;'}: 
...
Think of the two items above as a two-part process: the `DESCRIPTION` requires specific fields,[^pkgs-13] and the IDE requires `.Rproj` fields to trigger the **Build** pane.
...
The Posit documentation lists the following way(s) to create R [packages]{style='font-weight: bold; color: #ad343e;'}:[^pkgs-15a] 
...
`create_package()` is a great option if you're looking for a way to quickly create or convert your Shiny [project]{style="font-weight: bold; color: #00916e"} into a Shiny [app-package]{style='font-weight: bold; color: #00a6ed;'}. In the following sections I'll cover some suggestions for using `create_package()`.[^pkgs-16]
...
### New [Shiny app-packages]{style="font-weight: bold; color: #00a6ed"} {#sec-create-new-app-packages}

If you haven't written any code and want to create a new Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"}, [`create_package()`](https://usethis.r-lib.org/reference/create_package.html) is the quickest way to get started.
...
Assuming your current working directory is where you want your new Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"}, call `usethis::create_package()` with the `path` argument set to `getwd()`
...
You're now free to develop `newApp`. Store and document any `.R` files in the `R/` folder, edit the `DESCRIPTION` file with details about the application, read through  [R Packages](https://r-pkgs.org/) and [Mastering Shiny](https://mastering-Shiny.org/), and add the Shiny code to complete your Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"}.

### Converting existing Shiny [projects]{style="font-weight: bold; color: #00916e"} {#sec-convert-shiny-app-packages}

If you already have a Shiny app [project]{style='font-weight: bold; color: #00916e;'} that needs to be converted into a Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"} (like the app files stored in the [`03_proj-app` branch](https://github.com/mjfrigaard/moviesApp/tree/03_proj-app)), you can also use `create_package(path = getwd())` in your root folder, but I recommend using the following arguments:
...
-   Two additional arguments from `use_description()` are passed to `create_package()`:
    ...
    -   **`check_name`**: verifies your Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"} name is valid for CRAN, so we can set this to `FALSE` (unless you're planning on submitting to CRAN)
...
The Shiny [app-package]{style="font-weight: bold; color: #00a6ed;"} structure is below:
...
```{r}
#| label: git_box_moviesApp_06a_create-package
#| echo: false
#| results: asis
#| eval: true
git_margin_box(
...
This chapter has covered the mandatory fields in the `DESCRIPTION` file and the R [package]{style='font-weight: bold; color: #ad343e;'} configuration fields in `.Rproj`. We also covered creating and converting Shiny app [projects]{style='font-weight: bold; color: #00916e;'} using the `usethis::create_package()` function.
