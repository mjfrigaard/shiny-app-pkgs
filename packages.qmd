...
co_box(
  ...
  header = "Caution",
  contents = "This section is still being developed--its contents are subject to change.",
  fold = FALSE
)
```
...
Below are folder trees with some of the typical files and folders found in R [packages]{style='font-weight: bold; color: #ad343e;'} (on the left) and the files that currently exist in `shinyAppProj` (on the right), our Shiny app [project]{style='font-weight: bold; color: #00916e;'}. 
...
We've created a Shiny app [project]{style='font-weight: bold; color: #00916e;'} (`shinyAppProj`) with a `DESCRIPTION` file and an `R/` directory, but it's not a functioning R [package]{style='font-weight: bold; color: #ad343e;'}. 
...
Let's see how each of these requirements work together to convert the contents of `shinyAppProj` from a Shiny [project]{style="font-weight: bold; color: #00916e"} into a Shiny [app-package]{style='font-weight: bold; color: #00a6ed;'}.
...
I've placed the `.Rproj` file from `shinyAppProj` with the example `.Rproj` file from [R Packages, (2ed)](https://r-pkgs.org/) side-by-side so you can compare them below:[^pkgs-11]
...
I've circled the fields in the `.Rproj` file that illustrate it's configured to work with an R [package]{style='font-weight: bold; color: #ad343e'}. Note that in `shinyAppProj`, the **Project build tools** are initially set to (**None**) under **Build Tools**:
...
[^pkgs-12]: The initial **Build Tools** settings (i.e., **(None)**) should help explain the absence of any R [package]{style='font-weight: bold; color: #ad343e;'} development fields in the `shinyAppProj.Rproj` when we compared it to the version in [R Packages, 2ed](https://r-pkgs.org/workflow101.html#what-makes-an-rstudio-project) (i.e., `BuildType`, `PackageUseDevtools`, `PackageInstallArgs`, and `PackageRoxygenize`). 
...
-   `BuildType: Package` tells the IDE `shinyAppProj` is an R [package]{style='font-weight: bold; color: #ad343e;'} and triggers the **Build** pane.
