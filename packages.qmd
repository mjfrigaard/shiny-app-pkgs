...
After adding the mandatory fields to the `DESCRIPTION` file in `shinyAppProj`, `load_all()` runs without the previous error, but the IDE still doesn't display the **Build** pane: 
